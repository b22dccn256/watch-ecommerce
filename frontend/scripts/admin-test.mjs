...
async function runAdminTests() {
  ...
  try {
    ...
    for (const tab of tabs) {
      const tabBtn = page
        .locator(
          `button:has-text("${["Dashboard", "Đơn hàng", "Danh mục", "Sản phẩm", "Kho hàng", "Marketing", "Email", "Reviews & Q&A", "Mã giảm giá", "Người dùng", "AI System", "Giao diện"][tabs.indexOf(tab)]}")`,
        )
        .first();
    ...
    const hasDuplicateKeyWarning = consoleLogs.some(
      (log) =>
    ...
    const hasReactErrors = consoleErrors.some(
      (err) =>
        err.includes("error") ||
        err.includes("Error") ||
    ...
    console.log(
      `  Duplicate key warnings: ${hasDuplicateKeyWarning ? "Yes" : "No"}`,
    );
