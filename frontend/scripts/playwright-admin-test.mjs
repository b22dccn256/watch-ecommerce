...
(async () => {
  ...
  const devSessionUrl =
    "http://localhost:5002/api/__dev/set-session?email=admin@watchstore.com";
  ...
  const t0 = Date.now();
  await page.goto("http://localhost:5174/admin?tab=products", {
    waitUntil: "networkidle",
  });
  console.log("Admin page loaded in", Date.now() - t0, "ms");

  // wait for products table
  await page.waitForSelector('button[title="Chỉnh sửa sản phẩm"]', {
    timeout: 10000,
  });
  ...
  try {
    await page.waitForSelector('h2:text("Chỉnh Sửa Sản Phẩm")', {
      timeout: 20000,
    });
  } catch (e) {
    // fallback: wait for any modal h2
    await page.waitForSelector('div[role="dialog"] h2, .fixed h2', {
      timeout: 20000,
    });
  }
