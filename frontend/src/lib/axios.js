  ...
  (response) => {
    try {
      ...
      if (meta && meta.startTime) {
        ...
        console.log(
          `[axios] ${response.config.method?.toUpperCase() || "GET"} ${response.config.url} ${elapsed}ms`,
        );
      }
    } catch (error) {
      void error;
  ...
  async (error) => {
    try {
      ...
      if (meta && meta.startTime) {
        const elapsed = Date.now() - meta.startTime;
        console.log(
          `[axios] ERROR ${error.config?.method?.toUpperCase() || "GET"} ${error.config?.url} ${elapsed}ms`,
        );
      }
    } catch (error) {
      void error;
