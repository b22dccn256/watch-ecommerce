...
const AdminPage = () => {
  ...
  const SidebarNav = () => (
    <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto admin-scroll overflow-x-hidden">
      ...
      <a
        href="/"
        className={`admin-sidebar-link group relative w-full transition-all duration-300 ease-in-out ${isSidebarCollapsed ? "!px-3 !justify-center" : "text-left"}`}
      >
      ...
      <div className="border-t border-gray-100 dark:border-luxury-border my-2" />
  ...
  return (
    <div className="min-h-screen flex bg-gray-50 dark:bg-luxury-dark font-sans" style={{ '--font-display': 'var(--font-sans)' }}>
      ...
      <aside className={`hidden md:flex flex-col flex-shrink-0 bg-white dark:bg-luxury-darker border-r border-gray-100 dark:border-luxury-border h-screen sticky top-0 left-0 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? "w-[4.5rem]" : "w-48"}`}>
        <div className={`flex items-center px-4 py-4 border-b border-gray-100 dark:border-luxury-border overflow-hidden transition-all duration-300 ${isSidebarCollapsed ? "justify-center" : "gap-3"}`}>
          <div className="w-8 h-8 rounded-md bg-luxury-gold flex items-center justify-center flex-shrink-0">
          ...
          <div className={`min-w-0 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? "w-0 opacity-0 hidden" : "w-auto opacity-100"}`}>
            ...
            <p className="text-xs text-gray-400 truncate capitalize">{currentRole}</p>
          </div>
        </div>
            ...
            >
              <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100 dark:border-luxury-border">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-md bg-luxury-gold flex items-center justify-center">
                    <Watch className="w-5 h-5 text-black" />
                  </div>
                  <p className="text-sm font-bold text-gray-900 dark:text-white">Watch Admin</p>
                </div>
                <button onClick={() => setSidebarOpen(false)} className="p-2 rounded-md text-gray-400 hover:text-white transition">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <SidebarNav />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-white dark:bg-luxury-darker border-b border-gray-100 dark:border-luxury-border px-4 py-3 flex items-center justify-between md:hidden admin-topbar">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="p-2 -ml-2 rounded-md text-gray-500 hover:text-luxury-gold hover:bg-gray-100 dark:hover:bg-luxury-border transition">
              <Menu className="w-5 h-5" />
            </button>
            <h1 className="text-base font-bold text-luxury-gold truncate max-w-[150px]">
          ...
          <div className="flex items-center gap-1">
            <a
              href="/"
              className="p-2 rounded-md text-gray-500 hover:text-luxury-gold hover:bg-gray-100 dark:hover:bg-luxury-border transition-colors"
              title="Về trang chủ"
            >
              <Home className="w-5 h-5" />
            </a>
            <button onClick={() => setSearchOpen(true)} className="p-2 rounded-md text-gray-500 hover:bg-gray-100 dark:hover:bg-luxury-border transition-colors">
              <Search className="w-5 h-5" />
            </button>
            <button onClick={() => setNotifOpen(o => !o)} className="p-2 rounded-md text-gray-500 hover:bg-gray-100 dark:hover:bg-luxury-border transition-colors relative">
              <Bell className="w-5 h-5" />
              {notifCount > 0 && <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full" />}
            </button>
          </div>
        </header>

        {/* Desktop topbar — compact */}
        <header className="hidden md:flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-luxury-border bg-white dark:bg-luxury-darker sticky top-0 z-30 gap-6 admin-topbar">
          <motion.div
            key={activeTab}
            ...
            {(() => {
              ...
              return t ? (
                <span className="flex items-center gap-2 text-base font-semibold text-gray-700 dark:text-gray-300">
                  <t.icon className="w-5 h-5 text-luxury-gold" />
                  {t.label}
                </span>
              ) : null;
            })()}
          ...
          >
            ...
            {searchOpen && searchQuery.trim() && (
              <div className="absolute top-full mt-2 left-0 right-0 bg-white dark:bg-luxury-darker border border-gray-100 dark:border-luxury-border rounded-xl shadow-2xl z-50 overflow-hidden max-h-80 overflow-y-auto admin-scroll">
                {searchLoading && <div className="px-4 py-3 text-sm text-gray-400">Đang tìm kiếm...</div>}
                {searchResults.products.length > 0 && (
                  <div>
                    <p className="px-4 py-2 text-xs font-bold text-gray-400 uppercase tracking-widest border-b dark:border-luxury-border">Sản phẩm</p>
                ...
                {searchResults.orders.length > 0 && (
                  <div>
                    <p className="px-4 py-2 text-xs font-bold text-gray-400 uppercase tracking-widest border-b dark:border-luxury-border">Đơn hàng</p>
            ...
            {notifOpen && (
              <div className="absolute right-0 top-full mt-3 w-96 bg-white dark:bg-luxury-darker border border-gray-100 dark:border-luxury-border rounded-xl shadow-2xl z-50 overflow-hidden">
                <div className="px-5 py-4 border-b border-gray-100 dark:border-luxury-border flex items-center justify-between">
                  ...
                  {notifCount > 0 && <span className="text-xs text-luxury-gold font-bold">{notifCount} mới</span>}
                </div>
                <div className="max-h-80 overflow-y-auto divide-y divide-gray-50 dark:divide-luxury-border/30 admin-scroll">
                  {notifications.length === 0 ? (
                    ...
                    <button key={n.id} onClick={() => { handleTabChange(n.tab); setNotifOpen(false); setNotifCount(0); }}
                      ...
                      <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${n.type === "order" ? "bg-amber-400" : "bg-red-400"}`} />
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-bold text-gray-900 dark:text-white">{n.title}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400 truncate mt-1">{n.desc}</p>
                      </div>
                    </button>
                  ))}
                </div>
          ...
          <div className="ml-4 flex items-center gap-3">
            <a
              href="/"
              className="p-2.5 rounded-lg bg-gray-50 dark:bg-luxury-dark border border-gray-200 dark:border-luxury-border text-gray-500 hover:text-luxury-gold hover:border-luxury-gold/30 transition"
              title="Về trang chủ"
            >
              <Home className="w-5 h-5" />
            </a>
            <button 
              ...
              className="p-2.5 rounded-lg bg-gray-50 dark:bg-luxury-dark border border-gray-200 dark:border-luxury-border text-gray-500 hover:text-luxury-gold transition"
              title="Cài đặt"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 p-1.5 px-3 rounded-lg bg-gray-50 dark:bg-luxury-darker border border-gray-200 dark:border-luxury-border hover:border-luxury-gold/30 transition"
              title={user?.name || 'Admin'}
            >
              <div className="w-8 h-8 rounded-full bg-luxury-gold/20 flex items-center justify-center text-xs font-bold text-luxury-gold">
                {user?.name?.substring(0,2)?.toUpperCase() || 'AD'}
              </div>
              <span className="hidden md:inline text-sm text-gray-700 dark:text-gray-200">{user?.name || 'Admin'}</span>
            </button>
          </div>
        </header>

        {/* Tab content — compact */}
        <main className="flex-1 p-4 md:p-6 overflow-auto space-y-5 admin-main">
              ...
              {[
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium opacity-80 mt-1 leading-tight">{item.label}</p>
                  </div>
                </button>
              ))}
            </div>
