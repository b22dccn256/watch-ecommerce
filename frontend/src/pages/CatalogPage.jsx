...
const CatalogPage = () => {
	...
	useEffect(() => {
		...
		if (resetParam === "true") {
			...
			const urlFilters = {};
			if (searchParams.get("brand")) urlFilters.brands = searchParams.get("brand").split(",");
			if (searchParams.get("machineType")) urlFilters.machineType = searchParams.get("machineType").split(",");
			...
			if (searchParams.get("sizeRange")) urlFilters.sizeRange = searchParams.get("sizeRange").split(",");
			if (searchParams.get("caseMaterial")) urlFilters.caseMaterial = searchParams.get("caseMaterial").split(",");
			if (searchParams.get("waterResistance")) urlFilters.waterResistance = searchParams.get("waterResistance").split(",");
			if (searchParams.get("glass")) urlFilters.glass = searchParams.get("glass").split(",");
			if (searchParams.get("functions")) urlFilters.functions = searchParams.get("functions").split(",");
	...
	const buildActiveChips = () => {
		const chips = [];
		if (filters.category) chips.push({ key: "category", label: filterLabel(WATCH_CATEGORY_FILTERS, filters.category) });
		(filters.brands || []).forEach((b) => chips.push({ key: `brands:${b}`, label: b }));
		(filters.machineType || []).forEach((t) => chips.push({ key: `machineType:${t}`, label: filterLabel(MOVEMENT_FILTERS, t) }));
		(filters.strapMaterial || []).forEach((m) => chips.push({ key: `strapMaterial:${m}`, label: filterLabel(STRAP_MATERIAL_FILTERS, m) }));
		(filters.colors || []).forEach((c) => chips.push({ key: `colors:${c}`, label: c }));
		(filters.sizes || []).forEach((s) => chips.push({ key: `sizes:${s}`, label: s }));
		(filters.sizeRange || []).forEach((s) => chips.push({ key: `sizeRange:${s}`, label: filterLabel(SIZE_RANGE_FILTERS, s) }));
		(filters.caseMaterial || []).forEach((m) => chips.push({ key: `caseMaterial:${m}`, label: filterLabel(CASE_MATERIAL_FILTERS, m) }));
		(filters.waterResistance || []).forEach((w) => chips.push({ key: `waterResistance:${w}`, label: filterLabel(WATER_RESISTANCE_FILTERS, w) }));
		(filters.glass || []).forEach((g) => chips.push({ key: `glass:${g}`, label: filterLabel(GLASS_FILTERS, g) }));
		(filters.functions || []).forEach((fn) => chips.push({ key: `functions:${fn}`, label: filterLabel(FUNCTION_FILTERS, fn) }));
					...
					<div className="flex-1 w-full max-w-full overflow-hidden">
						<div className="mb-4 flex items-center text-sm text-gray-500">
							...
							{category && (
								<>
									<span className="mx-2">/</span>
									<span className="text-luxury-gold font-medium">{filterLabel(WATCH_CATEGORY_FILTERS, category)}</span>
								</>
							)}
						</div>
						...
						<div className="flex flex-wrap justify-between items-center mb-4 gap-3 rounded-[1.5rem] border border-black/5 dark:border-white/5 bg-white/85 dark:bg-white/5 px-4 py-4 shadow-sm">
							<div>
								...
								<h2 className="heading-section text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">
									{category ? filterLabel(WATCH_CATEGORY_FILTERS, category) : "Tất cả đồng hồ"}
								</h2>
							</div>
						...
						{activeChips.length > 0 && (
							<div className="flex flex-wrap items-center gap-2 mb-4 rounded-2xl border border-black/5 dark:border-white/5 bg-white/70 dark:bg-white/5 px-4 py-4">
								{activeChips.map((chip) => (
									<button key={chip.key} onClick={() => removeChip(chip.key)} className="filter-chip">
										{chip.label}
										<X className="w-3 h-3" />
									</button>
								))}
								<button onClick={clearAllFilters} className="text-xs text-gray-500 hover:text-[color:var(--color-gold)] underline transition ml-1">
									Xóa tất cả
								</button>
							</div>
						)}
						...
						{loading ? (
							...
							>
								{products.map((product) => (
									<motion.div key={product._id} variants={{ hidden: { opacity: 0, y: 22 }, visible: { opacity: 1, y: 0, transition: { duration: 0.38 } } }}>
