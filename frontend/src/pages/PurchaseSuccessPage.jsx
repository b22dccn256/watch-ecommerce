...
const PurchaseSuccessPage = () => {
  ...
  const handleChangeToCOD = async () => {
    try {
      ...
      const res = await axios.post("/payments/change-payment-method", {
        orderId: order._id,
        method: "cod",
      });
  ...
  useEffect(() => {
    ...
    const orderIdParam = new URLSearchParams(window.location.search).get(
      "order_id",
    );
    const trackingTokenParam = new URLSearchParams(window.location.search).get(
      "tracking_token",
    );
    ...
    if (sessionId) {
      ...
      clearSelectedCart();
      fetchOrderDetailsByToken(
        trackingTokenParam,
        orderIdParam || trackingTokenParam,
      );
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4">
        <div className="bg-surface border border-default rounded-2xl p-8 max-w-md text-center shadow-lg">
          ...
          <Link
            to="/"
            className="inline-flex bg-primary hover:brightness-110 px-6 py-2.5 rounded-xl text-white font-semibold transition"
          >
            Quay về trang chủ
          </Link>
        </div>
      ...
      <div className="max-w-3xl w-full card-premium bg-surface border border-default shadow-xl rounded-2xl overflow-hidden relative z-10 p-6 sm:p-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-secondary text-base mb-4">
            Mã đơn hàng:{" "}
            <span className="font-bold text-[color:var(--color-gold)] tracking-wider">
              {order.orderCode}
            </span>
          </p>
          ...
          {order.trackingToken && (
            <div className="mt-4 bg-surface-soft border border-default rounded-xl p-4 flex flex-col items-center gap-2 max-w-lg mx-auto shadow-sm">
              ...
              <div className="flex items-center gap-2 w-full justify-center">
                <code className="text-[color:var(--color-gold)] font-mono text-xs bg-black/5 dark:bg-black/35 px-3 py-2 rounded-lg border border-default break-all select-all">
                  {order.trackingToken}
                </code>
                <button
                  onClick={() => {
              ...
              <p className="text-[10px] text-muted">
                Dùng mã này để theo dõi tiến trình giao hàng
              </p>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3.5 gap-x-8 text-sm">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-[color:var(--color-gold)]">
        ...
        <div className="bg-surface-soft border border-default rounded-xl p-6 mb-6 shadow-sm">
          <h3 className="text-sm font-bold text-primary mb-4 pb-2 border-b border-default flex items-center gap-2 uppercase tracking-wider">
            <MapPin size={16} className="text-[color:var(--color-gold)]" /> Chi
            tiết giao nhận
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3.5 gap-x-8 text-sm">
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-start gap-4">
                ...
                <span
                  className="font-medium text-primary text-right line-clamp-2"
              ...
              {order.shippingDetails?.orderNotes && (
                <div className="border-t border-default pt-2 mt-2 flex justify-between items-start gap-4">
                  ...
                  <span
                    className="font-medium text-amber-500 dark:text-amber-400 text-right line-clamp-2 text-xs"
                    title={order.shippingDetails?.orderNotes}
                  >
                    {order.shippingDetails?.orderNotes}
                  </span>
                </div>
        ...
        <div className="mb-8">
          <h3 className="text-sm font-bold text-primary mb-4 flex items-center gap-2 uppercase tracking-wider">
            <Package size={16} className="text-[color:var(--color-gold)]" /> Sản
            phẩm đã đặt mua ({order.products?.length || 0})
          </h3>
              ...
              <Fragment key={item._id}>
                <div className="flex items-center gap-4 bg-surface-soft p-4 rounded-xl border border-default shadow-sm">
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className="text-primary font-semibold text-sm truncate"
                    ...
                    <div className="mt-1 flex flex-wrap gap-1.5 text-[10px] text-secondary">
                      {item.selectedColor && (
                        <span className="rounded-md bg-black/5 dark:bg-white/5 border border-default px-2 py-0.5">
        ...
        <div className="bg-[color:var(--color-gold)]/5 border border-[color:var(--color-gold)]/20 rounded-xl p-5 mb-8 flex items-center justify-between shadow-sm">
          ...
          <span className="text-2xl font-bold text-[color:var(--color-gold)] tracking-tight">
            ...
            <span className="text-base font-medium">VNĐ</span>
          </span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 border-t border-default pt-6">
          <Link
            to="/"
            className="flex-1 flex justify-center items-center px-6 py-3.5 rounded-xl border border-default bg-surface hover:bg-surface-soft text-primary font-semibold text-sm transition group"
          >
            <ArrowRight
              size={16}
              className="mr-2 rotate-180 text-secondary group-hover:-translate-x-0.5 transition-transform"
            />
            Tiếp tục mua sắm
          </Link>
          <Link
            to={
              order.trackingToken
                ? `/order-tracking/${order.trackingToken}`
                : "/profile"
            }
