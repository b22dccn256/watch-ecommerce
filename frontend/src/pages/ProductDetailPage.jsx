...
const ProductDetailPage = () => {
  ...
  const { currentProduct, fetchProductBySlug, loading, notFound } =
    useProductStore();
  ...
  const routeSlugToken =
    slugToken || (slug && token ? `${slug}--${token}` : null);
  ...
  useEffect(() => {
    ...
    const canonicalLinkSelector =
      'link[rel="canonical"][data-product-meta="true"]';
  ...
  useEffect(() => {
    ...
    if (
      canonicalPath &&
      currentProduct.slugToken === routeToken &&
      currentProduct.slug !== routeSlug
    ) {
  ...
  useEffect(() => {
    ...
    const firstAvailableWristSize =
      currentProduct.wristSizeOptions?.find((opt) => opt.stock > 0)?.size ||
    ...
    try {
      ...
      list = [
        currentProduct._id,
        ...list.filter((id) => id !== currentProduct._id),
      ];
    ...
    const categoryParam =
      currentProduct.categoryId?._id || currentProduct.category || "";
    axios
      .get(`/products?category=${encodeURIComponent(categoryParam)}&limit=12`)
      .then((res) => {
        const all = (res.data.products || []).filter(
  ...
  const activeStock = useMemo(() => {
    ...
    if (selectedWristSize && currentProduct.wristSizeOptions?.length > 0) {
      const match = currentProduct.wristSizeOptions.find(
        (opt) => opt.size === selectedWristSize,
      );
  ...
  const category =
    currentProduct.categoryId?.name || currentProduct.category || "Đồng hồ";
  const brandName =
    currentProduct.brand?.name || currentProduct.brand || "Không rõ";
  const machineTypeLabel =
    MOVEMENT_LABELS[currentProduct.type] || currentProduct.type || null;
  const specItems = [
      ...
      <div className="mx-auto max-w-[1200px] px-4 sm:px-2">
        ...
        <div className="bg-white rounded-t-xl px-4 pt-4 pb-3 border-b flex flex-wrap items-center justify-between gap-3 shadow-sm">
          ...
          <div className="flex items-center gap-4 text-[13px] text-blue-600">
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-gray-500">
                  {currentProduct.reviewsCount || 0} đánh giá
                </span>
              </div>
            </div>
            <button
            ...
            <button
              ...
              className="flex items-center gap-1 hover:text-blue-800"
            >
              <ArrowLeftRight className="h-4 w-4" /> So sánh
            </button>
          </div>
        </div>
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm">
              <div className="relative aspect-square w-full bg-white flex items-center justify-center rounded-lg border border-gray-100 p-4 mb-4">
                ...
                <div className="absolute top-2 left-2 flex flex-col gap-1">
                  ...
                  <span className="bg-yellow-500 text-white text-[10px] font-bold px-2 py-1 rounded">
                    CHÍNH HÃNG
                  </span>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-2 gap-3 text-[13px] text-gray-700">
                ...
                <div className="flex gap-2">
                  ...
                  <p>
                    Giá sản phẩm đã bao gồm thuế VAT, có hỗ trợ xuất hóa đơn.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div
                  onClick={() => {
                    navigate("/contact");
                    toast.success("Liên hệ tư vấn thu cũ lên đời!");
                  }}
                  className="bg-red-50 border border-red-200 rounded-lg p-2.5 flex items-center gap-3 w-full sm:w-auto hover:bg-red-100 cursor-pointer transition"
                >
                  <div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-bold text-[15px]">
                      Chu vi cổ tay / Đo cắt dây (Miễn phí)
                    </h3>
                    <button
                      type="button"
                    ...
                    {currentProduct.wristSizeOptions.map((option) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            !isOut && setSelectedWristSize(option.size)
                          }
                          disabled={isOut}
                        ...
                        >
                          ...
                          <span className="text-[10px] text-gray-500 font-medium">
                            {isOut ? "Hết hàng" : `Còn ${option.stock} cái`}
                          </span>
                          {isSelected && (
                            <div className="absolute top-0 right-0 w-4 h-4 bg-red-600 rounded-bl-lg rounded-tr-sm flex items-center justify-center">
                              <svg
            ...
            >
              <div className="bg-red-50 px-4 py-2 border-b border-red-200 flex items-center gap-2">
                ...
                <h3 className="font-bold text-[15px] text-red-600 uppercase">
                  Khuyến mãi đi kèm
                </h3>
              </div>
              <div className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg">
                  <div className="bg-red-600 text-white rounded text-center w-14 shrink-0 px-1 py-1">
                    <span className="text-[10px] uppercase font-semibold block border-b border-white/30 pb-0.5 mb-0.5">
                      Giảm
                    </span>
                  ...
                  <div className="flex-1">
                    <p className="font-bold text-[13px]">
                      Voucher ưu đãi thanh toán
                    </p>
                    <p className="text-[11px] text-gray-500">
                      Áp dụng mua cho đơn hàng thanh toán qua VNPay hoặc Stripe.
                    </p>
                  </div>
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText("LUXURYWATCH500K");
                      toast.success("Đã sao chép mã: LUXURYWATCH500K");
                    }}
                    className="text-[12px] font-bold bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded text-red-600 shrink-0"
                  >
                    Lấy mã
                  </button>
                </div>
                ...
                <ul className="text-[13px] space-y-2 text-gray-700">
                  <li className="flex items-start gap-2">
                    ...
                    <span>
                      Giảm ngay <b>1.000.000đ</b> khi mua kèm gói bảo hiểm rơi
                      vỡ, vào nước.{" "}
                      <span
                  ...
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 text-green-500 shrink-0 mt-0.5" />
                    <span>
                      Tặng phiếu mua hàng trị giá <b>200.000đ</b> cho các phụ
                      kiện Dây đeo đồng hồ.
                    </span>
                  </li>
                </ul>
              </div>
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <h3 className="font-bold text-[15px] mb-3 flex items-center gap-2">
                <CreditCard className="h-5 w-5 text-red-600" /> Ưu đãi thanh
                toán
              </h3>
              <div className="flex overflow-x-auto gap-3 pb-2 custom-scrollbar">
                <div className="min-w-[200px] border border-gray-200 rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-8 h-5 bg-blue-100 rounded text-[9px] font-bold text-blue-800 flex items-center justify-center">
                      VIB
                    </div>
                    <p className="text-[12px] font-bold">Mở thẻ VIB</p>
                  </div>
                  <p className="text-[11px] text-gray-600 mb-2">
                    Hoàn tiền đến 2 triệu đồng khi chi tiêu.
                  </p>
                  <span
                ...
                <div className="min-w-[200px] border border-gray-200 rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-8 h-5 bg-black/80 rounded text-[9px] font-bold text-white flex items-center justify-center">
                      STR
                    </div>
                    <p className="text-[12px] font-bold">Thanh toán Stripe</p>
                  </div>
                  <p className="text-[11px] text-gray-600 mb-2">
                    Hỗ trợ thẻ quốc tế cho đơn hàng phù hợp.
                  </p>
                  <span
                    onClick={() => navigate("/checkout")}
                    className="text-[10px] text-blue-600 font-medium cursor-pointer hover:underline"
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              ...
              <p className="text-[12px] mb-2 text-green-600 font-medium">
                Có 12 cửa hàng có sản phẩm
              </p>
              <div className="grid sm:grid-cols-2 gap-2">
                <div className="border border-gray-200 rounded p-2 text-[12px]">
                  <p className="font-medium text-gray-800 mb-1">
                    113-115 Xô Viết Nghệ Tĩnh, Q. Bình Thạnh
                  </p>
                  <div className="flex items-center justify-between text-red-600 font-medium">
                    <a
                      href="tel:02871066115"
                      className="flex items-center gap-1 hover:underline"
                    >
                      <Phone className="h-3 w-3" /> 02871066115
                    </a>
                    <span
                      onClick={() =>
                        window.open(
                          "https://www.google.com/maps/search/113-115+X%C3%B4+Vi%E1%BA%BFt+Ngh%E1%BB%87+T%C4%A9nh+B%C3%ACnh+Th%E1%BA%A1nh",
                          "_blank",
                        )
                      }
                ...
                <div className="border border-gray-200 rounded p-2 text-[12px]">
                  ...
                  <div className="flex items-center justify-between text-red-600 font-medium">
                    <a
                      href="tel:02871066393"
                      className="flex items-center gap-1 hover:underline"
                    >
                      <Phone className="h-3 w-3" /> 02871066393
                    </a>
                    <span
                      onClick={() =>
                        window.open(
                          "https://www.google.com/maps/search/393+%C4%90%C6%B0%E1%BB%9Dng+3%2F2+Qu%E1%BA%ADn+10",
                          "_blank",
                        )
                      }
                      className="flex items-center gap-1 cursor-pointer hover:underline"
            ...
            <div className="space-y-2 sticky bottom-0 bg-white/80 backdrop-blur-md p-2 rounded-xl sm:static sm:bg-transparent sm:p-0 z-50">
              <div className="flex items-center gap-2 text-[13px] text-gray-700 bg-blue-50 px-3 py-2 rounded-lg border border-blue-100 mb-2">
                ...
                <span>
                  Chọn địa chỉ giao hàng để nhận <b>Giao nhanh 2h</b> miễn phí
                </span>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={handleBuyNow}
                  className="flex-1 bg-red-600 hover:bg-red-700 text-white rounded-lg flex flex-col items-center justify-center py-2.5 transition shadow-lg shadow-red-600/30"
                >
                ...
                <button
                  ...
                  className="w-16 sm:w-20 bg-white border-2 border-red-600 rounded-lg flex flex-col items-center justify-center text-red-600 hover:bg-red-50 transition"
                >
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => {
                    toast.success("Chuyển đến trang trả góp 0%");
                    navigate("/checkout");
                  }}
                  className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg flex flex-col items-center justify-center py-2 transition shadow-lg shadow-blue-600/20"
                >
                ...
                <button
                  onClick={() => {
                    toast.success("Chuyển đến trang trả góp qua thẻ");
                    navigate("/checkout");
                  }}
                  className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg flex flex-col items-center justify-center py-2 transition shadow-lg shadow-blue-600/20"
                >
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-12 bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <h2 className="text-[20px] font-bold text-gray-900 mb-4 uppercase">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {relatedProducts.map((p) => (
                <div
                  key={p._id}
                  className="border border-gray-100 rounded-xl overflow-hidden hover:shadow-lg transition bg-white relative"
                >
                  ...
                  <Link to={buildProductPath(p) || "#"}>
                    <img
                      src={p.image}
                      alt={p.name}
                      className="w-full aspect-square object-contain p-2 hover:scale-105 transition duration-300"
                    />
                  </Link>
                  <div className="p-3">
                    <Link
                      to={buildProductPath(p) || "#"}
                      className="font-semibold text-[13px] text-gray-800 line-clamp-2 leading-snug hover:text-red-600"
                    >
                      {p.name}
                    </Link>
                    ...
                    <div className="mt-1 flex gap-1">
                      <span className="bg-gray-100 text-[10px] px-1.5 py-0.5 rounded text-gray-600">
                        Trả góp 0%
                      </span>
        ...
        <RecentlyViewed
          max={6}
          className="mt-8 bg-white rounded-xl p-4 md:p-6 shadow-sm border border-gray-100"
        />
      </div>
    </div>
  );
