...
const Field = ({
  ...
  <div className="space-y-1.5">
    <label
      htmlFor={id}
      className="block text-[10px] uppercase tracking-[0.18em] text-secondary"
    >
      {label}
    </label>
...
const SignUpPage = () => {
  ...
  const validatePasswordStrength = (password) => {
    ...
    const score = [/[a-z]/, /[A-Z]/, /\d/, /[^A-Za-z0-9]/].filter((r) =>
      r.test(password),
    ).length;
  ...
  const validateField = (field, value) => {
    ...
    if (field === "name") {
      if (!value.trim()) msg = "Vui lòng nhập họ và tên";
        ...
        msg = "Chỉ chứa chữ cái và khoảng trắng (2–50 ký tự)";
    }
    if (field === "email") {
    ...
    if (
      field === "phone" &&
      value.trim() &&
      !/^0[35789]\d{8}$/.test(value.trim())
    )
      msg = "Số điện thoại không hợp lệ";
    if (field === "password")
      msg = value ? validatePasswordStrength(value) : "Vui lòng nhập mật khẩu";
  ...
  const handleSubmit = async (e) => {
    ...
    const hasError = Object.keys(formData).some((k) =>
        ...
        >
          ...
          <div className="mb-10 flex items-center gap-3 lg:hidden">
            <div className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[color:var(--color-gold)]/40 bg-[color:var(--color-gold)]/14">
              <span className="text-[10px] font-bold tracking-[0.08em] text-[color:var(--color-gold)]">
                LW
              </span>
            </div>
            <div>
              <p className="font-serif text-base tracking-[0.26em] text-primary">
                LUXURY
              </p>
          ...
          <div className="mb-8 space-y-1.5">
            ...
            <p className="text-sm text-muted">
              Gia nhập cộng đồng những người yêu đồng hồ
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-muted">
                Tối thiểu 8 ký tự, gồm hoa, thường, số
              </p>
            ...
            <button
              type="submit"
              disabled={loading}
              className="btn-base btn-primary mt-2 h-11 w-full"
            >
              {loading ? (
                <>
          ...
          <p className="mt-6 text-center text-sm text-muted">
            ...
            <Link
              to="/login"
              className="font-medium text-[color:var(--color-gold)] transition hover:underline"
            >
