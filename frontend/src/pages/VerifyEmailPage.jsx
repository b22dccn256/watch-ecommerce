...
const VerifyEmailPage = () => {
  ...
  useEffect(() => {
    if (import.meta.env.DEV && pendingEmail && status === "email_sent") {
      ...
      console.log(
        "%cMở terminal chạy backend server để lấy link xác minh.",
        "color: #aaa;",
      );
    }
  }, [pendingEmail, status]);
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      if (pendingEmail) {
        ...
        setMessage(
          `Chúng tôi đã gửi email xác minh đến ${pendingEmail}. Vui lòng kiểm tra hộp thư đến (bao gồm cả thư mục Spam) và nhấn vào liên kết xác minh để kích hoạt tài khoản.`,
        );
      } else {
        setStatus("no_token");
        setMessage(
          "Vui lòng kiểm tra email để tìm liên kết xác minh tài khoản.",
        );
      }
      return;
    }
    ...
    axios
      ...
      .then(async (res) => {
        if (res.data?.alreadyVerified) {
          ...
          setStatus("success");
          setMessage(
            res.data.message || "Tài khoản đã được xác minh thành công.",
          );
      ...
      .catch((error) => {
        ...
        const errMsg =
          ...
          "Không thể xác minh tài khoản. Liên kết có thể đã hết hạn.";
        setMessage(errMsg);
        toast.error(errMsg);
      });
    ...
    <div className="min-h-screen flex items-center justify-center px-4 pt-16 pb-20">
      ...
      >
        <div className="rounded-2xl border border-black/6 bg-surface p-6 sm:p-8 dark:border-white/6 text-center">
          ...
          >
            <StatusIcon
              className={`h-6 w-6 ${status === "loading" ? "animate-spin" : ""} ${statusColor}`}
            />
          </div>
            ...
            {status === "success" && (
              ...
              >
                ...
                <p className="text-[11px] text-muted">
                  Tự động chuyển hướng sau {redirectCountdown}s
                </p>
              </motion.div>
            )}
              ...
              >
                ...
                >
                  {resendLoading ? (
                    ...
                    <>
                      <RefreshCw className="h-4 w-4" /> Gửi lại sau{" "}
                      {resendCooldown}s
                    </>
                  ) : (
                    <>
                ...
                <Link
                  to="/login"
                  className="btn-base btn-outline h-10 w-full text-sm"
                >
                  Quay lại đăng nhập
                </Link>
              </motion.div>
