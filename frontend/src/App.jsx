...
function App() {
  ...
  const { fetchWishlist, mergeWishlist, syncFromLocalStorage } =
    useWishlistStore();
  ...
  useEffect(() => {
    if (!config) return;
    const isDashboard =
      window.location.pathname.startsWith("/secret-dashboard");
  ...
  return (
    <GlobalErrorBoundary>
      <I18nContext.Provider value={{ t, lang, currency }}>
        <div
          className={`min-h-screen relative ${theme === "dark" ? "bg-[color:var(--color-bg)] text-[color:var(--color-primary)]" : "bg-[color:var(--color-bg)] text-[color:var(--color-primary)]"}`}
        >
          <ShimmerStyle />
          <ScrollToTop />

          <div
            className={`relative ${!location.pathname.startsWith("/secret-dashboard") ? (config?.announcementEnabled ? "pt-24" : "pt-16") : ""} min-h-screen flex flex-col`}
          >
              ...
              <div className="mx-auto w-full max-w-screen-xl px-4 sm:px-6 md:px-8 py-2.5 bg-amber-50/95 dark:bg-amber-950/40 border-b border-amber-200 dark:border-amber-800/50 text-amber-800 dark:text-amber-200 flex justify-between items-center gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-semibold">
                    Tài khoản cần xác minh bảo mật
                  </p>
                  <p className="text-xs text-amber-700 dark:text-amber-300/80 mt-0.5">
                    Chúng tôi đã gửi email xác minh. Vui lòng mở email và nhấn
                    nút xác thực để kích hoạt tài khoản.
                  </p>
                </div>
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      toast.success(
                        res.data.message || "Đã gửi lại email xác minh",
                      );
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <Routes location={location}>
                    ...
                    <Route
                      path="/profile"
                      element={privateRoute(<AccountPages />)}
                    />
                    <Route
                      path="/wishlist"
                      element={privateRoute(<WishlistPage />)}
                    />
                    <Route
                    ...
                    <Route
                      ...
                      element={privateRoute(<PurchaseCancelPage />)}
                    />
                    <Route
                      path="/payment/vnpay-return"
                      element={<PaymentReturnPage method="vnpay" />}
                    />
                    <Route
                      path="/product/:slugToken"
                      element={<ProductDetailPage />}
                    />
                    <Route
                      path="/order-tracking/:trackingToken?"
