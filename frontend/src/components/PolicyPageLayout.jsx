    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#f8f5f0_0%,#ffffff_18%,#ffffff_100%)] dark:bg-[radial-gradient(circle_at_top,rgba(212,175,55,0.08)_0%,rgba(15,12,8,1)_45%,rgba(10,10,10,1)_100%)] pb-20">
      ...
      >
        ...
        <div className="mb-8 grid gap-4 rounded-[2rem] editorial-surface px-5 py-5 md:grid-cols-[1.15fr_0.85fr] md:px-7 md:py-6 shadow-[0_24px_80px_-40px_rgba(0,0,0,0.35)]">
          <div>
            <p className="hero-kicker font-serif text-xs font-semibold text-luxury-gold mb-2">
              Trung tâm Hỗ trợ
            </p>
            <p className="font-serif text-sm md:text-base text-gray-600 dark:text-luxury-text-muted leading-relaxed">
              {description}
            </p>
          </div>
          <div className="md:justify-self-end md:text-right flex items-center md:justify-end">
            <h1 className="hero-title text-3xl md:text-5xl text-gray-900 dark:text-white leading-tight">
              {title}
            </h1>
          </div>
        </div>
          ...
          <aside className="w-full lg:w-72 shrink-0">
            <div className="sticky top-28 space-y-4 rounded-[2rem] border border-black/5 dark:border-luxury-border bg-white/90 dark:bg-luxury-darker/90 p-4 shadow-[0_24px_80px_-45px_rgba(0,0,0,0.35)] backdrop-blur-md">
              ...
              {menuItems.map((item) => {
                ...
                const isActive =
                  item.id === activeId || location.pathname === item.path;
                return (
                  <NavLink
                    key={item.id}
                    to={item.path}
                  ...
                  >
                    <Icon
                      className={`w-5 h-5 ${isActive ? "text-luxury-gold" : "text-gray-400"}`}
                    />
                    <span className="flex-1 text-left text-sm font-medium">
                      {item.label}
                    </span>
                    <ChevronRight
                      className={`w-4 h-4 transition-transform ${isActive ? "translate-x-0.5 text-luxury-gold" : "text-gray-300 dark:text-gray-600"}`}
                    />
                  </NavLink>
                );
              })}
              ...
              <div className="rounded-[1.5rem] border border-black/5 dark:border-white/5 bg-gradient-to-br from-[#f8f5ef] to-white dark:from-white/5 dark:to-white/0 p-4">
                <p className="text-[10px] uppercase tracking-[0.24em] text-gray-500 dark:text-luxury-text-muted font-bold mb-3">
                  Thông tin nhanh
                </p>
            ...
            >
              <div
                className={
                  noProse
                    ? ""
                    : "prose prose-luxury dark:prose-invert max-w-none"
                }
              >
                {children}
              </div>
