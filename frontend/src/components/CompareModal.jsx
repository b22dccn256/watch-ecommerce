...
const CompareModal = ({ isOpen, onClose }) => {
  ...
  const compareSlots = [
    ...compareItems,
    ...Array.from({ length: Math.max(0, 3 - compareItems.length) }, () => null),
  ];
              ...
              {compareItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full py-12 text-muted">
                  ...
                  <p className="text-base">
                    Bạn chưa có sản phẩm nào trong danh sách so sánh.
                  </p>
                  <button
                    onClick={onClose}
                    className="mt-6 font-bold text-[color:var(--color-gold)] hover:underline"
                  >
                    Tiếp tục mua sắm
                  </button>
                </div>
                ...
                <div className="min-w-[680px] sm:min-w-[780px]">
                  ...
                  <div className="grid grid-cols-4 gap-4 sticky top-0 bg-surface z-10 pb-4 border-b border-black/8 dark:border-white/8">
                    <div className="col-span-1 bg-surface-soft p-3.5 rounded-2xl border border-black/8 dark:border-white/8 min-h-[320px] flex items-start">
                      <div className="text-muted font-semibold tracking-[0.04em]">
                        TỔNG QUAN
                      </div>
                    </div>
                    {compareSlots.map((item, index) => (
                      <div
                        ...
                        className="col-span-1 relative group bg-surface-soft p-3.5 rounded-2xl border border-black/8 dark:border-white/8 hover:border-[color:var(--color-gold)]/40 transition min-h-[320px] flex flex-col"
                      >
                        {item ? (
                          <>
                            <button
                            ...
                            <img
                              src={item.image}
                              alt={item.name}
                              className="w-36 h-36 mx-auto object-cover rounded-lg mb-3 cursor-pointer"
                              onClick={() => handleNavigate(item)}
                            />
                            <h3
                              className="font-semibold text-base leading-tight mb-1 line-clamp-2 cursor-pointer hover:text-[color:var(--color-gold)]"
                              onClick={() => handleNavigate(item)}
                            >
                              {item.name}
                            </h3>
                            <p className="text-lg font-semibold text-[color:var(--color-gold)] mb-3">
                  ...
                  <div className="mt-6 rounded-2xl border border-black/8 dark:border-white/8 bg-surface-soft/45 overflow-hidden text-sm">
                    ...
                    <div className="grid grid-cols-4 gap-6 items-center border-t border-black/8 dark:border-white/8 px-4 py-4">
                      ...
                      {compareSlots.map((item, index) => (
                        <div key={item?._id || `reserve-${index}`}>
                    ...
                    <div className="grid grid-cols-4 gap-6 items-center border-t border-black/8 dark:border-white/8 px-4 py-4">
                      ...
                      {compareSlots.map((item, index) => (
                        <div key={item?._id || `diameter-${index}`}>
                          {item ? (
                            <>
                              {item.specs?.case?.diameter || "-"} <br />
                              <span className="text-gray-400 text-xs">
                                Độ dày: {item.specs?.case?.thickness || "-"}
                              </span>
                            </>
                          ) : (
                            "-"
