...
export default function OrderDetailModal({
  ...
  const {
    ...
    handleChangeStatus,
    STATUS_LABELS,
  } = useOrderStatus(initialOrder);
  const {
    form,
    handleChange,
    saveDetails: performSave,
    saving,
        ...
        >
          ...
          <div className="mb-5 flex items-start justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={onClose}
                className="rounded-lg p-1.5 text-muted transition hover:text-primary"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
          ...
          {currentStepIndex >= 0 &&
            !["cancelled", "return_requested", "returned"].includes(
              order.status,
            ) && (
              <div className="mb-5 flex items-center gap-1 overflow-x-auto pb-1">
                {STATUS_SEQUENCE.map((step, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`flex flex-col items-center gap-1 ${current ? "text-[color:var(--color-gold)]" : done ? "text-green-600" : "text-muted"}`}
                      >
                        <Icon
                          className={`h-4 w-4 ${current ? "text-[color:var(--color-gold)]" : done ? "text-green-600" : "text-muted opacity-40"}`}
                        />
                      ...
                      {idx < STATUS_SEQUENCE.length - 1 && (
                        <div
                          className={`mx-1 h-px w-6 shrink-0 ${idx < currentStepIndex ? "bg-green-500" : "bg-black/10 dark:bg-white/10"}`}
                        />
                      )}
                    </div>
                  );
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-black/8 p-4 dark:border-white/8">
                ...
                <span
                  className={`mt-1.5 inline-flex items-center rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide ${PAYMENT_STATUS_STYLES[order.paymentStatus] || "bg-surface text-muted"}`}
                >
                  {order.paymentStatus}
                </span>
                {order.coupon && (
                  <p className="mt-2 text-xs text-secondary">
                    ...
                    {order.coupon.type === "fixed"
                      ? `(−${order.coupon.discountValue.toLocaleString("vi-VN")} ₫)`
                      : `(−${order.coupon.discountValue || order.coupon.discountPercentage}%)`}
                  </p>
                )}
              </div>
              ...
              <div className="rounded-xl border border-black/8 p-4 dark:border-white/8 space-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleChange("carrierTrackingNumber", e.target.value)
                    }
