...
const DAYS_OPTIONS = [{ label: "7 ngĂ y", value: 7 }, { label: "30 ngĂ y", value: 30 }, { label: "90 ngĂ y", value: 90 }];
const formatVND = (v) => { if (v >= 1e9) return (v/1e9).toFixed(1)+" tá»·"; if (v >= 1e6) return (v/1e6).toFixed(1)+" tr"; return v?.toLocaleString("vi-VN"); };
    ...
    <div className="bg-white dark:bg-luxury-darker border border-gray-200 dark:border-luxury-border p-3 rounded-xl shadow-xl text-xs">
      ...
      {payload.map(e => (
        <p key={e.name} className="mt-0.5" style={{ color: e.color }}>
          {e.name === "revenue" ? `Doanh thu: ${formatVND(e.value)} â‚«` : `ÄÆ¡n hĂ ng: ${e.value}`}
        </p>
      ))}
    </div>
  );
...
const AnalyticsTab = () => {
  ...
  const handleExportCSV = () => {
    ...
    const csv = ["NgĂ y,ÄÆ¡n hĂ ng,Doanh thu", ...dailySalesData.map(d=>`${d.name},${d.sales},${d.revenue}`)].join("\n");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-bold text-gray-900 dark:text-white">Tá»•ng quan hiá»‡u suáº¥t</h2>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{days} ngĂ y gáº§n nháº¥t</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard loading={isLoading} title="Tá»•ng ngÆ°á»i dĂ¹ng" value={data.users.toLocaleString()} icon={Users} gradient="bg-emerald-500" />
        <KpiCard loading={isLoading} title="Tá»•ng sáº£n pháº©m" value={data.products.toLocaleString()} icon={Package} gradient="bg-blue-500" />
        <KpiCard loading={isLoading} title="ÄÆ¡n hĂ ng" value={data.totalSales.toLocaleString()} icon={ShoppingCart} gradient="bg-violet-500" delta={saleDelta} />
        <KpiCard loading={isLoading} title="Doanh thu" value={formatVND(data.totalRevenue)+" â‚«"} icon={DollarSign} gradient="bg-amber-500" delta={revDelta} />
      </div>

      {/* Secondary KPIs */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label:"GiĂ¡ trá»‹ Ä‘Æ¡n TB (AOV)", value: formatVND(data.aov)+" â‚«" },
          { label:"ÄÆ¡n Ä‘Ă£ thanh toĂ¡n",   value: (data.totalOrdersPlaced||0).toLocaleString()+" Ä‘Æ¡n" },
          { label:"Tá»· lá»‡ chuyá»ƒn Ä‘á»•i",   value: (() => { const r=data.conversionRate>0?data.conversionRate:data.totalSales>0?(data.totalOrdersPlaced/data.totalSales)*100:0; return r>0?r.toFixed(2)+"%":"â€”"; })() },
        ].map(item => (
      ...
      <div className="bg-white dark:bg-luxury-dark border border-gray-100 dark:border-luxury-border rounded-xl p-5">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-gray-900 dark:text-white">Doanh thu & ÄÆ¡n hĂ ng</h3>
          </div>
        ...
        {isLoading ? <ChartSkeleton /> : !hasChart ? (
          <EmptyChart message={`ChÆ°a cĂ³ Ä‘Æ¡n hĂ ng Ä‘Ă£ thanh toĂ¡n trong ${days} ngĂ y qua`} />
        ) : (
            ...
            {chartReady && (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={dailySalesData}>
                  ...
                  <Legend />
                  <Line yAxisId="left" type="monotone" dataKey="sales" stroke="#10b981" strokeWidth={2} dot={false} activeDot={{r:4}} name="ÄÆ¡n hĂ ng" />
                  <Line yAxisId="right" type="monotone" dataKey="revenue" stroke="#c9a96e" strokeWidth={2} dot={false} activeDot={{r:4}} name="Doanh thu (â‚«)" />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-luxury-dark border border-gray-100 dark:border-luxury-border rounded-xl p-5">
          <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">Doanh thu theo Thanh toĂ¡n</h3>
          {isLoading ? <ChartSkeleton /> : !data.paymentStats?.length ? (
            <EmptyChart message="ChÆ°a cĂ³ dá»¯ liá»‡u thanh toĂ¡n" />
          ) : (
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                ...
                <Tooltip formatter={v=>formatVND(v)+" â‚«"} />
                <Legend />
              </PieChart>
            </ResponsiveContainer>
          )}
        ...
        <div className="bg-white dark:bg-luxury-dark border border-gray-100 dark:border-luxury-border rounded-xl p-5">
          <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">Top Size Cá»• Tay</h3>
          {isLoading ? <ChartSkeleton /> : !data.wristSizeStats?.length ? (
            <EmptyChart message="ChÆ°a cĂ³ dá»¯ liá»‡u size" />
          ) : (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={data.wristSizeStats} layout="vertical" margin={{left:8}}>
                ...
                <Bar dataKey="count" fill="#c9a96e" radius={[0,4,4,0]} maxBarSize={20} name="Sá»‘ lÆ°á»£ng" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-luxury-dark border border-gray-100 dark:border-luxury-border rounded-xl p-5">
          <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <Trophy className="w-4 h-4 text-amber-400" /> Top 8 BĂ¡n cháº¡y
          </h3>
          {isLoading ? <div className="space-y-3">{Array(5).fill(0).map((_,i)=><SkeletonBlock key={i} h="h-10"/>)}</div>
          : topProducts.length === 0 ? <p className="text-xs text-gray-400 py-4 text-center">ChÆ°a cĂ³ dá»¯ liá»‡u</p>
          : topProducts.map((p,i) => (
            <div key={p._id} className="flex items-center gap-3 py-2.5 border-b border-gray-100 dark:border-luxury-border/50 last:border-0">
              ...
              <div className="text-right flex-shrink-0">
                <p className="text-xs font-bold text-luxury-gold">{(p.salesCount||0).toLocaleString()} bĂ¡n</p>
                <p className="text-[10px] text-gray-400">Tá»“n: {p.stock}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="bg-white dark:bg-luxury-dark border border-gray-100 dark:border-luxury-border rounded-xl p-5">
          <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-red-400" /> Tá»“n Kho Tháº¥p
          </h3>
          {isLoading ? <div className="space-y-3">{Array(5).fill(0).map((_,i)=><SkeletonBlock key={i} h="h-10"/>)}</div>
          : bottomProducts.length === 0 ? <p className="text-xs text-gray-400 py-4 text-center">KhĂ´ng cĂ³ cáº£nh bĂ¡o đŸ‰</p>
          : bottomProducts.map(p => (
            <div key={p._id} className="flex items-center gap-3 py-2.5 border-b border-gray-100 dark:border-luxury-border/50 last:border-0">
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded text-center w-10 flex-shrink-0 ${p.stock===0?"text-red-400 bg-red-400/10":"text-amber-400 bg-amber-400/10"}`}>
                {p.stock===0?"Háº¾T":p.stock}
              </span>
              {p.image && <img src={p.image} alt={p.name} className="w-8 h-8 rounded object-cover flex-shrink-0" />}
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-900 dark:text-white truncate">{p.name}</p>
                <p className="text-[10px] text-gray-400">NgÆ°á»¡ng: {p.lowStockThreshold}</p>
              </div>
              <p className="text-xs font-bold text-gray-700 dark:text-gray-300 flex-shrink-0">{p.price?.toLocaleString("vi-VN")} â‚«</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-luxury-dark border border-gray-100 dark:border-luxury-border rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-luxury-gold" /> BĂ¡o CĂ¡o P&L
            </h3>
            <span className="text-[10px] text-gray-400">{plData.summary?.days} ngĂ y gáº§n nháº¥t</span>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
            {[
              { label:"Doanh thu",     value:formatVND(plData.summary?.totalRevenue||0)+" â‚«", color:"text-blue-500 dark:text-blue-400" },
              { label:"GiĂ¡ vá»‘n",       value:formatVND(plData.summary?.totalCogs||0)+" â‚«",     color:"text-orange-500 dark:text-orange-400" },
              { label:"Lá»£i nhuáº­n gá»™p", value:formatVND(plData.summary?.totalGrossProfit||0)+" â‚«", color:"text-emerald-500 dark:text-emerald-400" },
              { label:"BiĂªn LN",       value:(plData.summary?.totalMargin||0).toFixed(1)+"%", color:"text-luxury-gold" },
            ].map(item=>(
          ...
          {plData.daily?.length > 0 && (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={plData.daily} margin={{top:4,right:8,left:8,bottom:0}}>
                ...
                <Tooltip formatter={(v,n)=>[formatVND(v)+" â‚«", n==="revenue"?"Doanh thu":n==="cogs"?"GiĂ¡ vá»‘n":"Lá»£i nhuáº­n"]} />
                <Legend formatter={v=>v==="revenue"?"Doanh thu":v==="cogs"?"GiĂ¡ vá»‘n":"Lá»£i nhuáº­n gá»™p"} />
          ...
          <p className="text-[10px] text-gray-400 mt-2">* GiĂ¡ vá»‘n dĂ¹ng costPrice tá»« sáº£n pháº©m. Náº¿u khĂ´ng cĂ³, Æ°á»›c tĂ­nh 60% giĂ¡ bĂ¡n.</p>
        </div>
      )}
    </div>
  );
