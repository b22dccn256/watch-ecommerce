...
const StockBadge = ({ stock }) => {
  ...
  if (stock <= 0)
    return (
      <span className="stock-badge-out px-2 py-0.5 rounded-full text-xs font-semibold">
        Hết hàng
      </span>
    );
  if (stock <= 5)
    return (
      <span className="stock-badge-low px-2 py-0.5 rounded-full text-xs font-semibold">
        Còn {stock}
      </span>
    );
  return (
    <span className="stock-badge-high px-2 py-0.5 rounded-full text-xs font-semibold">
      Còn {stock}
    </span>
  );
};
        ...
        <thead className="bg-gray-50/95 dark:bg-gray-700/95 backdrop-blur-sm sticky top-0 z-10">
          <tr>
            ...
            <th className="px-3 py-3 w-10">
              ...
              >
                {allPageSelected ? (
                  <CheckSquare className="w-4 h-4 text-luxury-gold pointer-events-none" />
                ) : (
                  <Square className="w-4 h-4 pointer-events-none" />
                )}
              </button>
            </th>
            <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider min-w-[350px]">
              Sản phẩm
            </th>
            <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider">
              Thương hiệu
            </th>
            <th className="px-4 py-3 text-right text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider">
              Giá
            </th>
            <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider">
              Bộ máy
            ...
            <tr>
              ...
              >
                ...
                <p className="text-xs mt-1 opacity-60">
                  Thêm sản phẩm đầu tiên bằng nút &quot;Thêm mới&quot;
                </p>
              </td>
            </tr>
          ) : (
                ...
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    {(() => {
                      ...
                      return (
                        <span
                          className="px-2 py-1 bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded text-xs font-medium text-gray-600 dark:text-gray-300 max-w-[170px] truncate inline-block"
                          title={categoryName}
                        >
                          {categoryName}
                        </span>
