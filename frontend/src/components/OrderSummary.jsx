...
const OrderSummary = () => {
  ...
  return (
    ...
    >
      <p className="hero-kicker text-[10px] font-semibold text-luxury-gold">
        Checkout summary
      </p>
      <p className="hero-title text-2xl font-semibold text-gray-900 dark:text-white">
        {t("order_summary")}
      </p>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <dl className="flex items-center justify-between gap-4">
            <dt className="text-base font-normal text-gray-600 dark:text-gray-300">
              {t("subtotal")}
            </dt>
            <dd className="text-base font-medium text-gray-900 dark:text-white">
              {formatCurrency(subtotal, currency, lang)}
            </dd>
          </dl>
          ...
          {savings > 0 && (
            <dl className="flex items-center justify-between gap-4">
              ...
              <dd className="text-base font-medium text-luxury-gold">
                -{formatCurrency(savings, currency, lang)}
              </dd>
            </dl>
          )}
          ...
          {coupon && isCouponApplied && (
            <dl className="flex items-center justify-between gap-4">
              ...
              <dd className="text-base font-medium text-luxury-gold">
                {coupon.type === "fixed"
                  ? `-${formatCurrency(coupon.discountValue, currency, lang)}`
          ...
          <dl className="flex items-center justify-between gap-4">
            ...
            <dd
              className={`text-base font-medium ${shippingFee === 0 ? "text-luxury-gold" : "text-gray-900 dark:text-white"}`}
            >
              {shippingFee === 0
                ? lang === "vi"
                  ? "Miễn phí"
                  : "Free"
                : formatCurrency(shippingFee, currency, lang)}
            </dd>
          </dl>
          ...
          <dl className="flex items-center justify-between gap-4 border-t border-gray-200 dark:border-gray-600 pt-2">
            ...
            <dd className="text-base font-bold text-luxury-gold">
