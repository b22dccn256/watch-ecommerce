...
const CartItem = ({ item }) => {
  ...
  const [draftColor, setDraftColor] = useState(
    item.selectedColor || item.colors?.[0] || null,
  );
  const [draftSize, setDraftSize] = useState(
    item.selectedSize || item.sizes?.[0] || null,
  );
  ...
  const availableWristOptions = useMemo(
    () => (Array.isArray(item.wristSizeOptions) ? item.wristSizeOptions : []),
    [item.wristSizeOptions],
  );
  const hasWristOptions = availableWristOptions.length > 0;
  const isFreeWristSizing =
    !hasWristOptions &&
    !!item.specs?.strap?.material
      ?.toLowerCase()
      .match(/steel|metal|titanium|thép|kim loại/);
            ...
            >
              <div className="p-5 md:p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => setShowAttributeModal(false)}
                  className="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-500 hover:text-black dark:hover:text-white"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
                      ...
                      {availableWristOptions.map((option) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              !isOut && setDraftWristSize(option.size)
                            }
                            disabled={isOut}
