...
const EditProductForm = ({ product, onSuccess, onClose }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    brand:
      typeof product?.brand === "object"
        ...
        : product?.brand || "",
    type: product?.type || "",
    wristSizeOptions: Array.isArray(product?.wristSizeOptions)
      ? product.wristSizeOptions
      : [],
  ...
  const handleSubmit = async (e) => {
    ...
    const finalStock =
      formData.wristSizeOptions.length > 0
        ? formData.wristSizeOptions.reduce(
    ...
    await updateProduct(product._id, {
      ...
      originalPrice: formData.originalPrice
        ...
        : undefined,
      wristSizeOptions: formData.wristSizeOptions.filter(
        (o) => o.size?.trim() !== "",
      ),
      colors:
  ...
  const discount =
    ...
    Number(formData.originalPrice) > 0
      ? Math.round(
          (1 - Number(formData.price) / Number(formData.originalPrice)) * 100,
        )
      : null;
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          {/* Thương hiệu */}
          <div>
            <label htmlFor="edit-brand" className={labelCls}>
              Thương hiệu *
            </label>
            <select
              id="edit-brand"
          ...
          {/* Tồn kho */}
          <div>
            <label htmlFor="edit-stock" className={labelCls}>
              Tồn kho (Tổng) *
            </label>
            <input
              type="number"
              ...
              {formData.images && formData.images.length > 0 ? (
                <div className="w-full">
                    ...
                    {formData.images.map((src, idx) => (
                      <div key={idx} className="relative">
                        <img
                          src={src}
                          alt={`img-${idx}`}
                          className="h-20 w-full object-cover rounded-lg border border-luxury-gold/30"
                        />
                        <button
                          type="button"
                  ...
                  <p className="text-xs text-gray-400 mt-2">
                    Kéo để thay đổi hoặc chọn thêm file (nhiều ảnh được hỗ trợ)
                  </p>
                </div>
              ) : (
                <>
      ...
      <div className="pt-4 border-t border-gray-100 dark:border-luxury-border">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <select
              value={formData.specsStrapMaterial}
              onChange={(e) =>
                setFormData({ ...formData, specsStrapMaterial: e.target.value })
              }
              className={inputCls}
            >
          ...
          <div>
            ...
            <select
              value={formData.specsCaseMaterial}
              onChange={(e) =>
                setFormData({ ...formData, specsCaseMaterial: e.target.value })
              }
              className={inputCls}
            >
              <option value="">Chọn</option>
          ...
          <div>
            ...
            <input
              type="text"
              value={formData.specsCaseDiameter}
              onChange={(e) =>
                setFormData({ ...formData, specsCaseDiameter: e.target.value })
              }
              className={inputCls}
              placeholder="40 mm"
            />
          ...
          <div>
            ...
            <input
              type="text"
              value={formData.specsWaterResistance}
              onChange={(e) =>
                setFormData({
                  ...formData,
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <label htmlFor="edit-description" className={labelCls + " mb-0"}>
            Mô tả sản phẩm *
          </label>
          <span
