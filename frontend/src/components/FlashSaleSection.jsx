...
const FlashSaleSection = ({ products, timeLeft, addToCart }) => {
    return (
        <div
            ...
            <div className="max-w-screen-2xl mx-auto px-6 relative z-10">
                <div className="flex items-center justify-between mb-10">
                    <div>
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-5xl font-bold">FLASH SALE</h2>
                        </div>
                        ...
                        {Object.entries(timeLeft).map(([label, value]) => (
                            <div key={label} className="bg-zinc-900 px-6 py-3 rounded-xl">
                                ...
                                <div className="text-xs uppercase tracking-widest text-gray-500">
                                    {label === "hours" ? "GIỜ" : label === "minutes" ? "PHÚT" : "GIÂY"}
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {products.map((product) => (
                        <ProductCard
                            key={product._id}
                            product={{ ...product, discount: Math.floor(Math.random() * 30) + 10 }} // demo %
                            addToCart={addToCart}
                        />
                    ))}
                </div>
