...
const resolveChatIntent = (text) => {
  ...
  if (includesAny(normalized, ["bao hanh", "warranty"]))
    return { type: "warranty" };
  if (includesAny(normalized, ["giao hang", "van chuyen", "ship", "delivery"]))
    return { type: "shipping" };
  if (
    includesAny(normalized, ["thanh toan", "payment", "cod", "vnpay", "stripe"])
  )
    return { type: "payment" };
  if (includesAny(normalized, ["doi tra", "tra hang", "return", "hoan tien"]))
    return { type: "returns" };
  if (
    includesAny(normalized, [
      "lien he",
  ...
  if (
    includesAny(normalized, BRAND_KEYWORDS) ||
    includesAny(normalized, PRODUCT_TYPE_KEYWORDS) ||
    includesAny(normalized, FEATURE_KEYWORDS) ||
    includesAny(normalized, SEARCH_KEYWORDS)
  ) {
    return { type: "search" };
  }
...
const parseBudgetQuery = (text) => {
  ...
  const rangeMatch = normalized.match(
    /(?:tu|from)\s*(\d+(?:\.\d+)?)\s*(trieu|tr|trieu|m)\s*(?:den|toi|to|-)\s*(\d+(?:\.\d+)?)\s*(trieu|tr|trieu|m)/,
  );
  if (rangeMatch) {
    return {
...
const buildAction = (label, to, description = "") => ({
  label,
  to,
  description,
});
...
const INTENT_RESPONSE = {
  wrist: {
    content:
      "Với cổ tay nhỏ, mình ưu tiên mặt 28-38mm, case mỏng, dây thanh và lug-to-lug ngắn để đeo cân đối. Mình sẽ lọc vài mẫu hợp dáng tay ngay đây:",
    action: buildAction(
      "Mở Size Guide",
      "/size-guide",
      "Đo cổ tay chính xác hơn",
    ),
  },
  compare: {
    content:
      "Nếu bạn muốn so sánh mẫu, mình có thể gợi ý theo 3 hướng: độ bền, giá trị giữ giá và độ hợp dáng tay. Bạn cũng có thể mở catalog để đối chiếu trực tiếp.",
    action: buildAction("Mở Catalog", "/catalog", "So sánh sản phẩm trực tiếp"),
  },
  sizeGuide: {
    content:
      "Mở size guide sẽ giúp bạn đo cổ tay và chọn đường kính mặt đồng hồ phù hợp nhất.",
    action: buildAction("Mở Size Guide", "/size-guide", "Công cụ đo size"),
  },
  tracking: {
    content:
      "Bạn có thể dán mã theo dõi để xem trạng thái đơn hàng theo thời gian thực.",
    action: buildAction(
      "Theo dõi đơn",
  ...
  warranty: {
    content:
      "Tất cả sản phẩm được bảo hành chính hãng 5 năm. Bảo hành gồm sửa chữa miễn phí, thay thế linh kiện chính hãng và vệ sinh định kỳ.",
    action: buildAction(
  ...
  shipping: {
    content:
      "Giao hàng hỏa tốc 2-4 giờ nội thành, toàn quốc 1-2 ngày làm việc, miễn phí với đa số đơn hàng.",
    action: buildAction(
      "Xem chính sách giao hàng",
      "/delivery-policy",
      "Thời gian và phí ship",
    ),
  },
  payment: {
  ...
  returns: {
    content:
      "Đổi trả trong vòng 30 ngày nếu sản phẩm còn nguyên tem, hộp và chưa qua sử dụng.",
  ...
  account: {
    content:
      "Nếu bạn cần đăng nhập, đăng ký hay xác thực email, mình có thể dẫn bạn sang đúng màn hình ngay.",
    action: buildAction("Mở hồ sơ", "/profile", "Trang tài khoản"),
  },
  fallback: {
    content:
      "Mình có thể tư vấn theo ngân sách, size cổ tay, top sản phẩm bán chạy hoặc đánh giá cao. Nếu bạn muốn, hãy nói ngắn hơn một chút để mình bắt đúng nhu cầu.",
...
const parseDiameterMm = (product) => {
  const diameter =
    product?.specs?.case?.diameter || product?.specs?.diameter || "";
...
const formatWristProducts = (products) => {
  if (!products.length) {
    return {
      content:
        "Hiện tại mình chưa tìm thấy mẫu phù hợp cho cổ tay nhỏ. Bạn có thể mở Size Guide để đo cổ tay chính xác hơn.",
      products: [],
    };
  }
  ...
  return {
    content:
      "Với cổ tay nhỏ, mình ưu tiên mặt 28-38mm, dây mảnh và case mỏng để đeo cân đối. Đây là vài mẫu phù hợp từ catalog:",
    products: products.slice(0, 5).map((product) => ({
      id: product._id,
      slug: product.slug,
      slugToken: product.slugToken,
  ...
  const processAIResponse = async (msgText) => {
    ...
    try {
      ...
      if (
        ...
        intent.type === "shipping" ||
        intent.type === "payment" ||
        intent.type === "returns" ||
        intent.type === "contact" ||
        intent.type === "account" ||
        intent.type === "sizeGuide" ||
        intent.type === "compare"
      ) {
        const response =
          INTENT_RESPONSE[intent.type] || INTENT_RESPONSE.fallback;
        const botMsg = {
          id: Date.now() + 1,
          role: "bot",
      ...
      if (intent.type === "wrist") {
        ...
        const wristProducts = allProducts
          .map((product) => ({
            ...product,
            _diameterMm: parseDiameterMm(product),
          }))
          .filter(
            (product) =>
              product._diameterMm == null || product._diameterMm <= 38,
          )
          .sort((left, right) => {
      ...
      if (intent.type === "topSelling" || intent.type === "topRated") {
        ...
        const botMsg = {
          ...
          actions: [
            buildAction("Mở Catalog", "/catalog", "Xem toàn bộ catalog"),
          ],
        };
        setMessages((prev) => [...prev, botMsg]);
      ...
      if (budget) {
        const res = await axios.get("/products", {
          params: {
            ...
            ...(budget.minPrice != null
              ? { minPrice: Math.floor(budget.minPrice) }
              : {}),
            ...(budget.maxPrice != null
              ? { maxPrice: Math.floor(budget.maxPrice) }
              : {}),
          },
        });
        ...
        const products = Array.isArray(res.data?.products)
          ...
          : [];
        const budgetText =
          budget.minPrice != null && budget.maxPrice != null
      ...
      try {
        ...
        const botMsg = {
          ...
          products: res.data.products || [],
          actions: [
            buildAction("Mở Catalog", "/catalog", "Xem toàn bộ sản phẩm"),
          ],
        };
        setMessages((prev) => [...prev, botMsg]);
        if (socketRef.current) socketRef.current.emit("send_message", { sessionToken, sender: "bot", content: botMsg.content, products: botMsg.products, actions: botMsg.actions });
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 mt-0.5">
                    ...
                    <span className="text-black/70 text-[10px] font-medium">
                            ...
                            <div className="flex gap-3">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="text-[10px] text-gray-500 mt-1">
                                  ⭐{" "}
                                  {Number(product.averageRating || 0).toFixed(
                                    1,
                                  )}{" "}
                                  · Mua{" "}
                                  {Number(
