        ...
        {[
          {
            ...
            dot: reviewStats.pending > 0,
            dotColor: "bg-red-500",
          },
          {
            key: "qa",
            label: `Hỏi Đáp (${questions.length})`,
            dot: questions.filter((q) => !q.isAnswered).length > 0,
                  ...
                  {filteredReviews.length === 0 ? (
                    ...
                    filteredReviews.map((r) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-4 text-sm text-secondary max-w-xs truncate">
                          {r.comment}
                        </td>
                        ...
                        <td className="px-5 py-4 whitespace-nowrap text-center">
                          {r.status === "pending" && (
                            <span className="px-2.5 py-1 bg-luxury-gold/10 text-luxury-gold rounded-full text-[10px] font-semibold uppercase">
                              Chờ duyệt
                            </span>
                          )}
                          {r.status === "approved" && (
                          ...
                          {r.status === "hidden" && (
                            <span className="px-2.5 py-1 bg-black/8 text-muted dark:bg-white/8 rounded-full text-[10px] font-semibold uppercase">
                              Đã ẩn
                            </span>
                          )}
                        </td>
                        <td className="px-5 py-4 whitespace-nowrap text-right">
                          <div className="flex justify-end gap-1">
                            <button
                              ...
                              className="p-1.5 rounded-lg text-luxury-gold hover:bg-luxury-gold/10 transition"
                              title="Xem chi tiết"
                            >
        ...
        >
          ...
          <div className="grid gap-4">
            {filteredQuestions.length === 0 ? (
              <div className="text-center py-12 text-muted rounded-2xl border border-black/8 dark:border-white/8">
              ...
              filteredQuestions.map((q) => (
                <div
                  key={q._id}
                  className="rounded-2xl border border-black/8 dark:border-white/8 bg-surface p-5 space-y-4"
                >
                  <div className="flex justify-between items-start gap-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {!q.isAnswered && (
                          <span className="px-2 py-0.5 bg-luxury-gold/10 text-luxury-gold text-[10px] font-semibold uppercase rounded-full">
                            Cần trả lời
                          </span>
                        )}
                      </div>
                  ...
                  {q.isAnswered && (
                    <div className="bg-[color:var(--color-surface-2)] p-3 rounded-xl border-l-2 border-[color:var(--color-gold)] flex gap-3">
                      ...
                      <div>
                        <p className="text-xs font-bold text-[color:var(--color-gold)] mb-1">
                          Cửa hàng trả lời:
                        </p>
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <p className="text-sm text-secondary bg-[color:var(--color-surface-2)] p-3 rounded-xl">
                  ...
                  <div className="flex gap-2">
                    {selectedReview.images.map((img, i) => (
                      <img
                        ...
                        src={img}
                        alt="Review"
                        className="w-20 h-20 object-cover rounded-xl border border-black/8 dark:border-white/8"
                      />
                    ))}
                  </div>
                )}
                <div className="pt-4 flex gap-3 border-t border-black/8 dark:border-white/8">
                  {selectedReview.status !== "approved" && (
                    <button
                      ...
                      className="flex-1 py-2 bg-green-500 hover:bg-green-600 text-white rounded-full font-semibold text-sm flex items-center justify-center gap-2"
                    >
                  ...
                  {selectedReview.status !== "hidden" && (
                    <button
                      onClick={() =>
                        updateReviewStatus(selectedReview._id, "hidden")
                      }
                      className="flex-1 py-2 rounded-full border border-black/10 dark:border-white/10 text-secondary font-semibold text-sm flex items-center justify-center gap-2 hover:text-primary"
                    >
            ...
            >
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-base font-semibold text-primary flex items-center gap-2">
                  <MessageCircle className="w-5 h-5 text-luxury-gold" /> Phản
                  hồi câu hỏi
                </h3>
                <button
                  onClick={() => setSelectedQuestion(null)}
              ...
              <div className="bg-[color:var(--color-surface-2)] p-3 rounded-xl mb-4">
                <p className="text-xs text-muted mb-1 font-semibold">
                  {selectedQuestion.user.name} hỏi:
                </p>
              ...
              <form onSubmit={submitReply}>
                ...
                <div className="flex justify-end gap-3 mt-4">
                  <button
                    type="button"
                    onClick={() => setSelectedQuestion(null)}
                    className="px-4 py-2 rounded-full border border-black/10 dark:border-white/10 text-sm font-semibold text-secondary hover:text-primary transition"
                  >
                    Hủy
                  </button>
                  <button
                    type="submit"
                    className="px-6 py-2 rounded-full border border-[color:var(--color-gold)] text-[color:var(--color-gold)] text-sm font-semibold transition hover:bg-[color:var(--color-gold)] hover:text-white"
                  >
                    Gửi phản hồi
                  </button>
                </div>
