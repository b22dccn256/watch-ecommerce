...
const EmailTemplatesView = ({
  ...
  const handleSave = async () => {
    ...
    const success = await onUpdateTemplate(selectedTemplate._id, {
      htmlContent: newHtml,
    });
    setIsSaving(false);
    if (success) {
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          ...
          <h5 className="font-bold text-center text-gray-700 dark:text-gray-300">
            Tạo mẫu mới
          </h5>
          <p className="text-xs text-gray-500 text-center mt-2">
            Thêm mẫu email tùy chỉnh
          </p>
        </div>
        {templates.map((t) => (
          <div
          ...
          >
            <div className="aspect-[3/4] bg-white border border-luxury-border rounded-3xl mb-3 overflow-hidden group-hover:shadow-2xl transition-all relative">
              <div className="absolute inset-0 bg-transparent group-hover:bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all z-10">
                <button className="bg-luxury-gold text-luxury-dark px-4 py-2 rounded-lg font-bold text-xs">
                  Xem chi tiết
                </button>
              </div>
        ...
        {selectedTemplate && (
          ...
          >
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  {isEditing ? (
                    <>
                      ...
                      >
                        {isSaving ? (
                          <span className="animate-spin w-4 h-4 border-2 border-lux-dark border-t-transparent rounded-full"></span>
                        ) : (
                    ...
                    <>
                      ...
                      >
                        <Edit2 className="w-4 h-4" /> Chỉnh sửa (Kéo thả, bôi
                        đen, xóa chữ)
                      </button>
                      <button
              ...
              <div
                className={`p-4 ${isEditing ? "bg-gray-200" : "bg-gray-50 dark:bg-black"} overflow-y-auto flex-1 flex flex-col items-center`}
              >
                {isEditing && (
                  <p className="text-xs text-red-500 font-medium mb-3 bg-red-50 px-3 py-2 rounded-lg border border-red-100">
                    Chú ý: Trình chỉnh sửa trực quan. Bạn có thể bôi đen chữ để
                    sửa hoặc xóa. Tuyệt đối không xóa các biến như{" "}
                    {"{{order.orderCode}}"} hoặc {"{{fullName}}"} vì hệ thống
                    cần dùng để tự động điền thông tin.
                  </p>
                )}
                <div
                  className={`w-full max-w-[600px] h-full min-h-[600px] bg-white ${isEditing ? "shadow-[0_0_0_4px_#3b82f6]" : "shadow-md"} transition-all overflow-hidden rounded`}
                >
                  <iframe
                    key={resetKey}
                    ref={iframeRef}
            ...
            >
              ...
              <div className="p-4 border-t border-gray-200 dark:border-gray-800 flex justify-end gap-3 bg-gray-50 dark:bg-gray-900/50">
                ...
                <button
                  ...
                  disabled={
                    isSaving || !newTemplate.name || !newTemplate.subject
                  }
