...
const StatCard = ({ label, value, changeText, isPositive, colorTheme }) => {
  return (
    <div
      className={`bg-white dark:bg-[#1a1a1a] border-2 ${colorTheme.border} p-6 rounded-2xl shadow-sm relative flex flex-col justify-between min-h-[160px] transition-transform hover:scale-[1.02]`}
    >
      <div>
        <h3 className="text-[16px] font-semibold text-gray-800 dark:text-gray-200">
          {label}
        </h3>
        <div className="text-[40px] font-bold text-gray-900 dark:text-white mt-1">
          {value}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mt-4">
        <div
          className={`flex items-center justify-center w-6 h-6 rounded-md ${colorTheme.iconBg} ${colorTheme.iconColor}`}
        >
          {isPositive ? (
      ...
      {/* Vertical Bar */}
      <div
        className={`absolute right-6 top-1/2 -translate-y-1/2 w-2.5 h-[70px] rounded-full ${colorTheme.bar}`}
      ></div>
    </div>
  );
};
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white dark:bg-luxury-dark border border-gray-100 dark:border-luxury-border p-8 rounded-3xl shadow-sm">
        <h3 className="text-xl font-bold mb-8 text-gray-900 dark:text-white">
          Hiệu quả chiến dịch (7 ngày qua)
        </h3>
