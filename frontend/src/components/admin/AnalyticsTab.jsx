...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="rounded-xl border border-black/10 bg-surface p-4 shadow-xl text-sm dark:border-white/10">
      <p className="font-bold text-primary mb-2">{label}</p>
      {payload.map(e => (
        <p key={e.name} className="mt-1" style={{ color: e.color }}>
...
const DeltaBadge = ({ delta }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-lg ${
      ...
      {delta.positive ? <ArrowUpRight className="w-4 h-4" /> : <ArrowDownRight className="w-4 h-4" />}
  ...
  return (
    <div className="card-admin transition hover:border-[color:var(--color-gold)]/25 p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <p className="text-sm text-secondary font-medium leading-tight">{title}</p>
...
const AnalyticsTab = () => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-2xl font-semibold text-primary">Tổng quan hiệu suất</h2>
          <p className="text-sm text-secondary mt-1">{days} ngày gần nhất</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={exportCsv}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-md border border-green-500/15 bg-green-500/6 text-green-600 transition hover:bg-green-500/12 dark:text-green-400"
          >
            <Download className="w-4 h-4" /> CSV
          </button>
          <button
            onClick={() => window.open("/api/products/export", "_blank")}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-md border border-blue-500/15 bg-blue-500/6 text-blue-600 transition hover:bg-blue-500/12 dark:text-blue-400"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="space-y-4">
          <p className="text-sm font-semibold text-secondary">Khối Bán hàng</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            <KpiCard
              ...
              delta={saleDelta}
              badge={data.cancellationRate > 0 ? <span className="text-xs font-bold px-2 py-0.5 rounded-full border border-red-500/20 bg-red-500/10 text-red-500">{data.cancellationRate.toFixed(1)}% hủy/hoàn</span> : null}
            />
        ...
        <div className="space-y-4">
          <p className="text-sm font-semibold text-secondary">Khối Vận hành</p>
          ...
          <div className="flex items-center gap-3">
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-semibold transition ${rangeType === 'custom' ? "bg-[color:var(--color-gold)] text-white" : "bg-[color:var(--color-surface-2)] text-secondary"}`}
              >
                Tùy chỉnh
              </button>
            </div>
            {rangeType === 'custom' && (
              <div className="flex items-center gap-2">
                <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)} className="text-sm px-3 py-1.5 border rounded-md" />
                <span className="text-sm text-secondary">→</span>
                <input type="date" value={customEnd} onChange={e => setCustomEnd(e.target.value)} className="text-sm px-3 py-1.5 border rounded-md" />
                <button onClick={() => applyRange('custom')} className="px-4 py-2 rounded-lg text-sm font-semibold bg-[color:var(--color-gold)] text-white">Áp dụng</button>
              </div>
            )}
          </div>
        }
            ...
            {chartReady && (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={dailySalesData}>
                  ...
                  <YAxis yAxisId="left" tick={{ fontSize: 13 }} tickLine={false} axisLine={false} />
                  <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 13 }} tickLine={false} axisLine={false} tickFormatter={v => formatVND(v)} />
        ...
        {isLoading ? <ChartSkeleton /> : !data.wristSizeStats?.length ? (
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={data.wristSizeStats} layout="vertical" margin={{ left: 8 }}>
              ...
              <XAxis type="number" tick={{ fontSize: 13 }} tickLine={false} axisLine={false} />
              <YAxis type="category" dataKey="size" tick={{ fontSize: 13 }} tickLine={false} axisLine={false} />
              <Tooltip cursor={{ fill: "transparent" }} />
              <Bar dataKey="count" fill="#c9a96e" radius={[0, 4, 4, 0]} maxBarSize={30} name="Số lượng" />
            </BarChart>
          </ResponsiveContainer>
        )}
      </CardShell>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {noTransactions ? (
          <CardShell title="Dữ liệu giao dịch" icon={Trophy}>
            <div className="py-10 text-center">
              <p className="text-base text-muted">Chưa có dữ liệu giao dịch trong khoảng thời gian đã chọn.</p>
              <p className="text-sm text-muted mt-2">Hãy chờ có đơn hàng hoặc thay đổi bộ lọc thời gian.</p>
            </div>
          </CardShell>
        ) : (
          <CardShell title="Top 8 Bán chạy" icon={Trophy}>
            {isLoading ? (
              ...
              <p className="text-sm text-muted py-6 text-center">Chưa có dữ liệu</p>
            ) : topProducts.map((p, i) => (
              <div key={p._id} className="flex items-center gap-4 py-3 border-b border-black/5 dark:border-white/5 last:border-0">
                <span className="text-sm font-bold text-muted w-6 flex-shrink-0">#{i + 1}</span>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted mt-0.5">{p.brand?.name}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-base font-bold text-[color:var(--color-gold)]">{(p.salesCount || 0).toLocaleString()} bán</p>
                  <p className="text-xs text-muted mt-0.5">Tồn: {p.stock}</p>
                </div>
              </div>
            ))}
          </CardShell>
        ...
        <CardShell title="Tồn Kho Thấp" icon={AlertTriangle}>
          {isLoading ? (
            ...
            <div key={p._id} className="flex items-center gap-4 py-3 border-b border-black/5 dark:border-white/5 last:border-0">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-base font-medium text-primary truncate">{p.name}</p>
                <p className="text-xs text-muted mt-0.5">Ngưỡng: {p.lowStockThreshold}</p>
              </div>
              <p className="text-base font-semibold text-secondary flex-shrink-0">{p.price?.toLocaleString("vi-VN")} ₫</p>
            </div>
          ))}
        </CardShell>
        ...
        <CardShell
          ...
          icon={Clock}
          action={
            <Link to="/admin?tab=orders&status=pending" className="text-sm font-semibold text-blue-500 hover:text-blue-600 flex items-center gap-1 transition">
        ...
        >
          {isLoading ? (
            ...
            <div className="py-10 text-center flex flex-col items-center">
              ...
              <p className="text-base font-medium text-primary">Tuyệt vời!</p>
              <p className="text-sm text-muted mt-1">Không còn đơn hàng nào đang chờ duyệt.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {recentPendingOrders.map(order => (
                <div key={order._id} className="flex items-center justify-between p-3 rounded-xl border border-transparent hover:border-black/5 dark:hover:border-white/5 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition">
                  <div>
                    <Link to={`/admin?tab=orders&search=${order.orderCode}`} className="text-base font-bold text-primary hover:text-blue-500 transition">
                      #{order.orderCode}
                    </Link>
                    <p className="text-sm text-muted mt-1 truncate max-w-[120px] sm:max-w-[160px]">{order.shippingDetails?.fullName}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-base font-bold text-[color:var(--color-gold)]">{formatVND(order.totalAmount)} ₫</p>
      ...
      {plData && (
        <CardShell title="Báo Cáo P&L" icon={TrendingUp}>
          <span className="text-sm text-muted">{plData.summary?.days} ngày gần nhất</span>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-4 mb-6">
            {[
              ...
              <div key={item.label} className="rounded-xl bg-[color:var(--color-surface-2)] p-4">
                <p className="text-sm text-muted font-medium uppercase tracking-wide">{item.label}</p>
                <p className={`text-xl font-bold mt-2 ${item.color}`}>{item.value}</p>
              </div>
            ))}
          </div>
          {plData.daily?.length > 0 && (
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={plData.daily} margin={{ top: 4, right: 8, left: 8, bottom: 0 }}>
                ...
                <YAxis tickFormatter={v => formatVND(v)} tick={{ fontSize: 12 }} axisLine={false} tickLine={false} width={60} />
                ...
                <Bar dataKey="cogs" fill="#f97316" radius={[4, 4, 0, 0]} maxBarSize={30} />
                <Bar dataKey="grossProfit" fill="#10b981" radius={[4, 4, 0, 0]} maxBarSize={30} />
              </BarChart>
            </ResponsiveContainer>
          )}
          <p className="text-sm text-muted mt-3">* Giá vốn dùng costPrice từ sản phẩm. Nếu không có, ước tính 60% giá bán.</p>
        </CardShell>
      )}
    </div>
  );
