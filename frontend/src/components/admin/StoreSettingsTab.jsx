...
import {
	...
	Image as ImageIcon, Sparkles, Clock, Megaphone, Phone, Mail,
	MapPin, Share2, Plus, Trash2, Globe, Paintbrush, Code2, BarChart3,
...
const THEME_PRESETS = [
	{ key: "midnight", label: "Midnight Gold (Mặc định)", desc: "Phông nền tối sâu thẳm, điểm nhấn sắc vàng hoàng gia xa hoa", accent: "bg-black text-[color:var(--color-gold)] border-[color:var(--color-gold)]/30" },
	{ key: "platinum", label: "Platinum Pearl (Sáng/Lịch lãm)", desc: "Phông nền kem ngọc trai ngà, chữ than và viền kim bạc thanh khiết", accent: "bg-stone-100 text-stone-900 border-stone-300" },
...
const StoreSettingsTab = () => {
	...
	useEffect(() => {
		if (config) {
			setFormData({
				...
				cookieConsentTitle: config.cookieConsentTitle || "Trang web này sử dụng Cookie",
				cookieConsentText: config.cookieConsentText || "Chúng tôi dùng cookie để cá nhân hóa trải nghiệm mua sắm của bạn.",
				// Custom CSS
				...
				promoPopupText: config.promoPopupText || "Đăng ký nhận tin để nhận ngay đặc quyền ưu đãi 5% cho đơn hàng đầu tiên.",
				promoPopupImage: config.promoPopupImage || "https://images.unsplash.com/photo-1523275335684-37898b6baf30?q=80&w=600",
				...
				footerAddress: config.footerAddress || "Số 1 Đại Cồ Việt, Hai Bà Trưng, Hà Nội",
				footerAboutText: config.footerAboutText || "Luxury Watch Gallery tự hào là hệ thống phân phối đồng hồ cao cấp chính hãng hàng đầu Việt Nam, với hơn 20 năm kinh nghiệm.",
				footerCopyright: config.footerCopyright || "© {year} Luxury Watch Gallery. Tất cả quyền được bảo lưu.",
				footerColumns: config.footerColumns || [
					{ title: "Dịch vụ Khách hàng", links: [{ label: "Trung tâm trợ giúp", link: "/support" }, { label: "Chính sách đổi trả", link: "/returns" }, { label: "Bảo hành & Sửa chữa", link: "/warranty" }, { label: "Hướng dẫn mua hàng", link: "/guide" }] },
					{ title: "Về Chúng tôi", links: [{ label: "Câu chuyện thương hiệu", link: "/about" }, { label: "Showroom", link: "/stores" }, { label: "Tuyển dụng", link: "/careers" }, { label: "Liên hệ", link: "/contact" }] },
				],
				...
				announcementText: config.announcementText || "Miễn phí vận chuyển toàn quốc cho tất cả đơn hàng đồng hồ cao cấp từ 10 triệu đồng!",
				announcementBg: config.announcementBg || "gold",
				announcementLink: config.announcementLink || "/catalog?reset=true",
				seoTitle: config.seoTitle || "Luxury Watch Gallery | Đồng Hồ Cao Cấp Chính Hãng",
				seoMetaDesc: config.seoMetaDesc || "Hệ thống phân phối đồng hồ cao cấp, chính hãng từ các thương hiệu hàng đầu thế giới như Rolex, Omega, Hublot. Bảo hành trọn đời.",
				...
				storeWorkingHours: config.storeWorkingHours || "Mở cửa hàng ngày: 09:00 - 21:00 (Cả Thứ Bảy & Chủ Nhật)",
			});
	...
	const handleChange = (e) => {
		...
		const numberFields = ["gridColumns", "featuredCount", "promoPopupDelay", "headingScale", "bodyScale", "productsPerPage"];
		setFormData((prev) => ({
			...prev,
	...
	return (
		<div className="w-full p-4">
			{/* Header */}
			<div className="mb-6 flex flex-col md:flex-row md:items-center justify-end gap-4 border-b border-gray-100 dark:border-white/5 pb-5">
				<button type="button" onClick={handleSubmit} disabled={loading}
					...
					{loading ? <span className="animate-spin rounded-full w-3.5 h-3.5 border-b-2 border-lux-dark"></span> : <Save className="w-4 h-4" />}
					LƯU & XUẤT BẢN NGAY
				</button>
			</div>
				...
				<main className="bg-white/60 dark:bg-luxury-darker/30 backdrop-blur-md rounded-2xl border border-gray-100 dark:border-white/5 p-5 md:p-6 min-h-[480px]">
						...
						<div className="space-y-5 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<Layers className="w-4 h-4 text-luxury-gold" /> Bật/Tắt & Sắp xếp Sections Trang Chủ
								</h3>
							...
							<div className="space-y-2">
								{sectionLayout.map((item, idx) => {
									...
									return (
										<div key={item.key} className={`flex items-center gap-3 p-3.5 rounded-xl border transition-all ${item.enabled ? "border-luxury-gold/30 bg-luxury-gold/5" : "border-gray-200 dark:border-gray-800 opacity-40 bg-transparent"}`}>
											<GripVertical className="w-4 h-4 text-gray-400 flex-shrink-0" />
											<div className="flex-1 min-w-0">
												<div className="flex items-center gap-2">
													<p className="font-bold text-sm text-gray-900 dark:text-white">{meta?.label}</p>
													{item.enabled ? (
														<span className="text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-1.5 py-0.5 rounded font-semibold">Hiển thị</span>
													) : (
														<span className="text-xs bg-gray-500/10 text-gray-400 border border-gray-500/20 px-1.5 py-0.5 rounded font-semibold">Đã ẩn</span>
													)}
												</div>
											...
											<div className="flex items-center gap-1 shrink-0">
												<button type="button" onClick={() => moveSection(item.key, -1)} disabled={idx === 0}
													className="p-1.5 rounded-lg hover:bg-white/10 disabled:opacity-20 text-gray-400 hover:text-white transition" title="Lên trên">
													<ChevronUp className="w-4 h-4" />
												</button>
												<button type="button" onClick={() => moveSection(item.key, 1)} disabled={idx === sectionLayout.length - 1}
													className="p-1.5 rounded-lg hover:bg-white/10 disabled:opacity-20 text-gray-400 hover:text-white transition" title="Xuống dưới">
					...
					{activeSubTab === "branding" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<Type className="w-4 h-4 text-luxury-gold" /> Bản sắc Thương hiệu, Logo & Menu Điều hướng
								</h3>
								<p className="text-xs text-gray-400 mt-1">Cấu hình Tên cửa hàng, Logo nghệ thuật, Thanh thông báo sự kiện (Announcement Bar) và Menu điều hướng linh hoạt.</p>
							</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								...
								<div className="space-y-4 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
									<h4 className="text-xs font-bold  text-luxury-gold flex items-center gap-1.5 mb-1">
										<ImageIcon className="w-3.5 h-3.5" /> Logo & Tên Cửa Hàng
									</h4>
									<div className="space-y-3">
										<div>
											<label className="block text-sm font-bold text-gray-400 mb-1">TÊN THƯƠNG HIỆU CHÍNH (LOGO TEXT)</label>
											<input
												type="text"
												name="logoText"
										...
										<div>
											<label className="block text-sm font-bold text-gray-400 mb-1">CHỮ VIẾT TẮT (LOGO SUBTEXT / ICON)</label>
											<input
												type="text"
												name="logoSubtext"
										...
										<div>
											<label className="block text-sm font-bold text-gray-400 mb-1">HOẶC DÙNG ẢNH LOGO RIÊNG (DESKTOP LOGO IMAGE)</label>
											<div className="flex gap-2">
												...
												<label className="flex items-center justify-center px-3 py-2 rounded border border-luxury-gold/30 bg-luxury-gold/5 text-luxury-gold hover:bg-luxury-gold/10 text-xs font-semibold cursor-pointer transition select-none flex-shrink-0 min-w-[90px]">
													{uploadingIndex?.index === "branding" && uploadingIndex?.field === "logoImage" ? (
													...
													<input
														type="file"
														accept="image/*"
														onChange={(e) => handleImageUpload("branding", "logoImage", e.target.files[0])}
														className="hidden"
													/>
												</label>
											</div>
											<p className="text-xs text-gray-500 mt-1">Khuyên dùng logo định dạng SVG hoặc PNG nền trong suốt để có hiển thị hoàn hảo nhất.</p>
										</div>
									</div>
								</div>
								...
								<div className="space-y-4 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
									<div className="flex items-center justify-between">
										<h4 className="text-xs font-bold  text-luxury-gold flex items-center gap-1.5">
											<Megaphone className="w-3.5 h-3.5" /> Thanh Thông Báo Khuyến Mãi Đầu Trang
										</h4>
									...
									<div className="space-y-3">
										<div>
											<label className="block text-sm font-bold text-gray-400 mb-1">NỘI DUNG THÔNG BÁO</label>
											<textarea
										...
										<div>
											<label className="block text-sm font-bold text-gray-400 mb-1">HÒA SẮC NỀN THANH THÔNG BÁO</label>
											<div className="grid grid-cols-3 gap-2">
												{[
													...
													<button
														key={preset.key}
														type="button"
														onClick={() => setFormData(prev => ({ ...prev, announcementBg: preset.key }))}
														className={`p-2 rounded-lg text-xs font-bold text-center flex flex-col items-center justify-center gap-1 transition-all ${preset.class} ${formData.announcementBg === preset.key ? "ring-2 ring-blue-500 scale-[1.03]" : "opacity-75 hover:opacity-100"}`}
													>
										...
										<div>
											<label className="block text-sm font-bold text-gray-400 mb-1">ĐƯỜNG DẪN LIÊN KẾT (URL CHUYỂN TRANG)</label>
											<input
												type="text"
							...
							<div className="space-y-4 bg-gray-50 dark:bg-black/20 p-5 rounded-xl border border-gray-200 dark:border-gray-800">
								<div className="flex items-center justify-between border-b border-gray-100 dark:border-white/5 pb-3">
									<div>
										<h4 className="text-xs font-bold  text-luxury-gold flex items-center gap-1.5">
											<Layers className="w-3.5 h-3.5" /> Quản Lý Menu Điều Hướng (Navigation Links)
										</h4>
										<p className="text-xs text-gray-500 mt-1">Thay đổi nhãn, hoán đổi vị trí và thêm mới các tab liên kết hiển thị ở Navbar đầu trang.</p>
									</div>
									<button
										type="button"
										onClick={addNavItem}
										className="bg-luxury-gold/10 hover:bg-luxury-gold/20 text-luxury-gold border border-luxury-gold/30 font-bold text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition"
									>
								...
								<div className="space-y-2">
									{(formData.navigationItems || []).map((item, index) => (
										<div key={index} className="flex items-center gap-3 p-3 bg-white/5 dark:bg-black/30 rounded-lg border border-gray-200/50 dark:border-white/5">
											<span className="text-xs font-bold text-luxury-gold shrink-0 w-6">#{index + 1}</span>

											<div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
												<div className="flex items-center gap-2">
													<span className="text-xs font-medium text-gray-400 whitespace-nowrap">Tên hiển thị:</span>
													<input
														type="text"
														value={item.label || ""}
												...
												<div className="flex items-center gap-2">
													<span className="text-xs font-medium text-gray-400 whitespace-nowrap">Đường dẫn:</span>
													<input
														type="text"
														value={item.link || ""}
					...
					{activeSubTab === "colors" && (
						<div className="space-y-6 animate-fade-in">
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
								{[
									{ key: "primaryColor", label: "Màu Chủ đạo (Primary)", desc: "Nút bấm chính, link, accent chính", defaultHex: "#D4AF37" },
									{ key: "secondaryColor", label: "Màu Phụ (Secondary)", desc: "Nút phụ, badge, hover phụ", defaultHex: "#B49450" },
									{ key: "accentColor", label: "Màu Nhấn (Accent)", desc: "Highlight, flash sale, khuyến mãi", defaultHex: "#EF4444" },
									{ key: "bgColor", label: "Nền Chính (Background)", desc: "Màu nền toàn trang", defaultHex: "#0A0A0A" },
									{ key: "cardBgColor", label: "Nền Thẻ SP (Card BG)", desc: "Nền các card sản phẩm", defaultHex: "#1A1A1A" },
									{ key: "textPrimaryColor", label: "Chữ Chính (Text Primary)", desc: "Chữ tiêu đề, chữ chính", defaultHex: "#FFFFFF" },
									{ key: "textSecondaryColor", label: "Chữ Phụ (Text Secondary)", desc: "Chữ mô tả, subtitle", defaultHex: "#9CA3AF" },
									{ key: "borderColor", label: "Viền (Border)", desc: "Viền card, input, divider", defaultHex: "#2A2A2A" },
								].map((color) => (
									<div key={color.key} className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-black/20">
										...
										<div className="flex-1 min-w-0">
											<p className="text-xs font-bold text-gray-900 dark:text-white">{color.label}</p>
							...
							{/* Preview */}
							<div className="p-4 rounded-xl border border-gray-200 dark:border-white/5" style={{ background: formData.bgColor || "#0A0A0A" }}>
								<p className="text-xs font-bold  mb-3" style={{ color: formData.textSecondaryColor || "#9CA3AF" }}>XEM TRƯỚC GIAO DIỆN</p>
								<div className="flex gap-2 mb-3">
									<button className="px-3 py-1.5 rounded text-xs font-bold" style={{ background: formData.primaryColor || "#D4AF37", color: "#000" }}>Primary Button</button>
									<button className="px-3 py-1.5 rounded text-xs font-bold border" style={{ borderColor: formData.secondaryColor || "#B49450", color: formData.secondaryColor || "#B49450" }}>Secondary</button>
									<span className="px-2 py-1.5 rounded text-xs font-bold" style={{ background: formData.accentColor || "#EF4444", color: "#fff" }}>Accent</span>
								</div>
								<div className="p-3 rounded-lg border" style={{ background: formData.cardBgColor || "#1A1A1A", borderColor: formData.borderColor || "#2A2A2A" }}>
									<p className="text-xs font-bold" style={{ color: formData.textPrimaryColor || "#FFFFFF" }}>Rolex Submariner Date</p>
									<p className="text-xs" style={{ color: formData.textSecondaryColor || "#9CA3AF" }}>Đồng hồ lặn chuyên nghiệp, chống nước 300m</p>
								</div>
							</div>
						</div>
					)}
					...
					{activeSubTab === "typography" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<Type className="w-4 h-4 text-luxury-gold" /> Kiểu Chữ & Phông Chữ
								</h3>
								<p className="text-xs text-gray-400 mt-1">Tuỳ chỉnh font chữ và kích thước hiển thị trên toàn bộ website. Để trống để dùng font mặc định.</p>
							</div>

							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div className="space-y-4 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
									<h4 className="text-xs font-bold  text-luxury-gold">Font Chữ Tiêu đề (Heading Font)</h4>
									<select
									...
									>
										...
										<option value="Merriweather">Merriweather (Truyền thống)</option>
										<option value="DM Serif Display">DM Serif Display (Độc đáo)</option>
										<option value="Libre Baskerville">Libre Baskerville (Lịch lãm)</option>
									</select>

									<h4 className="text-xs font-bold  text-luxury-gold mt-4">Font Chữ Nội dung (Body Font)</h4>
									<select
									...
									>
										...
										<option value="Source Sans 3">Source Sans 3 (Chuyên nghiệp)</option>
										<option value="Noto Sans">Noto Sans (Hỗ trợ tiếng Việt tốt)</option>
									</select>
								</div>

								<div className="space-y-4 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
									<h4 className="text-xs font-bold  text-luxury-gold">Tỉ lệ Kích thước</h4>
					...
					{activeSubTab === "footerDetail" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<Star className="w-4 h-4 text-luxury-gold" /> Footer Nâng Cao – Giới thiệu, Cột Link, Mạng Xã Hội
								</h3>
								<p className="text-xs text-gray-400 mt-1">Quản lý văn bản giới thiệu, bản quyền, các cột liên kết động và mạng xã hội mở rộng.</p>
							</div>

							{/* About Text & Copyright */}
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div className="space-y-2">
									<label className="block text-xs font-semibold text-luxury-gold">Văn bản Giới thiệu Footer</label>
									<textarea
										value={formData.footerAboutText || ""}
										onChange={(e) => setFormData(prev => ({ ...prev, footerAboutText: e.target.value }))}
								...
								<div className="space-y-2">
									<label className="block text-xs font-semibold text-luxury-gold">Dòng Bản quyền (Copyright)</label>
									<input
										type="text"
									...
									/>
									<p className="text-xs text-gray-500">Dùng {'{year}'} để tự động điền năm hiện tại.</p>
								</div>
							</div>
							...
							<div className="border-t border-gray-100 dark:border-white/5 pt-5 space-y-4">
								<div className="flex items-center justify-between">
									<h4 className="text-xs font-bold  text-luxury-gold">Cột Liên kết Footer (Footer Columns)</h4>
									<button
										type="button"
										onClick={() => setFormData(prev => ({ ...prev, footerColumns: [...(prev.footerColumns || []), { title: "Cột mới", links: [{ label: "Link mới", link: "/" }] }] }))}
										className="bg-luxury-gold/10 hover:bg-luxury-gold/20 text-luxury-gold border border-luxury-gold/30 font-bold text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5"
									>
								...
								{(formData.footerColumns || []).map((col, colIdx) => (
									<div key={colIdx} className="p-4 rounded-xl border border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-black/20 space-y-3">
										<div className="flex items-center gap-3">
											<input
												...
												onChange={(e) => {
													...
													setFormData(prev => ({ ...prev, footerColumns: cols }));
												}}
											...
											<button
												type="button"
												onClick={() => setFormData(prev => ({ ...prev, footerColumns: (prev.footerColumns || []).filter((_, i) => i !== colIdx) }))}
										...
										<div className="space-y-1.5 pl-2">
												...
												<div key={linkIdx} className="flex items-center gap-2">
													...
													<input
														...
														onChange={(e) => {
															const cols = [...(formData.footerColumns || [])];
															const links = [...(cols[colIdx].links || [])];
															links[linkIdx] = { ...links[linkIdx], link: e.target.value };
															cols[colIdx] = { ...cols[colIdx], links };
															setFormData(prev => ({ ...prev, footerColumns: cols }));
														}}
											...
											<button
												...
												onClick={() => {
													const cols = [...(formData.footerColumns || [])];
													cols[colIdx] = { ...cols[colIdx], links: [...(cols[colIdx].links || []), { label: "Link mới", link: "/" }] };
													setFormData(prev => ({ ...prev, footerColumns: cols }));
												}}
												className="text-xs text-luxury-gold hover:underline flex items-center gap-1 mt-1"
											>
							...
							<div className="border-t border-gray-100 dark:border-white/5 pt-5 space-y-3">
								...
								<div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
									<div>
										<label className="block text-sm font-bold text-gray-400 mb-1">TikTok URL</label>
										<input type="text" value={formData.footerTiktok || ""} onChange={(e) => setFormData(prev => ({ ...prev, footerTiktok: e.target.value }))} className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-white/10 bg-transparent text-xs outline-none focus:border-luxury-gold" placeholder="https://tiktok.com/@..." />
									</div>
									<div>
										<label className="block text-sm font-bold text-gray-400 mb-1">YouTube URL</label>
										<input type="text" value={formData.footerYoutube || ""} onChange={(e) => setFormData(prev => ({ ...prev, footerYoutube: e.target.value }))} className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-white/10 bg-transparent text-xs outline-none focus:border-luxury-gold" placeholder="https://youtube.com/@..." />
									</div>
									<div>
										<label className="block text-sm font-bold text-gray-400 mb-1">Pinterest URL</label>
										<input type="text" value={formData.footerPinterest || ""} onChange={(e) => setFormData(prev => ({ ...prev, footerPinterest: e.target.value }))} className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-white/10 bg-transparent text-xs outline-none focus:border-luxury-gold" placeholder="https://pinterest.com/..." />
									</div>
								</div>
							</div>
						</div>
					...
					{activeSubTab === "integrations" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<BarChart3 className="w-4 h-4 text-luxury-gold" /> Tích hợp Theo dõi & Phân tích
								</h3>
								<p className="text-xs text-gray-400 mt-1">Kết nối Google Analytics, Facebook Pixel, TikTok Pixel để theo dõi khách hàng và tối ưu quảng cáo.</p>
							</div>

							<div className="space-y-4">
								{[
									{ key: "googleAnalyticsId", label: "Google Analytics ID (GA4)", icon: BarChart3, placeholder: "G-XXXXXXXXXX", desc: "Đo lường toàn diện hành vi người dùng trên website." },
									{ key: "facebookPixelId", label: "Facebook Pixel ID (Meta)", icon: Share2, placeholder: "1234567890123456", desc: "Theo dõi chuyển đổi và tạo đối tượng quảng cáo trên Facebook & Instagram." },
									{ key: "tiktokPixelId", label: "TikTok Pixel ID", icon: Zap, placeholder: "XXXXXXXXXXXXXXX", desc: "Đo lường hiệu quả quảng cáo TikTok Shop." },
								].map((item) => (
									<div key={item.key} className="p-4 rounded-xl border border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-black/20">
										<div className="flex items-center gap-2 mb-2">
											<item.icon className="w-4 h-4 text-luxury-gold" />
											<label className="text-xs font-bold text-gray-900 dark:text-white">{item.label}</label>
										</div>
										<input
											type="text"
							...
							<div className="border-t border-gray-100 dark:border-white/5 pt-5 space-y-4">
								<div className="flex items-center justify-between">
									...
									<label className="flex items-center gap-2 cursor-pointer">
										<input type="checkbox" checked={!!formData.cookieConsentEnabled} onChange={(e) => setFormData(prev => ({ ...prev, cookieConsentEnabled: e.target.checked }))} className="sr-only peer" />
										<div className="relative w-9 h-5 bg-gray-300 dark:bg-gray-700 rounded-full peer peer-focus:ring-1 peer-focus:ring-luxury-gold peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-luxury-gold"></div>
										<span className="text-xs font-bold text-gray-300">{formData.cookieConsentEnabled ? "Đang bật" : "Đang tắt"}</span>
									</label>
								</div>
								<div className={`space-y-3 transition-all ${formData.cookieConsentEnabled ? "opacity-100" : "opacity-40 pointer-events-none"}`}>
									<div>
										<label className="block text-sm font-bold text-gray-400 mb-1">Tiêu đề Cookie Banner</label>
										<input type="text" value={formData.cookieConsentTitle || ""} onChange={(e) => setFormData(prev => ({ ...prev, cookieConsentTitle: e.target.value }))} className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-white/10 bg-transparent text-xs outline-none focus:border-luxury-gold" />
									</div>
									<div>
										<label className="block text-sm font-bold text-gray-400 mb-1">Nội dung Cookie Banner</label>
										<textarea value={formData.cookieConsentText || ""} onChange={(e) => setFormData(prev => ({ ...prev, cookieConsentText: e.target.value }))} className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-white/10 bg-transparent text-xs outline-none focus:border-luxury-gold" rows={2} />
									</div>
								</div>
							</div>
						</div>
					...
					{activeSubTab === "catalog" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<ShoppingBag className="w-4 h-4 text-luxury-gold" /> Cấu Hình Trang Danh Mục Sản Phẩm
								</h3>
								<p className="text-xs text-gray-400 mt-1">Cài đặt cách hiển thị sản phẩm trên trang Catalog / Bộ sưu tập.</p>
							</div>

							<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
								<div className="space-y-2 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
									<label className="block text-xs font-bold text-luxury-gold">Số sản phẩm mỗi trang</label>
									<div className="flex gap-2">
										{[12, 24, 36, 48].map(num => (
											<button key={num} type="button" onClick={() => setFormData(prev => ({ ...prev, productsPerPage: num }))}
								...
								<div className="space-y-2 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
									<label className="block text-xs font-bold text-luxury-gold">Sắp xếp mặc định</label>
									<select
										value={formData.defaultSort || "newest"}
										onChange={(e) => setFormData(prev => ({ ...prev, defaultSort: e.target.value }))}
										className="w-full px-3 py-2.5 rounded-lg border border-gray-200 dark:border-white/10 bg-transparent text-xs outline-none focus:border-luxury-gold"
									>
								...
								<div className="space-y-3 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800 flex flex-col justify-between">
									<label className="flex items-center gap-3 cursor-pointer">
										<input type="checkbox" checked={!!formData.showOutOfStock} onChange={(e) => setFormData(prev => ({ ...prev, showOutOfStock: e.target.checked }))} className="sr-only peer" />
										<div className="relative w-9 h-5 bg-gray-300 dark:bg-gray-700 rounded-full peer peer-focus:ring-1 peer-focus:ring-luxury-gold peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-luxury-gold"></div>
										<span className="text-xs font-bold text-gray-400">Hiển thị sản phẩm hết hàng</span>
									</label>
									<p className="text-xs text-gray-500">Khi tắt, sản phẩm hết hàng sẽ bị ẩn khỏi trang danh mục.</p>
								</div>
							</div>
						</div>
					)}
					...
					{activeSubTab === "advanced" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<Code2 className="w-4 h-4 text-luxury-gold" /> CSS Tuỳ Chỉnh & Biểu Tượng Trang
								</h3>
								<p className="text-xs text-gray-400 mt-1">Dành cho nhà quản lý muốn tinh chỉnh giao diện ở cấp độ code. CSS sẽ được áp dụng toàn trang.</p>
							</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								...
								<div className="space-y-3 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
									<h4 className="text-xs font-bold  text-luxury-gold flex items-center gap-1.5">
										<Monitor className="w-3.5 h-3.5" /> Favicon (Biểu tượng Tab trình duyệt)
									</h4>
									<div className="flex gap-3">
										...
										<label className="flex items-center justify-center px-3 py-2 rounded border border-luxury-gold/30 bg-luxury-gold/5 text-luxury-gold hover:bg-luxury-gold/10 text-xs font-semibold cursor-pointer transition shrink-0">
											Tải lên
											<input type="file" accept="image/*" onChange={(e) => handleImageUpload("advanced", "favicon", e.target.files[0])} className="hidden" />
										</label>
									</div>
									{formData.favicon && (
										<div className="flex items-center gap-2 p-2 rounded bg-white dark:bg-black/30">
											<img src={formData.favicon} alt="Favicon" className="w-6 h-6 object-contain" onError={(e) => { e.target.style.display = "none"; }} />
											<span className="text-xs text-gray-500 truncate">{formData.favicon}</span>
										</div>
									)}
									<p className="text-xs text-gray-500">Khuyên dùng ảnh PNG hoặc ICO kích thước 32x32px hoặc 64x64px.</p>
								</div>
								...
								<div className="space-y-3 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
									...
									<div className="flex gap-3">
										<input
											...
											onChange={(e) => setFormData(prev => ({ ...prev, mobileLogoImage: e.target.value }))}
											className="flex-1 px-3 py-2 rounded-lg border border-gray-200 dark:border-white/10 bg-transparent text-xs outline-none focus:border-luxury-gold"
											placeholder="URL logo mobile (tuỳ chọn)"
										/>
										<label className="flex items-center justify-center px-3 py-2 rounded border border-luxury-gold/30 bg-luxury-gold/5 text-luxury-gold hover:bg-luxury-gold/10 text-xs font-semibold cursor-pointer transition shrink-0">
											Tải lên
											<input type="file" accept="image/*" onChange={(e) => handleImageUpload("advanced", "mobileLogoImage", e.target.files[0])} className="hidden" />
										</label>
									</div>
									<p className="text-xs text-gray-500">Nếu để trống, hệ thống sẽ dùng logo chính co dãn tự động.</p>
								</div>
							</div>
					...
					{activeSubTab === "seo" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<Globe className="w-4 h-4 text-luxury-gold" /> Tối ưu hóa Tìm kiếm (SEO) & Thiết lập Vận hành
								</h3>
								<p className="text-xs text-gray-400 mt-1">Cấu hình tiêu đề hiển thị trên Google, Mô tả tiếp thị SEO để tăng thứ hạng tìm kiếm và giờ mở cửa Showroom.</p>
							</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div className="space-y-4 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
									...
									<div className="space-y-3">
										<div>
											<label className="block text-sm font-bold text-gray-400 mb-1">TIÊU ĐỀ TRANG CỬA HÀNG (SEO TITLE)</label>
											<input
												type="text"
												name="seoTitle"
											...
											/>
											<p className="text-xs text-gray-500 mt-1">Khuyên dùng độ dài từ 50-60 ký tự để hiển thị tốt nhất trên kết quả tìm kiếm Google.</p>
										</div>
										<div>
											<label className="block text-sm font-bold text-gray-400 mb-1">MÔ TẢ THƯƠNG HIỆU (META DESCRIPTION)</label>
											<textarea
												name="seoMetaDesc"
												...
												rows={4}
											/>
											<p className="text-xs text-gray-500 mt-1">Khuyên dùng dưới 160 ký tự để Google không bị cắt bớt nội dung mô tả của bạn.</p>
										</div>
									</div>
								</div>
								...
								<div className="space-y-4 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800 flex flex-col justify-between">
									<div>
										<h4 className="text-xs font-bold  text-luxury-gold flex items-center gap-1.5 mb-1">
											<Clock className="w-3.5 h-3.5" /> Thời Gian Vận Hành Cửa Hàng
										</h4>
										<div className="space-y-3 mt-3">
											<div>
												<label className="block text-sm font-bold text-gray-400 mb-1">GIỜ MỞ CỬA SHOWROOM</label>
												<input
													type="text"
												...
												/>
												<p className="text-xs text-gray-500 mt-1">Giờ làm việc này sẽ được hiển thị ở các khu vực thông tin liên hệ và Chân trang Footer.</p>
											</div>
										</div>
									</div>
									...
									<div className="border border-gray-300 dark:border-gray-700 bg-white dark:bg-black/40 rounded-xl p-3.5 mt-4 space-y-1">
										<p className="text-xs text-gray-400 flex items-center gap-1.5"><Globe className="w-2.5 h-2.5" /> {window.location.origin}</p>
										<h5 className="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline cursor-pointer truncate">{formData.seoTitle}</h5>
										<p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed line-clamp-2">{formData.seoMetaDesc}</p>
									</div>
								</div>
							</div>
						</div>
					...
					{activeSubTab === "theme" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<Palette className="w-4 h-4 text-luxury-gold" /> Chủ đề Thẩm mỹ & Hệ màu sắc
								</h3>
								<p className="text-xs text-gray-400 mt-1">Thay đổi ngay lập tức cảm quan phong cách và hòa sắc cốt lõi của website theo định hướng sang trọng.</p>
							</div>

							<div className="space-y-3">
								<label className="block text-xs font-bold  text-luxury-gold">1. Chọn Hệ màu chủ đề chính</label>
										...
										return (
											...
											>
												...
												<div className="flex items-center justify-between mb-2">
													<span className="font-bold text-xs text-gray-900 dark:text-white">{p.label}</span>
													<span className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center ${selected ? "border-luxury-gold" : "border-gray-300"}`}>
														{selected && <span className="w-1.5 h-1.5 rounded-full bg-luxury-gold" />}
													</span>
												</div>
												<p className="text-sm text-gray-500 leading-relaxed mb-3">{p.desc}</p>
												<div className={`mt-auto text-xs font-mono px-2 py-1 rounded text-center border ${p.accent}`}>
							...
							<div className="border-t border-gray-100 dark:border-white/5 pt-5 space-y-3">
								<label className="block text-xs font-bold  text-luxury-gold">2. Chế độ hiển thị phông sáng/tối mặc định</label>
								<div className="grid grid-cols-3 gap-3">
									{[
										{ key: "dark", label: "Always Dark (Khuyên dùng)", desc: "Đúng chuẩn quiet luxury" },
										{ key: "light", label: "Always Light (Lịch lãm)", desc: "Phong cách tối giản tinh tế" },
										{ key: "system", label: "System Auto (Hệ thống)", desc: "Theo thiết bị của khách" }
									].map((mode) => {
										...
										return (
											...
											>
												<input
													...
													className="sr-only"
												/>
												<span className="font-bold text-xs text-gray-900 dark:text-white mb-1">{mode.label}</span>
								...
								{(formData.heroSlides || []).map((slide, index) => (
									<div key={index} className="rounded-xl border border-gray-200 dark:border-gray-800 bg-black/10 dark:bg-black/30 p-4 space-y-4 relative">
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
											<div className="space-y-3">
												<div>
													<label className="block text-sm font-bold  text-gray-400 mb-1">Tiêu đề Slide (Title)</label>
													<input
														type="text"
														value={slide.title || ""}
												...
												<div>
													<label className="block text-sm font-bold  text-gray-400 mb-1">Mô tả phụ (Subtitle)</label>
													<textarea
												...
												<div>
													<label className="block text-sm font-bold  text-gray-400 mb-1">Link liên kết nút bấm (URL)</label>
													<input
														type="text"
														value={slide.link || ""}
											...
											<div className="space-y-3">
												<div>
													<label className="block text-sm font-bold  text-gray-400 mb-1">Ảnh nền (Desktop Banner URL)</label>
													<div className="flex gap-2">
														<input
															type="text"
															value={slide.image || ""}
												...
												<div>
													<label className="block text-sm font-bold  text-gray-400 mb-1">Ảnh nền riêng cho Mobile URL (Tùy chọn)</label>
													<div className="flex gap-2">
														<input
															type="text"
					...
					{activeSubTab === "marketing" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<Megaphone className="w-4 h-4 text-luxury-gold" /> Các Văn bản & Tiêu đề Tiếp thị
								</h3>
								<p className="text-xs text-gray-400 mt-1">Tinh chỉnh các slogan truyền cảm hứng và tiêu đề hiển thị các phân khu trang chủ.</p>
							</div>

							<div className="space-y-4">
								<h4 className="text-xs font-semibold tracking-widest text-luxury-gold">1. Tiêu đề khối & Slogan Tiếp thị</h4>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									...
									<div>
										<label className="block text-xs font-medium mb-1 text-gray-400">Tiêu đề Khối Sản phẩm Bán chạy</label>
										<input
											type="text"
					...
					{activeSubTab === "popup" && (
						<div className="space-y-6 animate-fade-in">
							<div className="flex items-center justify-between border-b border-gray-100 dark:border-white/5 pb-3">
								<div>
									<h3 className="text-base font-bold flex items-center gap-2 text-primary">
										<Sparkles className="w-4 h-4 text-luxury-gold" /> Pop-up Ưu Đãi Đón Khách VIP (Welcome Promo)
									</h3>
									<p className="text-xs text-gray-400 mt-1">Hiển thị một bảng thông điệp chào mừng ưu đãi sang trọng khi khách vừa ghé thăm boutique.</p>
								</div>
								<label className="flex items-center gap-2 cursor-pointer">
									<div className="relative">
										<input
											...
											className="sr-only"
										/>
										<div className={`block w-9 h-5 rounded-full transition-colors ${formData.promoPopupEnabled ? "bg-emerald-500" : "bg-gray-600"}`}></div>
										<div className={`dot absolute left-0.5 top-0.5 bg-white w-4 h-4 rounded-full transition-transform ${formData.promoPopupEnabled ? "transform translate-x-4" : ""}`}></div>
									</div>
									<span className="text-xs font-bold text-gray-300">Bật Pop-up</span>
								</label>
							</div>

							<div className={`grid grid-cols-1 md:grid-cols-2 gap-6 transition-all ${formData.promoPopupEnabled ? "opacity-100" : "opacity-40 pointer-events-none"}`}>
								<div className="space-y-4">
									<div>
										<label className="block text-xs font-medium mb-1 text-gray-400">Tiêu đề Pop-up chào mừng</label>
										<input
											type="text"
									...
									<div>
										<label className="block text-xs font-medium mb-1 text-gray-400">Thời gian chờ kích hoạt pop-up (Delay tính bằng giây)</label>
										...
										<p className="text-xs text-gray-500 mt-1">Xuất hiện sau khi khách hàng vào lướt website đúng N giây.</p>
									</div>
								</div>

								<div className="space-y-4">
									<div>
										<label className="block text-xs font-medium mb-1 text-gray-400">URL Hình ảnh nền bên trái Pop-up</label>
										<input
											type="text"
									...
									{formData.promoPopupImage && (
										<div className="border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden h-36 bg-black/20 flex items-center justify-center relative">
											...
											<div className="absolute inset-0 bg-black/40 p-3 flex flex-col justify-end text-white">
												<p className="text-xs font-bold tracking-wider text-luxury-gold uppercase">{formData.promoPopupTitle}</p>
												<p className="text-xs text-gray-200 truncate mt-0.5">{formData.promoPopupText}</p>
											</div>
										</div>
									)}
								</div>
					...
					{activeSubTab === "footer" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<Globe className="w-4 h-4 text-luxury-gold" /> Thông tin Chân trang & Mạng xã hội của Showroom
								</h3>
								<p className="text-xs text-gray-400 mt-1">Đồng bộ Hotline, địa chỉ liên hệ và các đường dẫn mạng xã hội hiển thị toàn diện ở chân trang Footer.</p>
							</div>

							<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
								<div className="space-y-1">
									<label className="text-xs font-medium text-gray-400 flex items-center gap-1.5">
										<Phone className="w-3.5 h-3.5 text-luxury-gold" /> Hotline Showroom
									</label>
									<input
										type="text"
										name="footerHotline"
								...
								<div className="space-y-1">
									<label className="text-xs font-medium text-gray-400 flex items-center gap-1.5">
										<Mail className="w-3.5 h-3.5 text-luxury-gold" /> Email liên hệ
									</label>
									<input
										type="email"
										name="footerEmail"
								...
								<div className="space-y-1">
									<label className="text-xs font-medium text-gray-400 flex items-center gap-1.5">
										<MapPin className="w-3.5 h-3.5 text-luxury-gold" /> Showroom chính (Địa chỉ)
									</label>
									<input
										type="text"
										name="footerAddress"
							...
							<div className="border-t border-gray-100 dark:border-white/5 pt-5 space-y-4">
								<h4 className="text-xs font-semibold tracking-widest text-luxury-gold flex items-center gap-1.5">
									<Share2 className="w-3.5 h-3.5" /> Đường dẫn Mạng xã hội của thương hiệu
								</h4>
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
									...
									<div className="space-y-1">
										<label className="text-xs font-medium text-gray-400">Zalo Chat / Page Link</label>
										<input
											type="text"
											name="footerZalo"
					...
					{activeSubTab === "layout" && (
						<div className="space-y-6 animate-fade-in">
							<div>
								<h3 className="text-base font-bold flex items-center gap-2 text-primary border-b border-gray-100 dark:border-white/5 pb-3">
									<Grid className="w-4 h-4 text-luxury-gold" /> Bố cục Lưới hiển thị sản phẩm & Hệ thống AI
								</h3>
								<p className="text-xs text-gray-400 mt-1">Điều khiển thiết lập lưới sản phẩm trên trang mua sắm và bật/tắt chatbot trí tuệ nhân tạo.</p>
							</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div>
									...
									<p className="text-xs text-gray-500 mt-1.5">Mật độ hiển thị lưới đồng hồ trên màn hình máy tính của trang Danh mục (Catalog).</p>
								</div>
								...
								<div>
									...
									<div className="flex gap-2">
										{[4, 6, 8, 12].map(num => (
											<label key={num} className={`flex-1 flex flex-col items-center justify-center cursor-pointer p-3.5 rounded-xl border-2 transition ${Number(formData.featuredCount) === num ? "border-luxury-gold bg-luxury-gold/5" : "border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 bg-transparent"}`}>
												...
												<span className="font-bold text-base mb-0.5">{num}</span>
												<span className="text-xs text-gray-500">Đồng hồ</span>
											</label>
										))}
									</div>
									<p className="text-xs text-gray-500 mt-1.5">Số lượng sản phẩm tối đa xuất hiện ở khối &quot;Tuyển chọn tinh hoa&quot;.</p>
								</div>
							</div>
							...
							<div className="border-t border-gray-100 dark:border-white/5 pt-5">
								<label className="flex items-center gap-4 cursor-pointer bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
									<div className="relative">
										...
										<div className={`block w-9 h-5 rounded-full transition-colors ${formData.showChatBot ? "bg-emerald-500" : "bg-gray-600"}`}></div>
										<div className={`dot absolute left-0.5 top-0.5 bg-white w-4 h-4 rounded-full transition-transform ${formData.showChatBot ? "transform translate-x-4" : ""}`}></div>
									</div>
									<div className="flex items-center gap-2">
										<MessageSquareText className="w-5 h-5 text-luxury-gold" />
										<div>
											<span className="font-bold text-xs text-gray-900 dark:text-white block">Kích hoạt Trợ lý AI Cố vấn mua sắm</span>
											<span className="text-xs text-gray-500">Hiển thị bong bóng trò chuyện tư vấn đồng hồ AI thông minh ở góc phải màn hình của khách hàng.</span>
										</div>
									</div>
								</label>
							</div>
