...
const ProductsList = () => {
  ...
  const {
    ...
    selectedCount,
    clearSelection,
  } = useProductsBulkSelect(products);
  const {
    openCreateModal,
    openEditModal,
    closeEditModal,
  ...
  const handleBulkDelete = () => {
    ...
    openBulkDeleteConfirm("bulkDeleteConfirm", {
      count: selectedCount,
  ...
  const handleConfirmPriceAdjust = async (value) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Lỗi khi điều chỉnh giá", {
        id: toastId,
      });
    }
  };
  ...
  const handleFilePreview = async (e) => {
    ...
    try {
      ...
      toast.success(
        `Đọc được ${res.data.total} dòng. Kiểm tra trước khi import.`,
        { id: toastId },
      );
    } catch (error) {
  ...
  const handleConfirmedImport = async () => {
    ...
    try {
      ...
      toast.success(`Thành công: ${res.data.success} sản phẩm`, {
        id: toastId,
        duration: 5000,
      });
  ...
  const handleFileImport = async (e) => {
    ...
    try {
      ...
      toast.success(
        `Thành công: ${res.data.success}, Thất bại: ${res.data.failed}`,
  ...
  return (
    <div className="space-y-4">
      ...
      <ProductsToolbar
        ...
        onApplyCampaign={() =>
          useModalStore.getState().openModal("campaignPicker")
        }
              ...
              <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-700">
                <div>
                  <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <Eye className="w-5 h-5 text-luxury-gold" /> Xem trước
                    Import Excel
                  </h2>
      ...
      <ConfirmModal
        ...
        onCancel={() =>
          useModalStore.getState().closeModal("bulkDeleteConfirm")
        }
        confirmText="Xóa"
        isDangerous
      />
