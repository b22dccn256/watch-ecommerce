...
export const useCampaignStore = createWithEqualityFn((set, get) => ({
  ...
  fetchCampaigns: async (force = false) => {
    ...
    if (
      !force &&
      get().campaigns.length > 0 &&
      now - fetchState.campaigns.lastFetched < FETCH_TTL_MS
    ) {
    ...
    fetchState.campaigns.promise = axios
      ...
      .catch((error) => {
        set({
          error:
            error.response?.data?.message || "Lỗi khi lấy danh sách chiến dịch",
          loading: false,
        });
        toast.error(
          error.response?.data?.message || "Lỗi khi lấy danh sách chiến dịch",
  ...
  fetchActiveCampaigns: async (force = false) => {
    ...
    if (
      !force &&
      get().campaigns.length > 0 &&
      now - fetchState.active.lastFetched < FETCH_TTL_MS
    ) {
      return get().campaigns;
    }
  ...
  deleteCampaign: async (campaignId) => {
    try {
      ...
      set((prevState) => ({
        campaigns: prevState.campaigns.filter(
          (camp) => camp._id !== campaignId,
        ),
      }));
