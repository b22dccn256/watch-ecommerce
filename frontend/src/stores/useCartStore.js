...
export const useCartStore = createWithEqualityFn((set, get) => ({
  ...
  selectedItems: JSON.parse(
    localStorage.getItem("watch_selected_items") || "[]",
  ), // stores unique cartItemIds
  isCartLoaded: false,
  coupon: null,
  total: 0,
  subtotal: 0,
  ...
  getUniqueId: (item) => {
    const baseId =
      item?.product?._id || item?.productId || item?._id || "unknown";
  ...
  selectAllItems: (isSelected, products) => {
    if (isSelected) {
      get().setSelectedItems(
        products.map((p) => (typeof p === "string" ? p : get().getUniqueId(p))),
      );
    } else {
      get().setSelectedItems([]);
    }
  ...
  getCartItems: async () => {
    ...
    try {
      ...
      if (error.response?.status !== 401) {
        toast.error(
          error.response?.data?.message || "An error occurred fetching cart",
        );
      }
    }
  },
  clearSelectedCart: () => {
    ...
    const newCart = cart.filter(
      (item) => !selectedItems.includes(getUniqueId(item)),
    );
    if (!user) {
  ...
  addToCart: async (product) => {
    ...
    if (
      !normalizedProduct.wristSize &&
      normalizedProduct.wristSizeOptions?.length > 0
    ) {
      const firstAvailable =
        normalizedProduct.wristSizeOptions.find((opt) => opt.stock > 0)?.size ||
    ...
    const existingItem = prevState.cart.find(
      (item) => prevState.getUniqueId(item) === uniqueId,
    );
    const newQuantity = existingItem ? existingItem.quantity + 1 : 1;

    if (
      normalizedProduct.stock !== undefined &&
      normalizedProduct.stock < newQuantity
    ) {
      return toast.error(
        `Sản phẩm này chỉ còn ${normalizedProduct.stock} cái trong kho`,
      );
    }
    ...
    if (!user) {
      ...
      const newCart = existingItem
        ? prevState.cart.map((item) =>
            prevState.getUniqueId(item) === uniqueId
  ...
  removeFromCart: async (
    ...
    if (get().selectedItems.includes(uniqueId)) {
      get().setSelectedItems(
        get().selectedItems.filter((id) => id !== uniqueId),
      );
    }

    if (!user) {
      const newCart = get().cart.filter(
        (item) => get().getUniqueId(item) !== uniqueId,
      );
      localStorage.setItem("watch_cart", JSON.stringify(newCart));
      set({ cart: newCart });
      get().calculateTotals();
    ...
    try {
      await axios.delete(`/cart`, {
        data: { productId, wristSize, selectedColor, selectedSize },
      });
      set((prevState) => ({
  ...
  updateQuantity: async (
    ...
    if (!user) {
      const newCart = get().cart.map((item) =>
        get().getUniqueId(item) === uniqueId ? { ...item, quantity } : item,
      );
      localStorage.setItem("watch_cart", JSON.stringify(newCart));
      set({ cart: newCart });
      get().calculateTotals();
  ...
  updateCartItemAttributes: async (
    productId,
    currentAttributes,
    nextAttributes,
  ) => {
    ...
    const mergeLocalCartItem = () => {
      ...
      const currentItem = currentCart.find(
        (item) => get().getUniqueId(item) === previousKey,
      );
      ...
      const conflictItem = currentCart.find(
        (item) =>
          get().getUniqueId(item) === updatedKey &&
          get().getUniqueId(item) !== previousKey,
      );
      ...
      return currentCart.map((item) =>
        get().getUniqueId(item) === previousKey ? updatedItem : item,
      );
    };
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Không thể cập nhật thuộc tính",
      );
    }
  },
