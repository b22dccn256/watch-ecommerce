...
export const useProductStore = createWithEqualityFn((set, get) => ({
  ...
  fetchBrands: async (force = false) => {
    ...
    if (
      !force &&
      get().brands.length > 0 &&
      now - fetchState.brands.lastFetched < FETCH_TTL_MS
    ) {
      return get().brands;
    }
  ...
  fetchCategories: async (force = false) => {
    ...
    if (
      ...
      now - fetchState.categories.lastFetched < FETCH_TTL_MS
    ) {
  ...
  fetchAllProducts: async (force = false) => {
    ...
    if (
      !force &&
      get().allProducts.length > 0 &&
      now - fetchState.allProducts.lastFetched < FETCH_TTL_MS
    ) {
  ...
  fetchProductsAdminPaginated: async ({
    page = 1,
    limit = 12,
    search = "",
    category = "",
    ...
    const now = Date.now();
    if (fetchState.adminPage.promise && fetchState.adminPage.lastKey === key)
      return;
    if (
      fetchState.adminPage.lastKey === key &&
      now - fetchState.adminPage.lastFetched < 1000
    )
      return;
    ...
    fetchState.adminPage.promise = (async () => {
      try {
        ...
        set({ loading: false });
        toast.error(
          error.response?.data?.error || "Lỗi khi lấy danh sách sản phẩm.",
        );
      } finally {
  ...
  fetchFilteredProducts: async (extraParams = {}) => {
    ...
    const effectiveMaxPrice =
      filters.maxPrice < 1_000_000_000 ? filters.maxPrice : undefined;

    const storefrontConfig = useStorefrontStore.getState().config;
    const productsPerPage = storefrontConfig?.productsPerPage
      ? Number(storefrontConfig.productsPerPage)
      : 12;
