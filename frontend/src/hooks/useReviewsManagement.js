...
export const useReviewsManagement = () => {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      handleError(error, {
        context: "useReviewsManagement.refresh",
        showToast: true,
      });
    } finally {
      setLoading(false);
  ...
  const filteredReviews = useMemo(() => {
    return reviews.filter((r) => {
      if (reviewRating !== "all" && r.rating !== Number(reviewRating))
        return false;
      return true;
    });
  ...
  const reviewStats = useMemo(() => {
    ...
    const avg =
      total > 0
        ? (reviews.reduce((acc, curr) => acc + curr.rating, 0) / total).toFixed(
            1,
          )
        : 0;
  ...
  const updateReviewStatus = async (id, newStatus) => {
    try {
      ...
      handleError(error, {
        context: "useReviewsManagement.updateStatus",
        showToast: true,
      });
    }
  };
  ...
  const deleteReview = (id) => {
    confirmToast("Xóa vĩnh viễn đánh giá này?", async () => {
      try {
        ...
        handleError(error, {
          context: "useReviewsManagement.delete",
          showToast: true,
        });
      }
    });
  ...
  const submitReply = async (e) => {
    ...
    try {
      ...
      handleError(error, {
        context: "useReviewsManagement.submitReply",
        showToast: true,
      });
    }
  };
