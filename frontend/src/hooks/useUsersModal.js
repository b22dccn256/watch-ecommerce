...
export const useUsersModal = (onUsersRefetch) => {
   ...
   * Update user tags
   */
  const handleUpdateTags = useCallback(async (userId, tags) => {
    try {
   ...
   */
  const handleUpdateNotes = useCallback(async (userId, adminNotes) => {
    try {
  ...
  const handleConfirmLoyalty = useCallback(
    async (value) => {
      ...
      try {
        const res = await axios.patch(`/auth/users/${selectedUser._id}/loyalty`, {
          delta: Number(value),
        });
