...
const DEFAULT_AUTOMATIONS = [
  ...
  },
  {
    id: "welcome-email",
    title: "Welcome Email",
    desc: "Gửi lời chào và mã giảm giá 10% khi khách đăng ký Newsletter.",
    active: true,
  },
  {
    id: "birthday-email",
    title: "Birthday Email",
    desc: "Tự động gửi lời chúc vào ngày sinh nhật khách hàng.",
    active: false,
  },
];
...
export const useEmailTabData = () => {
  ...
  const fetchData = useCallback(async () => {
    const tabState = fetchStateRef.current[activeTab] || {
      promise: null,
      lastFetched: 0,
    };
  ...
  const handleMarkMessageRead = async (id) => {
    ...
    const previousStatus =
      data.messages.find((m) => m._id === id)?.status || "new";
    setData((prev) => ({
      ...prev,
    ...
    try {
      ...
      setData((prev) => ({
        ...prev,
        messages: prev.messages.map((m) =>
          m._id === id ? { ...m, status: previousStatus } : m,
        ),
      }));
  ...
  const handleDeleteTemplate = (id) => {
    confirmToast(
      `Xóa mẫu email này? Hành động này không thể hoàn tác.`,
      async () => {
        try {
