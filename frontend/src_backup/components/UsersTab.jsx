						...
						<div className="space-y-4">
							...
							{userDetailTab === "info" && (
								...
								<div className="pt-4 border-t border-gray-100 dark:border-luxury-border">
									<p className="text-[10px] uppercase font-bold text-gray-400 mb-2 tracking-widest">Điểm Tích Lũy</p>{/* FIX C2 */}
									<div className="flex items-center gap-3">
										<span className="text-2xl font-bold text-luxury-gold">{selectedUser.rewardPoints || 0}</span>
										<span className="text-xs text-gray-400">/ {selectedUser.totalPointsEarned || 0} tổng đã tích</span>{/* FIX C2 */}
										<button onClick={() => setShowLoyaltyModal(true)} className="ml-auto px-3 py-1 text-xs font-bold bg-amber-400/10 border border-amber-400/20 text-amber-400 rounded-lg hover:bg-amber-400/20 transition">+/- Điểm</button>
									</div>
								</div>
								{/* D2: Admin Notes & Tags */}
								<div className="pt-4 border-t border-gray-100 dark:border-luxury-border space-y-2">
									<p className="text-[10px] uppercase font-bold text-gray-400 tracking-widest">Tags Nội Bộ</p>{/* FIX C3 */}
