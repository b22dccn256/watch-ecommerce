  ...
  try {
    ...
    if (coupon.userId) {
      if (!req.user)
        return res
          .status(403)
          .json({ message: "Coupon này chỉ dành cho người dùng đã đăng nhập" });
      if (String(coupon.userId) !== String(req.user._id))
        return res
          .status(403)
          .json({ message: "Bạn không có quyền sử dụng coupon này" });
    }
...
export const getAllCoupons = async (req, res) => {
  try {
    ...
    const couponsWithStats = coupons.map((coupon) => {
      const usedToday = coupon.usageHistory
        ? coupon.usageHistory.filter((h) => new Date(h.usedAt) >= startOfToday)
            .length
        : 0;
...
export const toggleCoupon = async (req, res) => {
  try {
    ...
    await coupon.save();
    res.json({
      message: coupon.isActive ? "Đã kích hoạt coupon" : "Đã tắt coupon",
  ...
  try {
    ...
    if (!code || discountValue === undefined || !expirationDate) {
      return res
        ...
        .json({
          message:
            "Thiếu thông tin bắt buộc: code, discountValue, expirationDate",
        });
    }
