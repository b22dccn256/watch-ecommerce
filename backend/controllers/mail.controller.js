...
let emailQueue = {
  add: async (name, payload) => {
    console.log(
      `[Mail Stub Queue] Fallback direct send queue initiated for "${name}" to ${payload.email}...`,
    );
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            console.error(
              "[Mail Stub Queue] Failed to create email log:",
              logErr.message,
            );
          }
        }
        ...
        const subject =
          payload.subject ||
          defaultSubjects[name] ||
          "Thông tin từ Luxury Watch Store";

        const recipient =
          payload.type === "admin-notification"
            ? process.env.ADMIN_EMAIL || "admin@watchstore.com"
            : payload.email;
        ...
        console.log(
          `[Mail Stub Queue] Fallback email sent successfully to ${recipient}`,
        );
      } catch (error) {
        console.error(
          `[Mail Stub Queue] Fallback direct send failed:`,
          error.message,
        );
      }
    })();
  ...
  addBulk: async (jobs) => {
    console.log(
      `[Mail Stub Queue] Processing ${jobs.length} bulk jobs directly...`,
    );
...
if (
  ...
  try {
    ...
    redisConnection.on("error", (err) => {
      console.error(
        "[Mail] IORedis connection error on controller Queue:",
        err.message,
      );
    });
    ...
    console.error(
      "[Mail] Redis connection failed, using stub queue:",
      err?.message || err,
    );
  }
}
...
export const getMailStats = async (req, res) => {
  try {
    ...
    const totalOpened = await EmailLog.countDocuments({
      openedAt: { $exists: true, $not: { $size: 0 } },
    });
    const openRate =
      sentEmails > 0 ? ((totalOpened / sentEmails) * 100).toFixed(1) : 0;
...
export const sendCampaignNow = async (req, res) => {
  try {
    const campaign = await MailCampaign.findById(req.params.id).populate(
      "template",
    );
    if (!campaign)
    ...
    if (
      ...
      campaign.targetAudience === "newsletter"
    ) {
    ...
    res.json({
      message: "Đang xử lý gửi " + targetEmails.length + " email...",
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
export const scheduleCampaign = async (req, res) => {
  try {
    const campaign = await MailCampaign.findByIdAndUpdate(
      req.params.id,
      {
        status: "scheduled",
...
export const getSubscribers = async (req, res) => {
  try {
    ...
    const search = (req.query.search || "").trim();
    const isSubscribed =
      req.query.isSubscribed !== undefined
        ? req.query.isSubscribed === "true"
        : undefined;
  ...
  try {
    ...
    if (existing) {
      if (existing.isSubscribed) {
        return res
          .status(200)
          .json({ message: "Bạn đã đăng ký nhận tin rồi!" });
      }
    ...
    try {
      ...
      try {
        ...
        await sendEmail(
          email,
          "Chào mừng bạn đến với Luxury Watch Store!",
          finalHtml,
        );
        console.info("Fallback direct welcome email sent successfully!");
      } catch (directError) {
        console.error(
          "Fallback direct welcome email also failed:",
          directError.message,
        );
      }
    }
  ...
  try {
    ...
    await NewsletterSubscription.findOneAndUpdate(
      { email },
      {
        ...
        unsubscribedReason: "User requested",
      },
    );
    res.send(
      "<h1>Bạn đã hủy đăng ký nhận tin thành công.</h1><p>Chúng tôi rất tiếc khi thấy bạn rời đi.</p>",
    );
  } catch (error) {
  ...
  try {
    ...
    const sub = await NewsletterSubscription.findOneAndUpdate(
      { unsubscribeToken: token },
      {
        isSubscribed: false,
        unsubscribedAt: new Date(),
        unsubscribedReason: "User requested via token",
      },
      { new: true },
    );
    if (!sub) {
      return res
        .status(404)
        .send(
          "<h1>Liên kết hủy đăng ký không hợp lệ hoặc đã được sử dụng.</h1>",
        );
    }
    res.send(
      "<h1>Bạn đã hủy đăng ký nhận tin thành công.</h1><p>Chúng tôi rất tiếc khi thấy bạn rời đi.</p>",
