  ...
  try {
    ...
    if (!name || !email || !message) {
      return res
        ...
        .json({
          message:
            "Vui lòng điền đầy đủ các trường bắt buộc (Tên, Email, Lời nhắn).",
        });
    }
    ...
    await emailQueue.add("admin-notification", {
      ...
      subject:
        "Tin nhắn mới từ " + name + ": " + (subject || "Không có chủ đề"),
    });

    res.status(200).json({
      message:
        "Cảm ơn bạn đã liên hệ. Đội ngũ của chúng tôi sẽ phản hồi sớm nhất qua email.",
