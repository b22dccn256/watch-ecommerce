...
export const getWishlistProducts = async (req, res) => {
  try {
    let wishlist = await Wishlist.findOne({ user: req.user._id }).populate(
      "items.product",
      "name price image category description stock originalPrice",
    );
...
export const addToWishlist = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Added to wishlist", count: wishlist.items.length });
  } catch (error) {
...
export const removeFromWishlist = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Removed from wishlist", count: wishlist.items.length });
  } catch (error) {
