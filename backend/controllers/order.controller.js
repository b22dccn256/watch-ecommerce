...
export const requestReturnOrder = async (req, res) => {
  try {
    ...
    if (!order.user || order.user.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Bạn không có quyền yêu cầu trả hàng cho đơn này" });
    }
    if (order.status !== "delivered") {
      return res
        .status(400)
        .json({ message: "Chỉ có thể trả hàng khi đơn đã giao thành công" });
    }
...
export const cancelOrder = async (req, res) => {
  try {
    ...
    if (
      !["pending", "awaiting_verification", "confirmed"].includes(order.status)
    ) {
      return res
        .status(400)
        .json({ message: "Chỉ có thể hủy đơn khi chưa được xử lý" });
    }
...
const ensureOrderProductsPopulated = async (order) => {
  ...
  orderObj.products = await Promise.all(
    orderObj.products.map(async (item) => {
      ...
      try {
        const productData = await Product.findById(item.product).select(
...
export const getAllOrders = async (req, res) => {
  try {
    ...
    const ordersWithProductData = await Promise.all(
      orders.map(ensureOrderProductsPopulated),
    );
    ...
    if (format === "csv") {
      ...
      const rows = orders.map((o) => {
        ...
        const products = (o.products || [])
          .map(
            (p) => `${p.product?.name || p.product}_${p.quantity}@${p.price}`,
          )
          .join("; ");
        return [
          o.orderCode,
    ...
    if (refundAmount !== undefined) {
      if (refundAmount < 0 || refundAmount > order.totalAmount) {
        return res
          ...
          .json({
            message:
              "Số tiền hoàn không hợp lệ (phải từ 0 đến tổng giá trị đơn hàng)",
          });
      }
    ...
    if (carrier !== undefined) {
      if (!VALID_CARRIERS.includes(carrier)) {
        return res
          .status(400)
          .json({
            message: `Đơn vị vận chuyển không hợp lệ. Hợp lệ: ${VALID_CARRIERS.join(", ")}`,
          });
      }
      order.carrier = carrier;
    }
    if (carrierTrackingNumber !== undefined)
      order.carrierTrackingNumber = carrierTrackingNumber;
...
export const getMyOrders = async (req, res) => {
  try {
    ...
    const orders = await Promise.all(
      ordersFetched.map(ensureOrderProductsPopulated),
    );
    res.json(orders);
  } catch (error) {
...
// Refactored: Call OrderService
export const createCODOrder = (req, res) =>
  OrderService.createNonStripeOrder(req, res, "cod");
...
export const getOrderTracking = async (req, res) => {
  try {
    ...
    const order = await Order.findOne({
      ...
      .select(
        "trackingToken orderCode status paymentMethod paymentStatus totalAmount estimatedDelivery carrier carrierTrackingNumber trackingEvents shippingDetails products createdAt",
      )
    ...
    if (safeOrder.shippingDetails) {
      if (safeOrder.shippingDetails.email) {
        ...
        safeOrder.shippingDetails.email =
          parts[0].substring(0, 2) + "***@" + parts[1];
      }
      if (safeOrder.shippingDetails.phoneNumber) {
        safeOrder.shippingDetails.phoneNumber =
          safeOrder.shippingDetails.phoneNumber.substring(0, 3) +
          "****" +
          safeOrder.shippingDetails.phoneNumber.slice(-3);
      }
    }
...
export const lookupOrder = async (req, res) => {
  try {
    ...
    if (!normalizedOrderNumber || !normalizedEmail) {
      return res
        .status(400)
        .json({ message: "Vui lòng nhập mã đơn hàng và email." });
    }
    ...
    if (!order) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy đơn hàng khớp với mã cung cấp." });
    }

    const isEmailMatch =
      (order.shippingDetails?.email || "").trim().toLowerCase() ===
        normalizedEmail ||
    ...
    if (!isEmailMatch) {
      return res
        .status(404)
        .json({ message: "Thông tin email hoặc mã đơn hàng chưa chính xác." });
    }
