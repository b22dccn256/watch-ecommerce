...
export const getAnalyticsData = async ({ startDate, endDate } = {}) => {
  ...
  const recentPendingOrders = await Order.find({
    ...
    .limit(5)
    .select(
      "_id orderCode totalAmount shippingDetails.fullName createdAt status paymentStatus",
    )
    .lean();
  ...
  const aov =
    totalOrdersPlaced > 0
      ? Math.round((salesData[0]?.totalRevenue || 0) / totalOrdersPlaced)
      : 0;

  const conversionRate =
    totalUsers > 0 ? (totalOrdersPlaced / totalUsers) * 100 : 0;

  // Hourly Sales (for today, timezone VN)
  const vnNow = new Date();
  const vnParts = getVietnamDateParts(vnNow);
  const todayStart = new Date(
    `${vnParts.year}-${String(vnParts.month).padStart(2, "0")}-${String(vnParts.day).padStart(2, "0")}T00:00:00+07:00`,
  );
...
export const getAnalytics = async (req, res) => {
  try {
    ...
    const localStart = new Date(
      Date.UTC(vnParts.year, vnParts.month - 1, vnParts.day),
    );
...
export const sendTestEmail = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Không thể gửi email test", error: error.message });
  }
};
              ...
              $multiply: [
                {
                  $ifNull: [
                    ...
                    { $multiply: ["$products.price", 0.6] },
                  ],
                }, // fallback: 60% of price
                "$products.quantity",
              ],
            },
          },
