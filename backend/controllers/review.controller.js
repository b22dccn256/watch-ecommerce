...
export const createReview = async (req, res) => {
  try {
    ...
    if (!rating || !comment) {
      return res
        .status(400)
        .json({ message: "Vui lòng nhập số sao và bình luận." });
    }
    ...
    if (!hasBought) {
      return res
        ...
        .json({
          message:
            "Bạn chỉ có thể đánh giá sản phẩm sau khi đã nhận hàng (Trạng thái: Đã giao).",
        });
    }
    ...
    if (product && process.env.NODE_ENV !== "production") {
      ...
      product.averageRating =
        (product.averageRating * (product.reviewsCount - 1) + review.rating) /
...
export const updateReviewStatus = async (req, res) => {
  try {
    ...
    if (!["pending", "approved", "hidden"].includes(status)) {
      return res
        .status(400)
        .json({ message: "Trạng thái review không hợp lệ" });
    }
    ...
    if (product) {
      ...
      product.averageRating =
        remainingReviews.length > 0
          ? remainingReviews.reduce((sum, item) => sum + item.rating, 0) /
