...
export const getProductBySlugToken = async (req, res) => {
  try {
    ...
    if (slug !== canonicalSlug || token !== canonicalToken) {
      return res.redirect(
        301,
        `/api/products/${canonicalSlug}--${canonicalToken}`,
      );
    }
...
export const getFeaturedProducts = async (req, res) => {
  try {
    ...
    const processed =
      await CampaignService.applyCampaignToProducts(featuredProducts);
    res.json(processed);
  } catch (error) {
...
export const updateProduct = async (req, res) => {
  try {
    ...
    if (wristSizeOptions !== undefined)
      product.wristSizeOptions = wristSizeOptions;
...
export const bulkUpdateProducts = async (req, res) => {
  try {
    ...
    if (action === "adjustPrice") {
      ...
      return res.json({
        message: `Đã điều chỉnh giá ${pct > 0 ? "+" : ""}${pct}% cho ${modifiedCount} sản phẩm`,
      });
    }
    ...
    if (action === "toggleFeatured") {
      ...
      await updateFeaturedProductsCache();
      return res.json({
        message: `Đã toggle nổi bật cho ${result.modifiedCount} sản phẩm`,
      });
    }
...
export const previewImportProducts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error during preview", error: error.message });
  }
};
...
export const importProducts = async (req, res) => {
  ...
  try {
    ...
    console.log(
      "ℹ️ Transactions not supported on this MongoDB setup. Importing without transaction safety.",
    );
  }
    ...
    for (const [index, row] of data.entries()) {
      try {
        await processImportRow(
          row,
          transactionActive ? session : null,
          req.user._id,
        );
        successCount++;
      } catch (err) {
