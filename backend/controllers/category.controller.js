...
const updateDescendantsLevelAndAncestors = async (
  parentId,
  parentLevel,
  parentAncestors,
) => {
  ...
  for (let child of children) {
    ...
    await updateDescendantsLevelAndAncestors(
      child._id,
      child.level,
      child.ancestors,
    );
  ...
  try {
    ...
    if (parentCategory) {
      if (categoryId === parentCategory) {
        return res
          .status(400)
          .json({ message: "Danh mục không thể làm cha của chính nó" });
      }
      ...
      if (isLoop) {
        return res
          ...
          .json({
            message:
              "Lặp cấu trúc vô hạn: Danh mục cha đang chọn thực chất là danh mục con/cháu của danh mục này.",
          });
      }
    }
    ...
    if (
      ...
      String(parentCategory) !== String(category.parentCategory)
    ) {
      let newLevel = 0;
      let newAncestors = [];
      ...
      if (parentCategory) {
        ...
        if (!parent)
          return res
            .status(404)
            .json({ message: "Không tìm thấy danh mục cha" });
  ...
  try {
    ...
    if (hasChildren) {
      return res
        ...
        .json({
          message:
            "Không thể xóa: Vui lòng chuyển hoặc xóa các danh mục con trước.",
        });
    }
