...
const csv = (value = "") =>
  String(value)
    ...
    .filter(Boolean);
const regexIn = (value = "") =>
  csv(value).map((item) => new RegExp(escapeRegex(item), "i"));
            ...
            in: {
              $or: keys
                .map((key) => {
                  ...
                  const conditions = [];
                  if (range.min != null)
                    conditions.push({ $gte: ["$$size", range.min] });
                  if (range.max != null)
                    conditions.push({ $lt: ["$$size", range.max] });
...
export async function buildProductQuery({
  ...
  if (machineType) {
    const types = csv(machineType)
      .map((type) => type.toLowerCase())
      .filter((type) => MOVEMENT_TYPES.has(type));
    if (types.length > 0)
      query.type = {
        $in: types.map((type) => new RegExp(`^${escapeRegex(type)}$`, "i")),
      };
  }
  ...
  if (waterResistance) {
    const waterConditions = csv(waterResistance)
      .map((key) => {
                    ...
                    $ifNull: [
                      {
                        $getField: {
                          ...
                          input: {
                            $regexFind: {
                              input: { $toString: "$specs.waterResistance" },
                              regex: /\d+(?:\.\d+)?/,
                            },
                          },
                        },
        ...
        return {
          "specs.waterResistance": new RegExp(`\\b${value}\\s*m?\\b`, "i"),
        };
      })
      .filter(Boolean);
...
export async function handleProductImage(newImage, oldImage = null) {
  ...
  if (oldImage) {
    try {
      ...
      console.error(
        "[ProductService] Failed to delete old Cloudinary image (swallowed):",
        err.message,
      );
    }
  }
  ...
  try {
    const response = await cloudinary.uploader.upload(newImage, {
      folder: "products",
    });
    return response.secure_url;
  } catch (cloudinaryError) {
    console.warn(
      "[ProductService] Cloudinary upload failed, falling back to local file upload:",
    ...
    if (String(newImage).startsWith("data:image/")) {
      try {
        ...
        console.error(
          "[ProductService] Local fallback write failed:",
          localWriteError.message,
        );
      }
    }

    // Default placeholder fallback
    return newImage.startsWith("http")
      ? newImage
...
export async function updateFeaturedProductsCache() {
  try {
    ...
    console.error(
      "[ProductService] Failed to update featured_products cache:",
      error.message,
    );
  }
}
...
export async function processImportRow(row, session, userId) {
  ...
  let categoryId = null;
  if (categoryName) {
    let cat = await Category.findOne({ name: categoryName }).session(
      session || undefined,
    );
    if (!cat) {
        ...
        [
          {
            name: categoryName,
            slug:
              slugifyProductName(categoryName) ||
  ...
  let brandId = null;
  if (brandName) {
    let brandObj = await Brand.findOne({ name: brandName }).session(
      session || undefined,
    );
    if (!brandObj) {
      const createOptions = session ? { session } : {};
  ...
  if (row.type || row["Loại máy"]) {
    const rawType = String(row.type || row["Loại máy"])
      .toLowerCase()
      .trim();
    if (rawType.includes("cơ tự động") || rawType.includes("automatic")) {
      productType = "automatic";
    } else if (
      rawType.includes("cơ lên cót") ||
      rawType.includes("mechanical") ||
      rawType.includes("hand-wound")
    ) {
  ...
  const descVal =
    row.description ||
    row["Mô tả"] ||
    `${productName} - Đồng hồ cao cấp chính hãng mang phong cách sang trọng và lịch lãm.`;
  const imgVal =
    row.image ||
    row["Ảnh đại diện"] ||
    row["Ảnh"] ||
  ...
  if (product) {
    ...
    const createOptions = session ? { session } : {};
    const costPriceVal =
      row.costPrice ?? row["Giá nhập"] ?? Math.round(priceVal * 0.7);
