...
export const handlePaymentSuccess = async (session) => {
    ...
    try {
      ...
      if (coupon) {
        ...
        if (
          ...
          String(coupon.userId) !== String(session.metadata.userId)
        ) {
          ...
          if (
            update &&
            update.maxUses > 0 &&
            (update.usedCount || 0) >= update.maxUses
          ) {
  ...
  if (order && order.paymentStatus === "pending") {
    ...
    if (session.metadata.userId && session.metadata.userId !== "guest") {
      ...
      if (userToClear) {
        const orderedProductIds = order.products.map((p) =>
          p.product.toString(),
        );
        userToClear.cartItems = userToClear.cartItems.filter(
          (item) =>
            item.product &&
