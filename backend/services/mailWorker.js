  ...
  worker = new Worker(
    ...
    async (job) => {
      ...
      try {
          ...
          if (job.name === "welcome-email") {
            ...
            finalHtml = `
						<div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #eee; border-radius: 8px;">
							...
							<p>Quý khách có thể xem chi tiết hành trình vận chuyển tại: <br/><a href="${process.env.CLIENT_URL || "https://timematrix.io.vn"}/order-tracking/${job.data.order.trackingToken}" style="color: #D4AF37; font-weight: bold; text-decoration: none;">Theo dõi đơn hàng</a></p>
        ...
        // 5. Send Email
        const recipient =
          type === "admin-notification" ? process.env.ADMIN_EMAIL : email;
