...
class OrderService {
  ...
  static async updateUserStatsAndSegment(
    userId,
    amountDelta,
    countDelta,
    pointsDelta = 0,
    ...
    if (amountDelta !== 0)
      user.totalSpend = Math.max(0, (user.totalSpend || 0) + amountDelta);
    if (countDelta !== 0)
      user.orderCount = Math.max(0, (user.orderCount || 0) + countDelta);
    if (pointsDelta !== 0)
      user.rewardPoints = Math.max(0, (user.rewardPoints || 0) + pointsDelta);
    if (earnedPointsDelta !== 0)
      user.totalPointsEarned = Math.max(
        0,
        (user.totalPointsEarned || 0) + earnedPointsDelta,
      );
  ...
  static async deductStock(
    ...
    for (const item of products) {
      ...
      const product = await Product.findById(item._id || item.id).session(
        session,
      );
      ...
      if (product && item.wristSize && product.wristSizeOptions?.length > 0) {
        sizeOption = product.wristSizeOptions.find(
          (o) => o.size === item.wristSize,
        );
      ...
      if (!product || availableStock < item.quantity) {
        throw new Error(
          `Sản phẩm "${product?.name || item._id}" (size ${item.wristSize || "mặc định"}) chỉ còn ${availableStock} cái`,
        );
      }
  ...
  static async restoreStock(
    ...
    session = null,
    orderId = null,
    note = "Hủy đơn hàng / Hoàn kho",
  ) {
    for (const item of products) {
      const product = await Product.findById(
        item.product ? item.product : item._id || item.id,
      ).session(session);
      if (product) {
        product.stock += item.quantity;

        if (item.wristSize && product.wristSizeOptions?.length > 0) {
          const sizeOption = product.wristSizeOptions.find(
            (o) => o.size === item.wristSize,
          );
          if (sizeOption) sizeOption.stock += item.quantity;
        }
  ...
  static async calculateTotals(products, coupon, city = "", session = null) {
    let subtotal = 0;
    for (const item of products) {
      let product = await Product.findById(item._id || item.id).session(
        session,
      );
      if (!product) {
    ...
    if (coupon) {
      if (coupon.minOrderAmount > 0 && subtotal < coupon.minOrderAmount) {
        throw new Error(
          "Đơn hàng tối thiểu để áp dụng coupon này là " +
  ...
  static async calculateTotalAmount(
    ...
    const { total } = await this.calculateTotals(
      products,
      coupon,
      city,
      session,
    ...
    try {
      ...
      for (const p of products) {
        if (!Number.isInteger(p.quantity) || p.quantity < 1) {
          ...
          return res
            .status(400)
            .json({ message: `Số lượng sản phẩm không hợp lệ: ${p.quantity}` });
        }
      }
      if (
        ...
        return res
          .status(400)
          .json({ message: "Thiếu thông tin giao hàng bắt buộc." });
      }
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "Coupon này không dành cho bạn" });
      }
      ...
      const { subtotal, discount, shippingFee } = await this.calculateTotals(
        products,
        coupon,
        normalizedShippingDetails.city,
        session,
      ...
      console.error(
        `Error in create${paymentMethod.toUpperCase()}Order:`,
        error.message,
      );
  ...
  static async updateOrderStatus(orderId, status, reqUser) {
    ...
    if (status === "cancelled" && oldStatus !== "cancelled") {
      ...
      try {
        ...
        console.error(
          "Error restoring stock for returned order:",
          restoreError.message,
        );
        order.internalNotes =
          (order.internalNotes || "") +
          "\n[SYSTEM] Lỗi khi hoàn cộng tồn kho: " +
          restoreError.message;
      }
    }
    ...
    await emailQueue.add("order-status-update", {
      email:
        ...
        (await mongoose.model("User").findById(order.user))?.email,
