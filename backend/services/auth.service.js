...
const validatePhone = (phone) =>
  /^(0|\+84)(3[2-9]|5[25689]|7[06-9]|8[0-9]|9[0-9])\d{7}$/.test(phone);
...
export const signup = async ({
  ...
  try {
    ...
    if (
      ...
      (saveError.code === 11000 ||
        (saveError.name === "MongoServerError" && saveError.code === 11000))
    ) {
      const dupField = saveError.keyPattern
  ...
  try {
    ...
    console.error(
      "Failed to queue verification email in signup:",
      queueError.message,
    );
    try {
      ...
      console.error(
        "Fallback direct verification email also failed:",
        directError.message,
      );
    }
  }
  ...
  return {
    message:
      "Tài khoản đã tạo thành công. Vui lòng kiểm tra email để xác minh.",
    email: user.email,
  };
};
...
export const login = async (email, password) => {
  ...
  if (
    ...
    const error = new Error(
      "Vui lòng xác minh email trước khi đăng nhập. Kiểm tra hộp thư để nhận liên kết xác minh.",
    );
...
export const verifyEmail = async (token) => {
  ...
  try {
    ...
    if (wasRecentlyVerified) {
      return {
        message: "Tài khoản đã được xác minh thành công. Chào mừng bạn!",
        alreadyVerified: false,
      };
    }
  } catch (redisError) {
    console.warn(
      "Non-critical: Redis cache lookup failed in verifyEmail:",
      redisError.message,
    );
  }
  ...
  if (!user) {
    const error = new Error(
      "Liên kết xác minh không hợp lệ hoặc đã hết hạn. Vui lòng yêu cầu gửi lại.",
    );
    error.statusCode = 400;
    throw error;
  }
  ...
  try {
    ...
    console.warn(
      "Non-critical: Redis cache set failed in verifyEmail:",
      redisError.message,
    );
  }
...
export const resendVerificationEmail = async (email) => {
  ...
  try {
    ...
    console.error(
      "Failed to queue verification email in resend:",
      queueError.message,
    );
    try {
      ...
      await sendEmail(user.email, "Xác minh tài khoản Luxury Watch", finalHtml);
      console.info(
        "Fallback direct resend verification email sent successfully!",
      );
    } catch (directError) {
      console.error(
        "Fallback direct resend email also failed:",
        directError.message,
      );
    }
  }
  ...
  return {
    message:
      "Email xác minh đã được gửi. Vui lòng kiểm tra hộp thư (bao gồm thư mục spam).",
  };
};
...
export const forgotPassword = async (email) => {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    try {
      ...
      console.error(
        "Failed to queue reset password email:",
        queueError.message,
      );
      try {
        ...
        console.error(
          "Fallback direct reset password email also failed:",
          directError.message,
        );
      }
    }
...
export const updateProfile = async (userId, updates) => {
  ...
  if (Array.isArray(updateData.addressBook)) {
    ...
    if (
      normalizedAddressBook.length === 0 &&
    ...
    const defaultIndex = normalizedAddressBook.findIndex(
      (item) => item.isDefault,
    );
    if (defaultIndex > 0) {
    ...
    if (
      !updateData.defaultAddressId ||
      !normalizedAddressBook.some(
        (item) => item.id === String(updateData.defaultAddressId),
      )
    ) {
  ...
  if (updateData.defaultAddressId && Array.isArray(updateData.addressBook)) {
    ...
    const byIndex = updateData.addressBook.findIndex(
      (item) => item.id === byId,
    );
    if (byIndex >= 0) {
  ...
  if (
    Array.isArray(updateData.addressBook) &&
    updateData.addressBook.length > 0
  ) {
    const defaultAddress =
      updateData.addressBook.find((item) => item.isDefault) ||
      updateData.addressBook[0];
  ...
  const newEmail = updates.email
    ? String(updates.email).toLowerCase().trim()
    : null;
...
export const initiateAdminLogin = async (email, userAgent, ip) => {
  ...
  if (lockUntil && lockUntil > Date.now()) {
    ...
    const error = new Error(
      `Tài khoản bị khóa do nhập sai OTP quá nhiều lần. Vui lòng thử lại sau ${minutesLeft} phút.`,
    );
  ...
  if (cooldown) {
    const error = new Error(
      "Vui lòng đợi 60 giây giữa các lần yêu cầu mã xác thực.",
    );
    error.statusCode = 429;
    throw error;
  }
  ...
  try {
    ...
    console.error(
      "Non-critical: Email delivery failed, but proceeding to OTP step for UI testing.",
      error.message,
    );
  }
};
...
export const validateAdminOTP = async (email, otp, user) => {
  ...
  if (lockUntil && lockUntil > Date.now()) {
    const minutesLeft = Math.ceil((lockUntil - Date.now()) / 60000);
    const error = new Error(
      `Tài khoản vẫn đang bị khóa. Thử lại sau ${minutesLeft} phút.`,
    );
    error.statusCode = 429;
    throw error;
  }
  ...
  if (isMatch) {
    ...
    if (attempts >= 5) {
      ...
      const error = new Error(
        "Bạn đã nhập sai quá nhiều lần. Tài khoản bị khóa trong 30 phút.",
      );
      error.statusCode = 429;
      throw error;
    }
...
export const resendAdminOTP = async (email, userAgent, ip) => {
  ...
  try {
    ...
    console.error(
      "Non-critical: Resend email failed, but proceeding for UI testing.",
      error.message,
    );
  }
};
