...
if (isCronEnabled) {
  cron.schedule("0 3 * * *", async () => {
    ...
    if (result.modifiedCount > 0) {
      console.log(
        `[Cron] Cleared carts for ${result.modifiedCount} guest users inactive >7 days.`,
      );
    }
  });
}
...
let emailQueue = {
  add: async (name, payload) => {
    console.log(
      `[Cron Stub] queued ${name}`,
      payload && payload.email ? payload.email : "(no email)",
    );
...
if (isCronEnabled) {
  try {
    ...
    console.error(
      "[Cron] Redis connection failed, using stub queue:",
      err.message,
    );
  }
} else {
      ...
      try {
        ...
        if (updatedOrder) {
          ...
          if (updatedOrder.trackingEvents.length > 0) {
            const lastEvent =
              updatedOrder.trackingEvents[
                updatedOrder.trackingEvents.length - 1
              ];
            if (lastEvent.status === "cancelled") {
              lastEvent.message =
                "Hệ thống tự động hủy đơn Stripe hết hạn thanh toán.";
            }
          }
        ...
        console.log(
          "[Cron] Cancelled abandoned Stripe order " +
            order._id +
            " and restored stock via OrderService.",
        );
      } catch (err) {
...
if (isCronEnabled) {
  ...
  cron.schedule("*/30 * * * *", async () => {
    try {
      ...
      if (!geminiKey && !groqKey) {
        console.log(
          "[Cron AI] Chưa cấu hình GEMINI_API_KEY hoặc GROQ_API_KEY, bỏ qua auto-confirm.",
        );
        return;
      }
        ...
        model = async (prompt) => {
          const completion = await groq.chat.completions.create({
            ...
            messages: [
              {
                role: "system",
                content: 'Trả về JSON: {"isSpam":bool,"reason":"string"}',
              },
      ...
      if (confirmed > 0 || cancelled > 0) {
        console.log(
          `[Cron AI ${providerName}] Auto-processed ${pendingOrders.length} COD orders: ${confirmed} confirmed, ${cancelled} cancelled.`,
        );
      }
    } catch (err) {
  ...
  cron.schedule("0 2 * * *", async () => {
    try {
      ...
      if (deleted > 0)
        console.log(`[Cron AI] Cleaned ${deleted} spam accounts.`);
    } catch (err) {
