...
async function run(){
  try {
    ...
    const product = (pRes.data && pRes.data[0]) || (pRes.data && pRes.data.products && pRes.data.products[0]);
    if (!product) {
    ...
    const payload = {
      products: [{ _id: product._id || product.id, name: product.name, price: product.price, quantity: 1 }],
      couponCode: null,
      shippingDetails: {
        fullName: 'Test Buyer',
    ...
    const cookieHeader = csrfToken ? `csrfToken=${csrfToken}` : (setCookie && setCookie.length ? setCookie.map(c => c.split(';')[0]).join('; ') : '');

    console.log('CSRF token:', csrfToken);
    console.log('Cookie header:', cookieHeader);
    console.log('Creating COD checkout session...');
    const res = await axios.post(`${API_BASE}/payments/create-checkout-session`, payload, {
      headers: {
