...
const run = async () => {
  try {
    ...
    for (const p of samples) {
      ...
      console.log(`Price: ${p.price.toLocaleString('vi-VN')} đ | Cost Price: ${p.costPrice.toLocaleString('vi-VN')} đ`);
      ...
      const calculatedTotalStock = p.wristSizeOptions?.reduce((sum, opt) => sum + opt.stock, 0) || 0;
      console.log(`Summed Stock: ${calculatedTotalStock} | Declared Product Stock: ${p.stock}`);
