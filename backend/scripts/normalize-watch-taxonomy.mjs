...
const ensureCategory = async (category) => {
  ...
  if (!doc) {
    if (DRY_RUN) {
      log(`[dry-run] Would create category: ${category.name} (${category.slug})`);
      return null;
    }
  ...
  if (Object.keys(updates).length > 0) {
    if (DRY_RUN) log(`[dry-run] Would normalize category ${doc.slug}:`, updates);
    else {
...
const targetSlugForProduct = (product) => {
  if (String(product.type || '').toLowerCase() === 'smartwatch') return 'smartwatch';
...
const run = async () => {
  ...
  await mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/watch-ecommerce');
  ...
  if (movementCategories.length > 0) {
    ...
    if (DRY_RUN) log(`[dry-run] Would delete movement category ids: ${ids.join(', ')}`);
    else {
  ...
  for (const brand of brands) {
    const shouldDeactivate = TEST_BRAND_PATTERNS.some((pattern) => pattern.test(brand.name || ''));
    if (!shouldDeactivate) continue;
    const productCount = await Product.countDocuments({ brand: brand._id, deletedAt: null });
    if (productCount > 0) {
  ...
  log(`${DRY_RUN ? 'Would deactivate' : 'Deactivated'} brands: ${deactivatedBrands}`);

  const finalCategories = await Category.find({}).sort({ level: 1, name: 1 }).select('name slug level parentCategory').lean();
  log('Final category snapshot:');
  finalCategories.forEach((category) => log(`- ${category.name} (${category.slug}) level=${category.level}`));
