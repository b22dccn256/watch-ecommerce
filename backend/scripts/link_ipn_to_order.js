...
async function run(){
  ...
  if (!txn || !orderCode) { console.error('Usage: node link_ipn_to_order.js <TXN_ID> <ORDER_CODE>'); process.exit(1); }
  await connectDB();
  ...
  if (order.paymentStatus === 'paid') {
    ...
    order.trackingEvents.push({ status: 'confirmed', message: `Linked IPN ${txn} via CLI`, timestamp: new Date(), updatedBy: 'cli-reconcile' });
    await order.save();
    console.log('Order updated to paid:', order.orderCode);
  }

  await ProcessedIPN.create([{ provider: 'vnpay', transactionId: txn, orderCode, status: 'processed', payload: { linkedBy: 'cli', linkedAt: new Date() } }]);
