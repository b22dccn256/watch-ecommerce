...
async function run() {
  ...
  const products = await Product.find({
    brand: { $in: dbBrands.map((b) => b._id) },
  }).lean();

  const exportsDir = path.resolve(__dirname, "../exports");
  if (!fs.existsSync(exportsDir)) fs.mkdirSync(exportsDir, { recursive: true });
  fs.writeFileSync(
    path.join(exportsDir, "curated-products-backup-before.json"),
  ...
  for (const p of products) {
    ...
    if (!p.slug || typeof p.slug !== "string" || p.slug.trim() === "")
      upd.slug = slugify(p.name || p._id?.toString?.() || "");
    // sku
    if (!p.sku || String(p.sku).trim() === "")
      upd.sku = `SKU-${(p._id || "").toString().slice(-6)}`;
    // images
    ...
    if (!Array.isArray(p.variants) || p.variants.length === 0) {
      ...
      const variantSku =
        upd.sku || p.sku || `SKU-${(p._id || "").toString().slice(-6)}`;
      upd.variants = [
        {
          sku: variantSku,
          price: price || 0,
          title: p.name || "Standard",
          isDefault: true,
        },
      ];
    }
  ...
  fs.writeFileSync(
    path.join(exportsDir, "curated-products-normalize-report.json"),
    JSON.stringify(
      { totalProducts: products.length, updatedCount: applied.length, applied },
      null,
      2,
    ),
  );
