...
async function run() {
  ...
  const toCreate = curated.filter(
    (cb) =>
      !dbBrands.some((b) => (b.name || "").toLowerCase() === cb.toLowerCase()),
  );
  const toDeactivate = dbBrands.filter(
    (b) => !curatedLower.includes((b.name || "").toLowerCase()),
  );
  ...
  fs.writeFileSync(
    path.join(exportsDir, "curated-sync-backup-brands-before.json"),
  ...
  const allProducts = await Product.find(
    {},
    { _id: 1, name: 1, brand: 1, isActive: 1 },
  ).lean();
  const brandIdNameMap = {};
  for (const b of dbBrands)
    brandIdNameMap[b._id?.toString?.() || ""] = b.name || "";

  const productsOutside = [];
  for (const p of allProducts) {
    const brandId = p.brand
      ? p.brand._id
        ? p.brand._id.toString()
        : p.brand.toString
          ? p.brand.toString()
          : ""
      : "";
    const brandName =
      brandIdNameMap[brandId] || (p.brand && p.brand.name) || "";
    if (!brandName || !curatedLower.includes(brandName.toLowerCase()))
      productsOutside.push(p);
  }

  fs.writeFileSync(
    path.join(exportsDir, "curated-sync-backup-products-affected-before.json"),
    JSON.stringify(
      {
        count: productsOutside.length,
        products: productsOutside.slice(0, 500),
      },
      null,
      2,
    ),
  ...
  if (toCreate.length) {
    ...
    for (const r of res)
      createdBrands.push({ id: r._id.toString(), name: r.name });
  }

  // Deactivate non-curated brands
  const deactivateNames = toDeactivate.map((b) => b.name).filter(Boolean);
  const deactivateResult = await Brand.updateMany(
    { name: { $in: deactivateNames } },
  ...
  const result = {
    ...
    deactivatedBrandsCount:
      ...
      deactivateResult.nModified ||
      deactivateResult.modified ||
      0,
    productsArchivedCount:
      productUpdateResult.modifiedCount || productUpdateResult.nModified || 0,
  };

  fs.writeFileSync(
    path.join(exportsDir, "curated-sync-apply-result.json"),
    JSON.stringify({ result, timestamp: new Date().toISOString() }, null, 2),
  );

  console.log(
    "Apply complete. Report:",
    path.join(exportsDir, "curated-sync-apply-result.json"),
  );
