...
const STRAPS = ['Da', 'Thép không gỉ', 'Cao su', 'Vải NATO', 'Ceramic', 'Titanium'];
const CASES = ['Thép không gỉ', 'Titanium', 'Vàng 18K', 'Ceramic', 'Nhựa'];
const TYPES = ['automatic', 'mechanical', 'quartz', 'digital', 'solar', 'smartwatch'];
...
async function seed() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    updates.sizes = shuffled.slice(0, nSizes).sort((a, b) => parseInt(a) - parseInt(b));
