...
async function fetchCsrfToken() {
  ...
  if (csrfCookie)
    csrfToken = csrfCookie.split(";")[0].replace("csrfToken=", "");
}
...
async function login() {
  ...
  if (rtCookie)
    refreshToken = rtCookie.split(";")[0].replace("refreshToken=", "");
...
async function testOrderExport() {
  ...
  ok(
    "Content-Type is csv",
    ct.includes("text/csv") || ct.includes("application/octet-stream"),
    ct,
  );
}
...
async function testBanners() {
  ...
  ok(
    "POST /api/banners (image validation passed)",
    res.status !== 400,
    `got ${res.status}: ${JSON.stringify(created).slice(0, 120)}`,
  );
  ...
  if (bannerOk && bannerId) {
    ...
    ok(
      "Banner create returned ok but no _id (Cloudinary likely rejected dummy)",
      true,
    );
  }
}
...
async function testCategoriesAdmin() {
  ...
  const created = await res.json();
  ok(
    "POST /api/categories (create) returns 201",
    res.status === 201,
    `got ${res.status}: ${JSON.stringify(created).slice(0, 120)}`,
  );
  ...
  if (categoryId) {
    ...
    ok(
      "PUT /api/categories/:id returns 200",
      res.status === 200,
      `got ${res.status}`,
    );
    ...
    ok(
      "DELETE /api/categories/:id returns 200",
      res.status === 200,
      `got ${res.status}`,
    );
  } else {
...
async function testCoupons() {
  ...
  const created = await res.json();
  ok(
    "POST /api/coupons returns 201",
    res.status === 201,
    `got ${res.status}: ${JSON.stringify(created).slice(0, 100)}`,
  );
  ...
  if (couponId) {
    ...
    ok(
      ...
      res.status === 200,
      `got ${res.status}: ${JSON.stringify(validated).slice(0, 100)}`,
    );
    ok(
      "Coupon validation message",
      validated.message === "Coupon is valid",
      JSON.stringify(validated).slice(0, 100),
    );
  }
...
async function testProducts() {
  ...
  ok(
    ...
    res.status === 200 && (json.products || Array.isArray(json)),
    `got ${res.status}`,
  );
}
...
async function testUsers() {
  ...
  ok(
    "GET /api/auth/users returns 200",
    res.status === 200,
    `got ${res.status}`,
  );
  const json = await res.json();
...
async function testUnauthorized() {
  ...
  ok(
    "GET /api/orders/export without auth returns 401",
    res.status === 401,
    `got ${res.status}`,
  );
}
