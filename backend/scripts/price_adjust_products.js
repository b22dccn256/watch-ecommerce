...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (args[i] === "--round" && args[i + 1])
      opts.roundTo = parseInt(args[++i], 10);
    if (args[i] === "--percent" && args[i + 1])
      opts.percent = parseFloat(args[++i]);
    if (args[i] === "--preview" && args[i + 1])
...
async function run() {
  ...
  for (const p of products) {
    ...
    if (opts.percent !== 0)
      newPrice = Math.round(newPrice * (1 + opts.percent / 100));
  ...
  const previewFile = path.join(
    outDir,
    `price_changes_preview_${Date.now()}.csv`,
  );
  ...
  for (let i = 0; i < Math.min(opts.preview, changes.length); i++) {
    const c = changes[i];
    w.write(
      `"${c._id}","${(c.slug || "").replace(/"/g, '""')}","${(c.name || "").replace(/"/g, '""')}",${c.oldPrice},${c.newPrice},${c.newPrice - c.oldPrice}\n`,
    );
  }
  w.end();
  ...
  if (opts.dryRun) {
    console.log(
      "Dry-run mode; no DB changes applied. Re-run with --no-dry-run to apply.",
    );
    process.exit(0);
  }
  ...
  for (let i = 0; i < changes.length; i += BATCH) {
    ...
    const bulk = batch.map((c) => ({
      updateOne: {
        filter: { _id: c._id },
        update: { $set: { price: c.newPrice } },
      },
    }));
    try {
      const res = await Product.bulkWrite(bulk, { ordered: false });
      console.log(
        `Applied batch ${i / BATCH + 1}:`,
