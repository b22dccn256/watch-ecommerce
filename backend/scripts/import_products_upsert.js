...
async function run() {
  ...
  try {
    ...
    console.log(
      "No brands collection found or failed to load brands; brand slugs will be preserved as-is.",
    );
  }
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      console.error("Skipping invalid JSON at line", lines);
      continue;
    }
  ...
  console.log(
    `Prepared ${ops.length} upsert operations (from ${lines} lines).`,
  );

  // preview
  if (opts.preview > 0) {
    console.log(
      `Preview first ${Math.min(opts.preview, ops.length)} operations:`,
    );
    for (let i = 0; i < Math.min(opts.preview, ops.length); i++) {
      ...
      console.log(
        `#${i + 1} slug=${s} name="${String(name).slice(0, 60)}" price=${price}`,
      );
    }
  }
  ...
  if (opts.dryRun) {
    ...
    for (const op of ops) {
      ...
      if (existingSet.has(s)) wouldUpdate++;
      else wouldInsert++;
    }
    console.log(
      `Dry-run: would update ${wouldUpdate} products, insert ${wouldInsert} new products.`,
    );
    process.exit(0);
  }
  ...
  for (let i = 0; i < ops.length; i += B) {
    ...
    try {
      ...
      console.log(
        `Batch ${i / B + 1}: result`,
        res.result ? JSON.stringify(res.result) : res,
      );
    } catch (err) {
