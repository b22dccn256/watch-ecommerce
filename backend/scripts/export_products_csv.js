...
async function run() {
  ...
  const headers = ['_id','slug','name','price','costPrice','stock','brand','collectionName'];
  const rows = [headers.join(',')];
  for (const p of products) {
    const row = [p._id, p.slug || '', p.name || '', p.price || '', p.costPrice || '', p.stock || '', p.brand || '', p.collectionName || '']
