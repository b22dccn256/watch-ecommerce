...
const fixPricesBrand = async () => {
  try {
    ...
    for (const [brandName, { min, max }] of Object.entries(BRAND_RANGES)) {
      ...
      const result = await Product.updateMany({ brand: brandName }, [
        {
          $set: {
            price: {
              $toInt: {
      ...
      if (result.modifiedCount > 0) {
        ...
        console.log(
          `   Range: ${min.toLocaleString("vi-VN")} - ${max.toLocaleString("vi-VN")} ₫`,
        );
        console.log(
          `   Sample before: ${Math.round(priceBefore).toLocaleString("vi-VN")} ₫`,
        );
        console.log(
          `   Sample after: ${Math.round(priceAfter).toLocaleString("vi-VN")} ₫\n`,
        );
