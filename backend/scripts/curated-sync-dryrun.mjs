...
async function run() {
  ...
  const dbBrands = await Brand.find(
    {},
    { name: 1, _id: 1, isActive: 1 },
  ).lean();
  const brandNames = dbBrands.map((b) => ({
    id: b._id.toString(),
    name: b.name,
    isActive: b.isActive,
  }));
  ...
  for (const cb of curated) {
    const found = dbBrands.find(
      (b) => b.name && b.name.toLowerCase() === cb.toLowerCase(),
    );
    if (found)
      toKeep.push({
  ...
  for (const p of allProducts) {
    ...
    if (!brandName) {
      // brand-less product
      productImpact.outsideCurated.push({
        id: p._id.toString(),
        name: p.name,
        brand: null,
        isActive: !!p.isActive,
    ...
    if (!curatedLower.includes(brandName.toLowerCase())) {
      ...
      productImpact.curatedButInactive.push({
        id: p._id.toString(),
        name: p.name,
        brand: brandName,
        isActive: !!p.isActive,
  ...
  fs.writeFileSync(
    outPath,
    JSON.stringify({ report, productImpact: productImpact }, null, 2),
    "utf8",
  );
