...
async function checkFields() {
  ...
  const countWithCategoryId = await Product.countDocuments({ categoryId: { $exists: true, $ne: null } });
  const countWithLegacyCategory = await Product.collection.countDocuments({ category: { $exists: true, $ne: null } });
