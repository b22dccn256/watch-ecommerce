...
const randomPrice = (min, max) =>
  Math.floor(Math.floor(Math.random() * (max - min) + min) / 10000) * 10000;
const randomDateWithinDays = (days) =>
  new Date(Date.now() - Math.floor(Math.random() * 1000 * 60 * 60 * 24 * days));
  ...
  types.forEach((type) => {
    for (let i = 0; i < 100; i++) {
      ...
      const p = {
        ...
        type: type,
        slug: slugifyText(
          baseName + "-" + Math.random().toString(36).substring(7),
        ),
        createdAt: randomDateWithinDays(30),
        colors: [getRandom(COLORS), getRandom(COLORS)].filter(
          (v, i, a) => a.indexOf(v) === i,
        ),
        sizes: [getRandom(SIZES), getRandom(SIZES)].filter(
          (v, i, a) => a.indexOf(v) === i,
        ),
        specs: {
...
const seed = async () => {
  try {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log(
        "Development environment detected. Clearing existing data...",
      );
      ...
      const allBrandNames = Array.from(
        new Set(Object.values(brandMapping).flat()),
      );
      ...
      console.log(
        "PRODUCTION ENV DETECTED. Proceeding with caution. No DB clearing!",
      );
    }
