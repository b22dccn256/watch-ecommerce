...
const suggestions = { timestamp: new Date().toISOString(), duplicateNameGroups: [] };
...
for (const g of duplicateNames) {
  ...
  suggestions.duplicateNameGroups.push({ nameKey: g._id, keep: { id: keep._id, score: keep._score }, remove });
}
...
for (const g of duplicateSKUs) {
  ...
  const keep = docs[0];
  const remove = docs.slice(1).map(d=> ({ id: d._id, score: d._score }));
  suggestions.duplicateSKUGroups.push({ skuKey: g._id, keep: { id: keep._id, score: keep._score }, remove });
}
...
const missingCat = await Product.find({ $or: [{ categoryId: { $exists: false } }, { categoryId: null }] }).limit(200).select('name _id tags').lean();
suggestions.assignCategorySuggestions = [];
for (const p of missingCat) {
  const name = (p.name || '').toLowerCase();
  let matched = null;
  for (const c of categories) {
    if (name.includes((c.name || '').toLowerCase())) { matched = c; break; }
  }
  suggestions.assignCategorySuggestions.push({ id: p._id, name: p.name, suggestedCategory: matched ? { id: matched._id, name: matched.name } : null });
}

const outPath = path.join(process.cwd(), 'backend', 'exports', 'cleanup-suggestions.json');
