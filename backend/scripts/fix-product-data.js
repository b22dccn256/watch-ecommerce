  ...
  try {
    ...
    for (const product of products) {
      ...
      if (
        ...
        if (Math.random() > 0.7)
          product.colors.push(
            getRandom(COLORS.filter((c) => !product.colors.includes(c))),
          );
        ...
        if (Math.random() > 0.7)
          product.sizes.push(
            getRandom(SIZES.filter((s) => !product.sizes.includes(s))),
          );
    ...
    console.log(
      `Successfully updated ${updatedCount} products with colors and sizes.`,
    );
    process.exit(0);
  } catch (err) {
