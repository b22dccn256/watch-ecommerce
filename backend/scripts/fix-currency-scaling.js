...
const fixCurrencyScalingComprehensive = async () => {
    try {
        ...
        if (statsBefore.length > 0) {
            const { minPrice, maxPrice, avgPrice, count, scaledCount } = statsBefore[0];
            ...
            if (maxPrice > REASONABLE_MAX) {
                console.log(`   ⚠️  Max price ${Math.round(maxPrice).toLocaleString("vi-VN")} exceeds realistic max`);
            }
        }

        // Sample products before fix
        const samples = await Product.find({ price: { $gt: OBVIOUS_SCALE_THRESHOLD } }).limit(3).lean();
        if (samples.length > 0) {
            console.log("\n📋 Sample products BEFORE fix:");
            samples.forEach((p, i) => {
                console.log(`   ${i + 1}. ${p.name}`);
                console.log(`      Price: ${Math.round(p.price).toLocaleString("vi-VN")} ₫`);
            });
        }
        ...
        if (statsAfter.length > 0) {
            ...
            if (maxPrice <= REASONABLE_MAX && minPrice >= 100000) {
                ...
                console.log(`   ⚠️  Warning: Max still high at ${Math.round(maxPrice).toLocaleString("vi-VN")}`);
            }
        }

        // Sample products after fix
        const samplesAfter = await Product.find({ brand: samples[0]?.brand }).limit(3).lean();
