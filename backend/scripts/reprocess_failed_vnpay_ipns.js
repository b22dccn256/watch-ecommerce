...
async function run(){
  await connectDB();
  const recs = await ProcessedIPN.find({ provider: 'vnpay', status: 'failed' }).sort({ createdAt: -1 }).limit(50).lean();
  if (!recs || recs.length === 0) { console.log('No failed vnpay records found.'); process.exit(0); }

  const backupPath = path.join(__dirname, '..', 'exports', `failed_vnpay_ipns_backup_${Date.now()}.json`);
  ...
  for (const r of recs) {
    ...
    console.log('---');
    console.log('orderCode:', r.orderCode, 'transactionId:', r.transactionId, 'signatureOK:', ok);
    if (!ok) {
    ...
    try {
      const res = await processIPN({ provider: 'vnpay', transactionId: r.transactionId, orderCode: r.orderCode, isSuccess: payload.vnp_ResponseCode === '00', payload });
