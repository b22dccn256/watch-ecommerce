...
function numeric(v){ return typeof v==='number' ? v : (v ? Number(v) : 0); }
...
async function run(){
  ...
  for(const p of products){
    ...
    let brandKey = '';
    if (p.brand && typeof p.brand === 'string') brandKey = p.brand.toLowerCase();
    ...
    if (price < minAllowed || (maxAllowed !== null && price > maxAllowed)){
      issues.push({ _id: p._id, slug: p.slug||'', name: p.name||'', price, cost, minAllowed, maxAllowed });
    }
  }
  ...
  for(const it of issues){
    w.write(`"${it._id}","${it.slug.replace(/"/g,'""')}","${(it.name||'').replace(/"/g,'""')}",${it.price},${it.cost},${it.minAllowed},${it.maxAllowed===null?'':it.maxAllowed}\n`);
  }
  w.end();

  console.log(`Checked ${products.length} products — issues found: ${issues.length}. Report: ${outFile}`);
