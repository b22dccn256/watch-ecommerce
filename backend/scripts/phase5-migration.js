...
const phase5Migration = async () => {
  try {
    ...
    console.log(`  - MongoDB Session Support: ${mongoose.connection.supportsSession ? "✓" : "✗"}`);
    console.log(`  - Replica Set: ${mongoose.connection.getClient().topology?.replTopology ? "✓" : "✗ (local dev)"}`);
    ...
    const db = mongoose.connection.getClient().db(process.env.MONGO_URI.split("/").pop()?.split("?")[0] || "watchstore_db");
    ...
    try {
      ...
      console.log("  - Products collection size: " + (stats[0].size / 1024 / 1024).toFixed(2) + " MB");
      console.log("  - Orders collection size: " + (stats[1].size / 1024 / 1024).toFixed(2) + " MB");
      console.log("  - Users collection size: " + (stats[2].size / 1024 / 1024).toFixed(2) + " MB");
    } catch (err) {
      console.log("  - Collection stats unavailable (expected for local MongoDB)");
    }
