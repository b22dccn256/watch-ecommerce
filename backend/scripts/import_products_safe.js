...
function parseArgs() {
  ...
  const opts = { file: 'exports/products_fixed_500.txt', dryRun: true, preview: 10 };
...
async function run() {
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      invalid.push({ line: lineNo, error: 'Invalid JSON', detail: err.message, raw: line.slice(0,200) });
      continue;
    }
    ...
    try {
      ...
      if (typeof docForValidate.brand === 'string' && !/^[a-fA-F0-9]{24}$/.test(docForValidate.brand)) {
        ...
        docForValidate.customAttributes.push({ name: 'originalBrand', value: String(docForValidate.brand) });
        delete docForValidate.brand;
      }
      if (docForValidate._id && !/^[a-fA-F0-9]{24}$/.test(String(docForValidate._id))) {
      ...
      if (docForValidate.type) docForValidate.type = String(docForValidate.type).toLowerCase();
      ...
      invalid.push({ line: lineNo, error: 'ValidationError', detail: err.message, fields: err.errors ? Object.keys(err.errors) : undefined, raw: doc });
    }
  }

  console.log(`Processed ${lineNo} lines. Valid: ${validDocs.length}, Invalid: ${invalid.length}`);

  const exportsDir = path.join(__dirname, '..', 'exports');
  if (!fs.existsSync(exportsDir)) fs.mkdirSync(exportsDir, { recursive: true });

  const invalidFile = path.join(exportsDir, `products_invalid_${Date.now()}.jsonl`);
  const validPreviewFile = path.join(exportsDir, `products_valid_preview_${Date.now()}.csv`);
  ...
  if (invalid.length > 0) {
    ...
    invalid.slice(0,5).forEach((it, idx) => {
      console.log(`#${idx+1} line=${it.line} error=${it.error} detail=${it.detail}`);
    });
  }
  ...
  if (previewCount > 0) {
    const headers = ['name','price','costPrice','stock','brand','collectionName','slug'];
    const w2 = fs.createWriteStream(validPreviewFile, { flags: 'w' });
    w2.write(headers.join(',') + '\n');
    for (let i = 0; i < previewCount; i++) {
      const d = validDocs[i].doc;
      const row = [d.name, d.price, d.costPrice, d.stock, d.brand, d.collectionName, d.slug].map(v => '"'+String(v).replace(/"/g,'""')+'"').join(',');
      w2.write(row + '\n');
    }
    w2.end();
    console.log(`Wrote ${previewCount} valid preview rows to ${validPreviewFile}`);
  }

  if (opts.dryRun) {
    console.log('Dry-run enabled — no database changes made. If output looks good, re-run with `--no-dry-run` to import.');
