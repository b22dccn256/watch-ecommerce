...
const run = async () => {
	...
	for await (const product of cursor) {
		const nextOldSlugs = Array.isArray(product.oldSlugs) ? [...new Set(product.oldSlugs.filter(Boolean))] : [];
		const baseSlug = slugifyProductName(product.name) || "product";
		const nextSlug = product.slug ? await ensureUniqueSlug(product.slug, product._id) : await ensureUniqueSlug(baseSlug, product._id);
		...
		const nextToken = product.slugToken || await ensureUniqueToken(product._id);
		const tokenChanged = product.slugToken !== nextToken;

		if (slugChanged || tokenChanged || nextOldSlugs.length !== (product.oldSlugs || []).length) {
