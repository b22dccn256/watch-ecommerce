...
async function main() {
  ...
  const testOrders = await ordersCol
    .find(testOrderFilter)
    .sort({ createdAt: -1 })
    .toArray();
  ...
  if (dupeAgg.length > 0) {
    ...
    for (const group of dupeAgg) {
      ...
      console.log(
        `  ${group._id}: ${group.count} copies → keeping newest, removing ${toRemove.length}`,
      );
    }
  } else {
  ...
  const allIdsToDelete = [
    ...new Set([...testIds, ...dupeIdsToRemove].map(String)),
  ];

  // Convert back to ObjectIds
  const objectIdsToDelete = allIdsToDelete
    .map((id) => {
      try {
