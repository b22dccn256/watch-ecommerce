...
let MONGO =
  process.env.MONGO_URI || process.env.MONGOURL || process.env.MONGODB_URI;
if (!MONGO) {
  ...
  if (!MONGO) {
    console.error(
      "MONGO_URI not set. Set env or ensure backend/.env contains MONGO_URI",
    );
    process.exit(1);
  }
}
...
async function run() {
  ...
  report.duplicateSKUs = await Product.aggregate([
    ...
    console.error("dup sku aggregate failed", e.message);
    return [];
  });
  ...
  report.missingImage = await Product.find({
    $or: [
      { image: { $exists: false } },
      { image: "" },
    ...
    .limit(50)
    .select("name _id image images")
    .lean();

  report.softDeletedCount = await Product.countDocuments({
    deletedAt: { $ne: null },
  });
  report.activeWithDeletedAt = await Product.find({
    isActive: true,
    deletedAt: { $ne: null },
  })
    .limit(50)
    .select("name _id deletedAt isActive")
    .lean();
  report.inactiveNotDeleted = await Product.find({
    isActive: false,
    deletedAt: null,
  })
    .limit(50)
    .select("name _id isActive deletedAt")
    .lean();

  report.zeroStockActive = await Product.find({
    stock: { $lte: 0 },
    isActive: true,
  })
    .limit(50)
    .select("name _id stock isActive")
    .lean();

  report.missingBrand = await Product.find({
    $or: [{ brand: { $exists: false } }, { brand: null }],
  })
    .limit(50)
    .select("name _id brand")
    .lean();
  report.missingCategory = await Product.find({
  ...
  report.negativePrice = await Product.find({
    $or: [{ price: { $lt: 0 } }, { price: { $exists: false } }],
  })
    .limit(50)
  ...
  report.expiredActiveCoupons = await Coupon.find({
    ...
    .lean();
  report.couponsOverused = await Coupon.find({
    $expr: { $gt: ["$usedCount", "$maxUses"] },
  })
    .limit(50)
  ...
  console.log(
    "Products with missing image (sample):",
    report.missingImage.length,
  );
  ...
  console.log(
    "Products with zero stock but active (sample):",
    report.zeroStockActive.length,
  );
  console.log("Products missing brand (sample):", report.missingBrand.length);
  console.log(
    "Products missing category (sample):",
    report.missingCategory.length,
  );
  console.log("Duplicate slugs (sample):", report.duplicateSlugs.length);
  console.log(
    "Negative or missing prices (sample):",
    report.negativePrice.length,
  );
  console.log("Brands count:", report.brandCount);
  console.log(
    "Brands without products (sample):",
    report.brandsWithoutProducts.length,
  );
  console.log("Coupons total:", report.couponCount);
  console.log(
    "Expired but active coupons (sample):",
    report.expiredActiveCoupons.length,
  );
  console.log(
    "Coupons with usedCount > maxUses (sample):",
    report.couponsOverused.length,
  );
