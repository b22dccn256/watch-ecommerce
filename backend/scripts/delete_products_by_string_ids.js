...
async function run(){
  ...
  for await (const line of rl) {
    ...
    if (!obj._id) continue;
    const idVal = (typeof obj._id === 'string') ? obj._id : (obj._id && obj._id.$oid ? obj._id.$oid : String(obj._id));
    ids.push(idVal);
  }
  ...
  if (matched === 0) { console.log('No matching docs to delete.'); process.exit(0); }
