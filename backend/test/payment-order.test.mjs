...
const createTestProduct = async ({ name, price }) => {
  const category = await Category.create({
    name: `${name} Category`,
    slug: `${name.toLowerCase().replace(/\s+/g, '-')}-cat-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
  });
  const brand = await Brand.create({
    name: `${name} Brand ${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
  });

  const product = await Product.create({
    name,
    description: `Auto-generated product for ${name}`,
    price,
    costPrice: Math.max(1, Math.round(price * 0.6)),
    stock: 10,
...
test('order-totals: calculates subtotal from products', async () => {
  const fixture = await createTestProduct({ name: 'Test Watch for Totals', price: 5000000 });
  testCategory = fixture.category;
  testBrand = fixture.brand;
...
test('order-totals: applies coupon discount', async () => {
  const fixture = await createTestProduct({ name: 'Discount Watch', price: 5000000 });
  testCategory = fixture.category;
  testBrand = fixture.brand;
  testProduct = fixture.product;
  ...
  assert.equal(result.shippingFee, 30000, '4.5M after discount still pays big-city shipping');
  assert.equal(result.total, 4530000, '5M - 500k + 30k = 4.53M');
...
test('order-totals: big city shipping (HN - 30k)', async () => {
  const fixture = await createTestProduct({ name: 'HN Shipping Watch', price: 5000000 });
  const products = [{ _id: fixture.product._id, quantity: 1 }];
  ...
  await Product.findByIdAndDelete(fixture.product._id);
  await Category.findByIdAndDelete(fixture.category._id);
  await Brand.findByIdAndDelete(fixture.brand._id);
});

test('order-totals: big city shipping (HCM - 30k)', async () => {
  const fixture = await createTestProduct({ name: 'HCM Shipping Watch', price: 5000000 });
  const products = [{ _id: fixture.product._id, quantity: 1 }];
...
test('order-totals: other province shipping (50k)', async () => {
  const fixture = await createTestProduct({ name: 'Province Shipping Watch', price: 5000000 });
  const products = [{ _id: fixture.product._id, quantity: 1 }];
  const coupon = { isActive: true, type: 'fixed', discountValue: 4500000 };
...
test('order-totals: empty city defaults to big city (30k)', async () => {
  const fixture = await createTestProduct({ name: 'Empty City Shipping Watch', price: 5000000 });
  const products = [{ _id: fixture.product._id, quantity: 1 }];
  const coupon = { isActive: true, type: 'fixed', discountValue: 4500000 };
...
test('order-totals: free shipping threshold at 5M', async () => {
  const fixture = await createTestProduct({ name: 'Threshold Watch', price: 5000000 });
  const thresholdProduct = fixture.product;

  // Product at 5M, no coupon → free ship
  const products1 = [{ _id: thresholdProduct._id, quantity: 1 }];
  ...
  const products2 = [{ _id: thresholdProduct._id, quantity: 2 }];
  ...
  await Product.findByIdAndDelete(thresholdProduct._id);
  await Category.findByIdAndDelete(fixture.category._id);
  await Brand.findByIdAndDelete(fixture.brand._id);
});
...
test('shipping: exactly at free ship threshold', async () => {
  const { category, brand, product: cheapProduct } = await createTestProduct({ name: 'Cheap Watch', price: 5000000 });
...
test('shipping: just below free ship threshold', async () => {
  const { category, brand, product: cheapProduct } = await createTestProduct({ name: 'Almost Free Watch', price: 4999000 });
