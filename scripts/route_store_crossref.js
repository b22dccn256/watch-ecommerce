...
function extractBackendRoutes(files) {
  const routes = [];
  const routerRegex = /router\.(get|post|put|patch|delete)\s*\(\s*([`'\"])(.*?)\2/gm;
  files.forEach((file) => {
    const src = fs.readFileSync(file, 'utf8');
    let m;
    while ((m = routerRegex.exec(src)) !== null) {
      const method = m[1].toUpperCase();
      const routePath = m[3];
      routes.push({ file: path.relative(process.cwd(), file), method, route: routePath });
    }
  });
  return routes;
}
...
function mapAppUses(serverFiles) {
  ...
  const mapping = {};
  const useRegex = /app\.use\s*\(\s*([`'\"])(.*?)\1\s*,\s*require\(\s*([`'\"])(.*?routes\/?(.*?))\3\s*\)\s*\)/gm;
...
function main() {
  ...
  const report = usages.map((u) => {
    const entry = backendWithPrefix.find((b) => {
      const candidate = `${b.method} ${path.posix.join(b.prefix || '', b.route)}`.replace(/\\/g, '/');
