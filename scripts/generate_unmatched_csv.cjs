...
const out = path.join(
  ...
  "..",
  "outputs",
  "unmatched_endpoints_prioritized.csv",
);
...
function main() {
  ...
  const rows = Object.entries(unmatched)
    .map(([k, v]) => ({
      endpoint: k,
      count: v.count,
      exampleFiles: Array.from(v.examples).slice(0, 3).join(" | "),
    }))
  ...
  const lines = rows.map(
    (r) =>
      `${r.count},"${r.endpoint.split(" ")[0]}","${r.endpoint.split(" ").slice(1).join(" ")}","${r.exampleFiles}"`,
  );
