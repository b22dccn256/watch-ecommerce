...
const out = path.join(__dirname, '..', 'outputs', 'route_store_crossref_fixed.json');

function load() { return JSON.parse(fs.readFileSync(input, 'utf8')); }
function normalize(p) { return (p||'').replace(/:\w+/g, ':param').replace(/\?[^\s]*/g,'').replace(/\/+/g,'/'); }
...
function main() {
  ...
  for (const rep of report) {
    ...
    for (const c of candidates) {
      const key = `${method} ${c}`;
      if (map.has(key)) { found = map.get(key); break; }
    }
    if (found) {
      rep.matched = {
