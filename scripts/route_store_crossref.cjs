...
function extractBackendRoutes(files) {
  ...
  const routerRegex =
    /router\.(get|post|put|patch|delete)\s*\(\s*([`'\"])(.*?)\2\s*(?:,\s*([\s\S]*?)\))?/gm;
  files.forEach((file) => {
    const src = fs.readFileSync(file, "utf8");
    let m;
    while ((m = routerRegex.exec(src)) !== null) {
      ...
      const hasManagement =
        /\bmanagementRoute\b/.test(args) || /\bmanagementOnly\b/.test(args);
      routes.push({
        file: path.relative(process.cwd(), file),
        method,
        route: routePath,
...
function mapControllerExports(controllersDir) {
  ...
  const files = walk(controllersDir).filter(
  ...
  files.forEach((f) => {
    ...
    while ((m = namedRegex.exec(src)) !== null) {
      m[1]
        ...
        .map((s) => s.trim())
        .forEach((name) => {
          if (name) exports.add(name.replace(/as\s+.+/, "").trim());
        });
    }
...
function findControllerImportsInRouter(routerFile) {
  ...
  const namedRegex =
    /import\s*\{([^}]+)\}\s*from\s*([`'\"])(.*?controllers\/(.*?))\2/gm;
  let m;
  while ((m = namedRegex.exec(src)) !== null) {
    const names = m[1]
      .split(",")
  ...
  const defRegex =
    /import\s+([A-Za-z0-9_$]+)\s+from\s*([`'\"])(.*?controllers\/(.*?))\2/gm;
...
function main() {
  ...
  const backendWithPrefix = backendRoutes.map((r) => {
    ...
    try {
      importedControllers = findControllerImportsInRouter(absRouterPath);
    } catch (err) {
      importedControllers = [];
    }
  ...
  const report = usages.map((u) => {
    ...
    if (frontendBase) {
      ...
      const b = frontendBase.startsWith("/")
        ? frontendBase
        : `/${frontendBase}`;
    ...
    const entry = backendWithPrefix.find((b) => {
      ...
      return usageCandidates.some(
        (uc) => candidate === uc || candidate.endsWith(uc),
      );
    });
