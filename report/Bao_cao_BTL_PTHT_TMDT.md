...
4. **Quản lý Giỏ hàng ảo:** Hỗ trợ khách hàng gom nhóm và điều chỉnh số lượng các sản phẩm muốn mua trước khi thanh toán. Yêu cầu khách hàng bắt buộc phải đăng nhập để thêm sản phẩm vào giỏ hàng; dữ liệu giỏ hàng được lưu trực tiếp vào CSDL để đồng bộ xuyên suốt trên nhiều thiết bị.
...
### 2.2. Cơ sở dữ liệu (Database Design)
...
**2.2.1 Danh sách các Collection cốt lõi**
...
**2.2.2 Sơ đồ quan hệ thực thể (ERD) - Mức logic**
...
### 2.3. Ngôn ngữ lập trình và Nền tảng công nghệ
...
**2.3.1 Phía Máy khách (Front-End)**
...
Biểu đồ Use Case mô tả trực quan các tương tác giữa người dùng (Actor) và hệ thống (System), định hình rõ các chức năng cơ bản theo nghiệp vụ Thương mại điện tử.

**a. Các tác nhân (Actors) chính:**
- **Khách hàng (Customer):** Người dùng có nhu cầu mua sắm, tìm kiếm sản phẩm. (Bao gồm cả Guest chưa đăng nhập và User đã đăng nhập).
- **Quản trị viên (Admin/Staff):** Người điều hành, quản lý nội dung, sản phẩm, đơn hàng và các chiến dịch khuyến mãi.
- **Hệ thống thứ 3 (External Systems):** Cổng thanh toán (VNPay, Stripe), Dịch vụ Email (Nodemailer), AI (Gemini/Llama).

**b. Các Use Case chính:**
- **Nhóm Khách hàng:** Đăng nhập/Đăng ký, Tìm kiếm & Lọc sản phẩm, Xem chi tiết sản phẩm, Quản lý Giỏ hàng, Đặt hàng & Thanh toán, Xem lịch sử đơn hàng, Chat với AI.
- **Nhóm Quản trị viên:** Quản lý User (Khóa/Mở khóa), Quản lý Sản phẩm (Thêm/Sửa/Xóa/Ẩn), Quản lý Đơn hàng (Cập nhật trạng thái), Quản lý Mã giảm giá, Xem Thống kê doanh thu.

> *(Sinh viên chèn ảnh Sơ đồ Use Case tổng quát vào đây)*

### 3.2. Phân tích thiết kế CSDL
Dựa trên kiến trúc NoSQL MongoDB, hệ thống được thiết kế với chính xác 23 collection. Dưới đây là 3 collection quan trọng nhất đóng vai trò "xương sống" cho toàn bộ nền tảng:
...
> **Lưu ý:** Danh sách sản phẩm yêu thích (Wishlist) không được nhúng (embed) vào `users` mà được tách thành collection `wishlists` độc lập (quan hệ 1-1 qua trường `user`). Khách chưa đăng nhập sẽ lưu tạm trong `localStorage` và tự động gộp vào `wishlists` khi đăng nhập.
...
### 3.3. Phân tích thiết kế chức năng

**3.3.1. Phân hệ Giỏ hàng và Đặt hàng (Cart & Checkout)**

**a. Luồng xử lý Giỏ hàng:**
- **Mục đích:** Quản lý giỏ hàng thống nhất và đồng bộ trên mọi thiết bị. Yêu cầu khách hàng bắt buộc phải đăng nhập mới được phép thao tác thêm sản phẩm vào giỏ hàng.
- **Logic xử lý:**
  - Mọi thao tác thêm, sửa, xóa sản phẩm trong giỏ hàng đều được kiểm tra xác thực (Authentication) và lưu trực tiếp qua API vào CSDL (thuộc Collection `users`).
  - Nếu người dùng chưa đăng nhập bấm "Thêm vào giỏ hàng", hệ thống sẽ tự động hiển thị thông báo yêu cầu đăng nhập trước khi tiếp tục.
...
**3.3.2. Phân hệ Quản lý Tồn kho (Inventory Management)**
...
**3.3.3. Phân hệ Trợ lý ảo AI (AI Chatbot Automation)**
...
**3.3.4. Phân hệ Xử lý Email và Tác vụ ngầm (Background Jobs)**
...
**3.3.5. Phân hệ Tự động hóa Bảo mật (AI Spam Detection)**
...
- **Logic xử lý:**
  ...
  - Nếu AI kết luận là SPAM (Độ tin cậy > 90%), hệ thống tự động đổi trạng thái đơn thành Cancelled, ghi log `[AI] Tự động hủy`, và hoàn trả tồn kho ngay lập tức. Ngược lại, nếu đơn hàng có vẻ hợp lệ, hệ thống sẽ giữ nguyên trạng thái `Pending` (Chờ xác nhận) để chờ Admin/Nhân viên kiểm duyệt và xác nhận thủ công trước khi xuất kho.

**3.3.6. Phân hệ Xác thực và Bảo mật (Auth & Security)**
...
**3.3.7. Phân hệ Quản lý Khuyến mãi (Coupon & Campaign)**
...
- **Logic xử lý 5 điều kiện bảo mật (Chia làm 2 tầng):** 
  - **Tầng 1 (Validate lúc nhập mã):** Khi khách nhập `COUPON`, API controller kiểm tra 4 điều kiện bắt buộc: (1) Mã đang kích hoạt (`isActive: true`); (2) Chưa hết hạn (`expirationDate > now`); (3) Còn lượt sử dụng (`usedCount < maxUses`); (4) Quyền sở hữu (nếu mã được cấp riêng cho một `userId` cụ thể, tài khoản đang đăng nhập phải khớp).
  - **Tầng 2 (Validate lúc thanh toán):** Khi tạo đơn hàng (Order Service), hệ thống tiếp tục kiểm tra điều kiện thứ (5): Tổng giá trị sản phẩm (`subtotal`) phải lớn hơn hoặc bằng giá trị tối thiểu của mã (`minOrderAmount`). 
  - *Lý do tách tầng:* Điều kiện (5) bắt buộc phải kiểm tra ở khâu cuối cùng vì giá trị đơn hàng có thể bị thay đổi sau khi hệ thống tự động áp dụng các chương trình Flash Sale (Campaign) chồng lên sản phẩm.
- **Ràng buộc:** Khi đơn hàng chuyển sang trạng thái "Paid", ObjectId của khách hàng mới chính thức được lưu vào lịch sử `usageHistory` của Collection `coupons` và tăng `usedCount` lên. Nếu khách hủy đơn, hệ thống chưa hỗ trợ hoàn mã tự động (theo thiết kế hiện tại).

**3.3.8. Phân hệ Đánh giá Sản phẩm (Verified Purchase Reviews)**
...
**3.3.9. Phân hệ Quản trị và Lưu vết (RBAC & Audit Logs)**
...
### 3.4. Quản trị hệ thống (System Administration)
...
**3.4.1. Quản lý và Phân quyền truy cập (Role-Based Access Control - RBAC)**
...
**3.4.2. Bảo mật Đăng nhập Quản trị (Admin Security & 2FA)**
...
**3.4.3. Quản lý Cấu hình Hệ thống (Store Configuration)**
...
**3.4.4. Ghi vết và Nhật ký hoạt động (Audit Logs)**
...
**3.4.5. Tự động hóa dọn dẹp hệ thống (AI & Cronjob Automation)**
...
**3.4.6. Bảo mật tầng Mạng và Chống tấn công (Network Security & Rate Limiting)**
...
**3.4.7. Quản lý Hàng đợi tác vụ ngầm (Background Jobs - BullMQ)**
...
> - **[http://localhost:5173/secret-dashboard](http://localhost:5173/secret-dashboard):** Đây là đường dẫn dành riêng để truy cập hệ thống Quản trị viên (Admin Dashboard). Hệ thống tách biệt URL so với trang khách hàng để đảm bảo bảo mật. Bạn có thể sử dụng tài khoản `admin@gmail.com` được cấp ở bước Seeding để đăng nhập vào khu vực này.
